...
This is some AI-generated code that massages the source code. You absolutely
*must* check its output before committing. It's possible that I may not even
run it in future, now that the new source style has been applied.
...
The brace policy is deliberately narrow and is applied repeatedly until no new
braces are required:

* A control statement that contains a braced body, directly or through a
  nested control statement, must itself have a braced body.
* If one direct branch of an if/else-if/else chain has a braced body, every
  direct branch in that chain must have a braced body.
* An if or else-if whose body contains another if or else-if, directly or
  through a nested control statement, must itself have a braced body. This
  rule applies even when neither body was previously braced.

The spacing policy makes one pass and adds one blank line after each eligible
closing brace line. Eligible lines contain only a closing brace, optionally
with a trailing comment, or a completed do-while terminator of the form
``} while (...);``. It does not add a blank line at end of file, inside a
protected region, before an existing blank line, before a line that starts
with a closing brace, or before an else or else-if branch.
"""
...
CLOSING_BRACE_RE = re.compile(r"^\s*}\s*(?:(?://.*)|(?:/\*.*\*/)|(?:while\b.*;))?\s*$")
...
def excluded_lines(lines):
    ...
    for line in lines:
        ...
        comment = block_comment or line.lstrip().startswith("/*")
        excluded.append(disabled or control is not None or preprocessor or comment)
    ...
    if control is not None:
        ...
        if kind in ("if", "else if"):
            ...
            if following is not None and control_at(lines, following, excluded) is not None:
                ...
                if following_kind in ("else", "else if"):
...
def body_contains_if(lines, body, excluded):
    """Determine whether a body's control structure contains an if branch."""
    ...
    if kind in ("if", "else if"):
        return True
    return body_contains_if(lines, body_for(lines, header_end, excluded), excluded)


def add_braces(add_before, add_after, lines, header_start, header_end, excluded):
    body = body_for(lines, header_end, excluded)
    ...
    if end is None:
        return
    indent = re.match(r"\s*", lines[header_start]).group(0)
...
def collect_brace_additions(lines):
    ...
    for index in range(len(lines)):
        ...
        if body is None:
            continue
        if not direct_braced(lines, body) and body_contains_brace(lines, body, excluded):
            add_braces(add_before, add_after, lines, index, header_end, excluded)

        # An if-family control that encloses another if-family control must have
        # braces, even if no body in that nesting is already braced.
        if (kind in ("if", "else if") and not direct_braced(lines, body) and body_contains_if(lines, body, excluded)):
            add_braces(add_before, add_after, lines, index, header_end, excluded)
        ...
        if kind != "if" or lines[index].lstrip().startswith("else"):
            continue
        branches = [(index, header_end, body)]
        end = statement_end(lines, body, excluded)
        while end is not None:
            ...
            if next_control is not None and next_control[0] == "else if":
            ...
            if next_body is None:
                break
            branches.append((following, next_header_end, next_body))
            end = statement_end(lines, next_body, excluded)

        if any(direct_braced(lines, branch_body) for _, _, branch_body in branches):
            for branch_start, branch_header_end, branch_body in branches:
                if not direct_braced(lines, branch_body):
                    add_braces(add_before, add_after, lines, branch_start, branch_header_end, excluded)
...
def add_closing_brace_spacing(lines):
    """Ensure a blank line follows each eligible close or do-while terminator."""
    ...
    for index, line in enumerate(lines):
        ...
        if index + 1 == len(lines):
            continue
        if index + 1 < len(lines):
            if (excluded[index + 1] or not lines[index + 1].strip() or lines[index + 1].lstrip().startswith("}")
                    or ELSE_FOLLOWING_RE.match(lines[index + 1]) is not None):
                continue
        rewritten.append("\n")
    return rewritten
