...
static const unsigned int utf8_table1[] = { 0x0000007f, 0x000007ff, 0x0000ffff,
                                            0x001fffff, 0x03ffffff, 0x7fffffff };

static const unsigned char utf8_table2[] = { 0, 0xc0, 0xe0, 0xf0, 0xf8, 0xfc };

static const unsigned char utf8_table3[] = { 0xff, 0x1f, 0x0f, 0x07, 0x03, 0x01 };
...
static int
utf82ord(const unsigned char *buffer, const unsigned char *buffend, long unsigned int *vptr,
         int *lenptr)
{
  ...
  for (; i < argc; i++)
  {
    ...
    else
    {
      ...
      for (;;)
      {
        ...
        y = y * 16 + (unsigned char)(tolower(*x) - ((isdigit(*x)) ? '0' : 'a' - 10));
        x++;
        if (z)
        {
          buffer[len++] = y;
      ...
      while (bptr < buffend)
      {
        ...
        else
        {
          ...
          {
          case 0:
            fputs("** First byte missing 0x40 bit", stdout);
            break;

          case -1:
            fputs("** First byte has too many high-order bits", stdout);
            break;

          case -2:
            fputs("** Incomplete UTF-8 sequence at end of string", stdout);
            break;

          case -3:
            fputs("** Incomplete UTF-8 sequence", stdout);
            break;

          default:
            printf("** Unexpected return %d from utf82ord()", rc);
            break;
          }
