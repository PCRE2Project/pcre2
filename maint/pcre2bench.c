...
static const char *const rare_needles[] = { "wombat", "user.name@example.com", "1234567890" };
...
static long
scan_all(pcre2_code *code, pcre2_match_data *match_data, PCRE2_SPTR subject, size_t length,
         int use_jit)
{
  PCRE2_SIZE offset = 0;
  long matches = 0;
  ...
  for (;;)
  {
    ...
    if (use_jit)
      ...
      rc = pcre2_match(code, subject, length, offset, PCRE2_NO_JIT, match_data, NULL);
...
static int
run_throughput(const char *label, PCRE2_SPTR subject, size_t length, int use_jit)
{
  int i;
  ...
  for (i = 0; i < BENCHMARK_COUNT; i++)
  {
    ...
    code = pcre2_compile(widen(benchmarks[i].pattern, pattern), PCRE2_ZERO_TERMINATED, 0,
    ...
    printf("%s\t%d\t%s\t%ld\t%.2f\t%.3f\t%d\t%s\n", label, PCRE2_CODE_UNIT_WIDTH,
           benchmarks[i].name, matches, best, 1e9 / (best * 1024.0 * 1024.0), use_jit,
           benchmarks[i].note);
    fflush(stdout);
...
#define SHORT_BENCHMARK_COUNT ((int)(sizeof(short_benchmarks) / sizeof(short_benchmarks[0])))

static const size_t short_lengths[] = { 8, 16, 32, 64, 128, 512 };

#define SHORT_LENGTH_COUNT ((int)(sizeof(short_lengths) / sizeof(short_lengths[0])))
...
run_short(const char *label, PCRE2_SPTR subject, size_t total, int use_jit)
{
  ...
  for (i = 0; i < SHORT_BENCHMARK_COUNT; i++)
    for (j = 0; j < SHORT_LENGTH_COUNT; j++)
    {
      ...
      code = pcre2_compile(widen(short_benchmarks[i].pattern, pattern), PCRE2_ZERO_TERMINATED, 0,
                           &errorcode, &erroroffset, NULL);
      if (code == NULL)
      {
        PCRE2_UCHAR message[TEXT_MAX];
      ...
      for (rep = 0; rep < REPEATS; rep++)
      {
        ...
        do
        {
          ...
          for (offset = 0; offset + slice <= total; offset += slice)
          {
            ...
            if (use_jit)
              rc = pcre2_jit_match(code, subject + offset, slice, 0, 0, match_data, NULL);
            else
              rc = pcre2_match(code, subject + offset, slice, 0, PCRE2_NO_JIT, match_data, NULL);
      ...
      is a constant for each pattern.  Check it. */
        ...
        if (best == 0.0 || per_call < best)
          best = per_call;
      }

      printf("%s\t%d\t%s\t%lu\t%ld\t%.1f\t%.2f\t%d\t%s\n", label, PCRE2_CODE_UNIT_WIDTH,
             short_benchmarks[i].name, (unsigned long)slice, expected, best,
             (double)slice / (best * 1e-9) / (1024.0 * 1024.0), use_jit, short_benchmarks[i].note);
      fflush(stdout);
...
main(int argc, char **argv)
{
  ...
  rc = short_mode ? run_short(label, subject, length, use_jit)
