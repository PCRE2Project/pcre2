...
static const unsigned char *type_names[] = { US "Cc", US "Control",
                                             US "Cf", US "Format",
                                             US "Cn", US "Unassigned",
...
static const unsigned char *gb_names[] = { US "CR",
                                           US "carriage return",
                                           US "LF",
                                           US "linefeed",
                                           US "Control",
                                           ...
                                           US "",
                                           US "L",
                                           US "Hangul syllable type L",
                                           US "V",
                                           US "Hangul syllable type V",
                                           US "T",
                                           US "Hangul syllable type T",
                                           US "LV",
                                           US "Hangul syllable type LV",
                                           US "LVT",
                                           US "Hangul syllable type LVT",
                                           US "Regional_Indicator",
                                           US "",
                                           US "Other",
                                           US "",
                                           US "ZWJ",
                                           US "zero width joiner",
                                           US "Extended_Pictographic",
                                           US "" };

static const unsigned char *bd_names[] = { US "AL",  US "ArabicLetter",
...
print_prop(unsigned int c, BOOL is_just_one)
{
  ...
  case ucp_N:
    ...
    break;
  case ucp_P:
    typename = US "Punctuation";
    break;
  case ucp_S:
    typename = US "Symbol";
    break;
  case ucp_Z:
    typename = US "Separator";
    break;
  }
  ...
  {
  case ucp_Cc:
    fulltypename = US "Control";
    break;
  case ucp_Cf:
    fulltypename = US "Format";
    break;
  case ucp_Cn:
    fulltypename = US "Unassigned";
    break;
  case ucp_Co:
    fulltypename = US "Private use";
    break;
  case ucp_Cs:
    fulltypename = US "Surrogate";
    break;
  case ucp_Ll:
    fulltypename = US "Lower case letter";
    break;
  case ucp_Lm:
    fulltypename = US "Modifier letter";
    break;
  case ucp_Lo:
    fulltypename = US "Other letter";
    break;
  case ucp_Lt:
    fulltypename = US "Title case letter";
    break;
  case ucp_Lu:
    fulltypename = US "Upper case letter";
    break;
  case ucp_Mc:
    fulltypename = US "Spacing mark";
    break;
  case ucp_Me:
    fulltypename = US "Enclosing mark";
    break;
  case ucp_Mn:
    fulltypename = US "Non-spacing mark";
    break;
  case ucp_Nd:
    fulltypename = US "Decimal number";
    break;
  case ucp_Nl:
    fulltypename = US "Letter number";
    break;
  case ucp_No:
    fulltypename = US "Other number";
    break;
  case ucp_Pc:
    fulltypename = US "Connector punctuation";
    break;
  case ucp_Pd:
    fulltypename = US "Dash punctuation";
    break;
  case ucp_Pe:
    fulltypename = US "Close punctuation";
    break;
  case ucp_Pf:
    fulltypename = US "Final punctuation";
    break;
  case ucp_Pi:
    fulltypename = US "Initial punctuation";
    break;
  case ucp_Po:
    fulltypename = US "Other punctuation";
    break;
  case ucp_Ps:
    fulltypename = US "Open punctuation";
    break;
  case ucp_Sc:
    fulltypename = US "Currency symbol";
    break;
  case ucp_Sk:
    fulltypename = US "Modifier symbol";
    break;
  case ucp_Sm:
    fulltypename = US "Mathematical symbol";
    break;
  case ucp_So:
    fulltypename = US "Other symbol";
    break;
  case ucp_Zl:
    fulltypename = US "Line separator";
    break;
  case ucp_Zp:
    fulltypename = US "Paragraph separator";
    break;
  case ucp_Zs:
    fulltypename = US "Space separator";
    break;
  }
  ...
  case ucp_gbLF:
    ...
    break;
  case ucp_gbControl:
    graphbreak = US "Control";
    break;
  case ucp_gbExtend:
    graphbreak = US "Extend";
    break;
  case ucp_gbPrepend:
    graphbreak = US "Prepend";
    break;
  case ucp_gbSpacingMark:
    graphbreak = US "SpacingMark";
    break;
  case ucp_gbL:
    graphbreak = US "Hangul syllable type L";
    break;
  case ucp_gbV:
    graphbreak = US "Hangul syllable type V";
    break;
  case ucp_gbT:
    graphbreak = US "Hangul syllable type T";
    break;
  case ucp_gbLV:
    graphbreak = US "Hangul syllable type LV";
    break;
  case ucp_gbLVT:
    graphbreak = US "Hangul syllable type LVT";
    break;
  case ucp_gbRegional_Indicator:
    graphbreak = US "Regional Indicator";
    break;
  case ucp_gbOther:
    graphbreak = US "Other";
    break;
  case ucp_gbZWJ:
    graphbreak = US "Zero Width Joiner";
    break;
  case ucp_gbExtended_Pictographic:
    graphbreak = US "Extended Pictographic";
    break;
  default:
    graphbreak = US "Unknown";
    break;
  }
  ...
  printf("U+%04X %s %s: %s, %s, %s", c, bidiclass, typename, fulltypename, scriptname, graphbreak);
...
find_chars(unsigned char *s)
{
  ...
  while (*s != 0)
  {
    ...
    *t = 0;
    ...
    if (strcmp(CS name, "script") == 0 || strcmp(CS name, "scriptx") == 0)
    {
      BOOL x = (name[6] == 'x');
    ...
    else if (strcmp(CS name, "bool") == 0)
    {
      ...
      {
        const ucp_type_table *u = PRIV(utt) + i;
        if (u->type == PT_BOOL && strcmp(CS(value + offset), PRIV(utt_names) + u->name_offset) == 0)
        {
    ...
    else if (strcmp(CS name, "bidi") == 0 || strcmp(CS name, "bidiclass") == 0 ||
  ...
  if (script < 0 && scriptx_count == 0 && bprop_count == 0 && type < 0 && gbreak < 0 &&
      bidiclass < 0)
  {
      ...
      for (i = 0; i < scriptx_count; i++)
      {
        ...
        if (scriptx_list[i] >= 0)
        {
          if (scriptx_list[i] == UCD_SCRIPT(c) ||
              ((scriptx_list[i] < ucp_Unknown) && (bits_scriptx[x] & (1u << y)) != 0))
            found++;
        }
        /* Negative requirement */
        else
        {
          if ((-(scriptx_list[i]) < ucp_Unknown) && (bits_scriptx[x] & (1u << y)) == 0)
            found++;
        }
      }

      if (found != scriptx_count)
        continue;
    }
...
main(int argc, char **argv)
{
  ...
  for (;;)
  {
...

    {
      if (interactive)
        printf("> ");
      if (fgets(CS buffer, sizeof(buffer), stdin) == NULL)
        break;
      if (!interactive)
        printf("%s", buffer);
    }
