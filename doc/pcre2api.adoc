...
the subject that is owned by the match data block instead. The copy contains
exactly the subject length that was passed to the matching function and is not
necessarily zero-terminated. It is freed when the match data block is reused
for another match or is freed. The returned pointer must not be used after that.
