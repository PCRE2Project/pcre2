...
For example, type:PCRE2_UCHAR16 is usually defined as [.c]`uint16_t`.
...
func:pcre2_compile_16(), you must do so with func:pcre2_match_16(), not
func:pcre2_match_8() or func:pcre2_match_32().
...
the file called _pcre2demo.c_ in the PCRE2 source distribution. A listing
...
unsigned integer type, currently always defined as _size_t_. The largest
...
section on func:pcre2_match() options
below.
...
[.c]`NULL`, but only one of them will be given the write lock, with the rest kept
...
[literal,subs=normal]
....
[.c]`void *private_malloc(PCRE2_SIZE, void *);`
[.c]`void  private_free(void *, void *);`
....
...
The following functions get, respectively, the convention for processing \R,
...
Enable/disable "auto-possessification" of variable quantifiers such as [.regex]`*` and [.regex]`+`.
This optimization, for example, turns [.regex]`a+b` into
[.regex]`a&#43;&#43;b` in order to avoid
backtracks into [.regex]`a&#43;&#43;` that can never be successful. However, if callouts are in
...
with [.regex]`.*` or with \A or \G or ^. Such a pattern is automatically anchored if
...
where [.regex]`(*MARK:1)` is encountered, but there is no "B", so the "last mark seen"
...
If none of these apply, just pass [.c]`NULL` as the context argument of
...
const:PCRE2_ERROR_NULL if either argument is [.c]`NULL`.
...
made by func:pcre2_substitute(). Details are given in the section entitled
...
For example, if the pattern [.regex]`/abc/` is matched against "123abc" with an offset
...
the value is in bytes; when requesting these values, _where_ should point
...
const:PCRE2_CONFIG_BSR::
      ...
      const:PCRE2_BSR_UNICODE means that \R matches any Unicode line ending sequence; a
      value of const:PCRE2_BSR_ANYCRLF means that \R matches only CR, LF, or CRLF. The
...
const:PCRE2_CONFIG_JITTARGET::
      The _where_ argument should point to a code-unit-aligned buffer. All
      ...
      applications should call func:pcre2_config() with `where` set to NULL to
...
const:PCRE2_ZERO_TERMINATED. A [.c]`NULL` pattern pointer with a length of zero is treated
as an empty string ([.c]`NULL` with a non-zero length causes an error return). The
function returns a pointer to a block of memory that contains the compiled
pattern and related data, or [.c]`NULL` if an error occurred.

If the compile context argument arg:ccontext is [.c]`NULL`, memory for the compiled
...
If func:pcre2_code_free() is called with a [.c]`NULL` argument, it returns
...
called with a [.c]`NULL` argument, it returns [.c]`NULL`.
...
func:pcre2_code_copy_with_tables() is called with a [.c]`NULL` argument, it returns
[.c]`NULL`.
...
contents of the _options_ argument specifies their settings at the start of
...
If arg:errorcode or arg:erroroffset is [.c]`NULL`, func:pcre2_compile() returns
[.c]`NULL` immediately. Otherwise, the variables to which these point are set to an
error code and an offset (number of code units) within the pattern,
respectively, when func:pcre2_compile() returns [.c]`NULL` because a compilation
error has occurred.
...
is successful _errorcode_ is set to a value that returns the message "no
error" if passed to func:pcre2_get_error_message().

The value returned in _erroroffset_ is an indication of where in the
...
(1) [.regex]`\U` matches an upper case "U" character; by default [.regex]`\U` causes a compile
time error (Perl uses \U to upper case subsequent characters).

(2) [.regex]`\u` matches a lower case "u" character unless it is followed by four
hexadecimal digits, in which case the hexadecimal number defines the code point
to match. By default, [.regex]`\u` causes a compile time error (Perl uses it to upper
case the following character).

(3) [.regex]`\x` matches a lower case "x" character unless it is followed by two
hexadecimal digits, in which case the hexadecimal number defines the code point
...
[.regex]`(*MARK:NAME)` is any sequence of characters that does not include a closing
...
included in a name either as \) or between \Q and \E. If the const:PCRE2_EXTENDED
...
LC (cased letter) when const:PCRE2_CASELESS is set.
...
If matched against "abc" with const:PCRE2_ENDANCHORED set, the first matches "c"
whereas the second matches "bc". The effect of const:PCRE2_ENDANCHORED can also be
...
quantifier and between a quantifier and a following [.regex]`+` that indicates
possessiveness. const:PCRE2_EXTENDED is equivalent to Perl's /x option, and it can be
...
created by
xref:pcre2_maketables.adoc[func:pcre2_maketables(),]
...
const:PCRE2_EXTENDED. These are `U+0085` (next line), `U+200E` (left-to-right mark),
...
const:PCRE2_DOTALL is set, the "any character" metacharacter (.) does not match at a
...
all the other branches also start with [.regex]`.*` or with \A or \G or ^. The
...
and const:PCRE2_MULTILINE is not set for any ^ items. Otherwise, the fact that any
...
such as \x{d800} you can set the const:PCRE2_EXTRA_ALLOW_SURROGATE_ESCAPES extra
...
Since release [.c]`10.38` PCRE2 has forbidden the use of [.regex]`\K` within lookaround
...
code points in the range [.c]`0xd800` to [.c]`0xdfff` are used in pairs in UTF-16 to encode
code points with values in the range [.c]`0x10000` to [.c]`0x10ffff`. The surrogates cannot
...
defined by ECMAscript [.c]`6`; setting const:PCRE2_EXTRA_ALT_BSUX has the effect of
...
automatically inserting the code [.regex]`^(?:` at the start of the compiled
...
If this option is set (note that its final character is the digit [.c]`0`) it locks
...
`uint32_t` variable. const:PCRE2_INFO_ARGOPTIONS returns exactly the options that
were passed to func:pcre2_compile(), whereas const:PCRE2_INFO_ALLOPTIONS returns
the compile options as modified by any top-level [.regex]`(*XXX)` option settings such as
...
unless const:PCRE2_MULTILINE is set
...
the \R escape sequence matches. A value of const:PCRE2_BSR_UNICODE means that \R
...
[.regex]`(*LIMIT_DEPTH=nnnn)` at the start, the value is returned. The third argument
...
[.regex]`(*LIMIT_HEAP=nnnn)` at the start, the value is returned. The third argument
should point to a uint32_t integer. If no such value has been set, the call to
func:pcre2_pattern_info() returns the error const:PCRE2_ERROR_UNSET. Note that this
limit will only be used during matching if it is less than the limit set or
defaulted by the caller of the match function.
...
(?-J) set and unset the local const:PCRE2_DUPNAMES option, respectively.
...
pattern [.regex]`/^a\d+z\d+/` the returned value is 1 (with "z" returned from
const:PCRE2_INFO_LASTCODEUNIT), but for /^a\dz\d/ the returned value is 0.
...
[.regex]`(*LIMIT_MATCH=nnnn)` at the start, the value is returned. The third argument
should point to a uint32_t integer. If no such value has been set, the call to
func:pcre2_pattern_info() returns the error const:PCRE2_ERROR_UNSET. Note that this
limit will only be used during matching if it is less than the limit set or
defaulted by the caller of the match function.
...
The map consists of a number of fixed-size entries. const:PCRE2_INFO_NAMECOUNT gives
the number of entries, and const:PCRE2_INFO_NAMEENTRYSIZE gives the size of each
...
argument is the number of pairs of offsets in the _ovector_.
...
pass [.c]`NULL`, which causes [.c]`malloc()` to be used.
...
context, but in this case if [.c]`NULL` is passed, the memory is obtained using the
...
const:PCRE2_COPY_MATCHED_SUBJECT option, which is described in the section entitled
"Option bits for func:pcre2_match()"
below.

When a match data block itself is no longer needed, it should be freed by
calling func:pcre2_match_data_free(). If this function is called with a [.c]`NULL`
...
UTF processing is enabled. As a special case, if arg:subject is [.c]`NULL` and
arg:length is zero, the subject is assumed to be an empty string. If
arg:length is non-zero, an error occurs if arg:subject is [.c]`NULL`.
...
which finds occurrences of [.c]`"iss"` in the middle of words. ([.regex]`\B` matches only if
the current position in the subject is not a word boundary.) When applied to
the string [.c]`"Mississippi"` the first call to func:pcre2_match() finds the first
occurrence. If func:pcre2_match() is called again with just the remainder of
the subject, namely [.c]`"issippi"`, it does not match, because [.regex]`\B` is always false
...
arg:start_offset set to [.c]`4`, it finds the second occurrence of `iss` because it
...
expected. For example, if the pattern is [.regex]`.+A` (and the const:PCRE2_DOTALL option is
not set), it does not match the string [.c]`"\r\nA"` because, after failing at the
...
[.c]`3`. If there are no captured substrings, the return value from a successful
match is [.c]`1`, indicating that just the first pair of offsets has been set.
...
match. For example, if the pattern [.regex]`(?=ab\K)` is matched against [.c]`"ab"`, the start
and end offset values for the match are [.c]`2` and [.c]`0`.
...
"abc" is matched against the pattern [.regex]`(a|(z[)](bc)` the return from the function
...
also set to const:PCRE2_UNSET. For example, if the string "abc" is matched against
...
parentheses, no more than [.c]`ovector[0]` to [.c]`ovector[2n+1]` are set by
...
func:pcre2_get_subject() may return [.c]`NULL`. If the arg:lengthptr argument
is not [.c]`NULL`, it receives the length of the string which is returned (the
subject string may contain embedded NUL characters).
...
available, [.c]`NULL` is returned. The length of the name (excluding the terminating
...
contains ([.regex]`*MARK:A`)([.regex]`*PRUNE`), the name "A" is returned. After a "no match" or a
...
The guarantees provided by func:pcre2_next_match() are that the [.c]`start_offset`
...
the error const:PCRE2_ERROR_PARTIAL. The next section describes similar functions for
extracting captured substrings by name.

If a pattern uses the [.regex]`\K` escape sequence within a positive lookahead assertion,
the reported start of a successful match can be greater than the end of the
match. For example, if the pattern [.regex]`(?=ab\K)` is matched against "ab", the start
...
[.regex]`(abc)|(def)` and the subject is "def", and the ovector contains at least two
...
therefore need the lengths, you may supply NULL as the arg:lengthsptr
...
the _replacement_ string, whose length is supplied in `rlength`, which
can be given as const:PCRE2_ZERO_TERMINATED for a zero-terminated string. As a
special case, if _replacement_ is NULL and _rlength_ is zero, the
replacement is assumed to be an empty string. If _rlength_ is non-zero, an
error occurs if _replacement_ is NULL.
...
permitted, and arg:match_data may be passed as [.c]`NULL`, in which case a match
...
If arg:match_data is not [.c]`NULL` and const:PCRE2_SUBSTITUTE_MATCHED is not set, the
...
([.regex]`$'` and [.regex]`$_`) cause const:PCRE2_ERROR_PARTIALSUBS to be returned when partial-matching,
...
using const:PCRE2_SUBSTITUTE_OVERFLOW_LENGTH.
...
Either a group number or a group name can be given for _n_, for example [.regex]`$2`
or [.regex]`$NAME`. Curly brackets are required only if the following character would be
...
"=abc=" and the replacement string [.regex]`$1$0$1`, the result is "=+babcb+=".

The JavaScript form [.regex]`$<name>`, where the angle brackets are part of the syntax,
is also recognized for group names, but not for group numbers or [.regex]`*MARK`.

The [.regex]`$*MARK` inserts the name from the last encountered backtracking control verb on
the matching path that has a name. ([.regex]`*MARK`) must always include a name, but the
other verbs need not. For example, in the case of ([.regex]`*MARK:A`)([.regex]`*PRUNE`) the name
inserted is "A", but for ([.regex]`*MARK:A`)([.regex]`*PRUNE:B`) the relevant name is "B". This
...
character. The usual forms such as [.regex]`\x{ddd}` can be used to specify particular
character codes, and backslash followed by any non-alphanumeric character
quotes that character. Extended quoting can be coded using [.regex]`\Q...\E`, exactly
as in pattern strings. The escapes [.regex]`\b` and [.regex]`\v` are interpreted as the
...
The Python form [.regex]`\g<n>`, where the angle brackets are part of the syntax and
_n_ is either a group name or number, is recognized as an alternative way
of inserting the contents of a group, for example [.regex]`\g<3>`.
...
groups and letters within [.regex]`\Q...\E` quoted sequences. The insertion mechanism
has three states: no case forcing, force upper case, and force lower case. The
escape sequences change the current state: [.regex]`\U` and [.regex]`\L` change to upper or lower
case forcing, respectively, and [.regex]`\E` (when not terminating a [.regex]`\Q` quoted
sequence) reverts to no case forcing. The sequences [.regex]`\u` and [.regex]`\l` force the next
...
However, if [.regex]`\u` is immediately followed by [.regex]`\L` or [.regex]`\l` is immediately followed
by [.regex]`\U`, the next character's case is forced by the first escape sequence, and
...
"heLLo", the replacement string [.regex]`\u\L$1` becomes "Hello".
...
Note that case forcing sequences such as [.regex]`\U...\E` do not nest. For example,
the result of processing [.regex]`\Uaa\LBB\Ecc\E` is "AAbbcc"; the final [.regex]`\E` has no
...
const:PCRE2_ERROR_NULL::
      ...
      arg:match_data argument is [.c]`NULL` or if the arg:subject or arg:replacement
      arguments are [.c]`NULL`. For backward compatibility reasons an exception is made for
      the arg:replacement argument if the arg:rlength argument is also [.c]`0`.
...
.Members
[.members]
[.c]`uint32_t` arg:version[.c]``;``::

[.c]`uint32_t` arg:subscount[.c]``;``::

type:PCRE2_SPTR arg:input[.c]``;``::

type:PCRE2_SPTR arg:output[.c]``;``::

type:PCRE2_SIZE [.c]``*``arg:ovector[.c]``;``::

[.c]`uint32_t` arg:oveccount[.c]``;``::

type:PCRE2_SIZE arg:output_offsets[.c]``[2];``::
...
One example implementation of the _callout_function_ using the ICU
library would be:
...
fourth arguments are [.c]`NULL`, the function returns a group number for a unique
name, or const:PCRE2_ERROR_NOUNIQUESUBSTRING otherwise.

When the third and fourth arguments are not [.c]`NULL`, they must be pointers to
...
above
in the section entitled _Information about a pattern_. Given all the
...
substring. Then return [.c]`1`, which forces func:pcre2_match() to backtrack and try
...
the arg:ovector, and can be extracted by number in the same way as for
...
The matched strings are stored in the arg:ovector in reverse order of length; that
is, the longest matching string is first. If there were too many matches to fit
into the arg:ovector, the yield of the function is zero, and the vector is filled
...
pattern [.regex]`a\d+` is compiled as if it were [.regex]`a\d++`. For DFA matching, this
means that only one possible match is found. If you really do want multiple
matches in such cases, either use an ungreedy repeat such as [.regex]`a\d&#43;?` or set
