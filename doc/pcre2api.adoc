...
mentioned, plus the single characters VT (vertical tab, Upass:[+]000B), FF (form feed,
Upass:[+]000C), NEL (next line, Upass:[+]0085), LS (line separator, Upass:[+]2028), and PS
(paragraph separator, Upass:[+]2029).
...
information such as the name of a (pass:[*]MARK) setting. Each thread must provide its
...
[literal,subs=normal]
....
`void pass:[*]private_malloc(PCRE2_SIZE, void pass:[*]);`
`void  private_free(void pass:[*], void pass:[*]);`
....

Whenever code in PCRE2 calls these functions, the final argument is the value
of _memory_data_. Either of the first two arguments of the creation
function may be NULL, in which case the system memory management functions
_malloc()_ and _free()_ are used. (This is not currently useful, as
there are no other fields in a general context, but in future there might be.)
The _private_malloc()_ function is used (if supplied) to obtain memory for
...
to the _free()_ function that matches the _malloc()_ function that was
...
nested parentheses. They return 0 on success or PCRE2_ERROR_NULL if either
argument is NULL.
...
The value returned by `pcre2_get_max_pattern_length()` is the largest
value a PCRE2_SIZE variable can hold when the limit is unlimited. Passing this
value to `pcre2_set_max_pattern_length()` makes the limit unlimited.
...
interpreted matching functions, _pcre2_match()_ and
_pcre2_dfa_match()_.
...
the _options_ argument of `pcre2_compile()` have been used up. To avoid
...
sequence such as (pass:[*]CRLF). See the
xref:pcre2pattern.adoc[`pcre2pattern`]
page for details.
...
functions, _pcre2_match()_ and _pcre2_dfa_match()_.
...
The permitted values of _directive_ are as follows:
...
Enable/disable "auto-possessification" of variable quantifiers such as pass:[*] and pass:[+].
This optimization, for example, turns apass:[+]b into apass:[+]pass:[+]b in order to avoid
backtracks into apass:[+] that can never be successful. However, if callouts are in
...
Enable/disable an optimization that is applied when .pass:[*] is the first significant
item in a top-level branch of a pattern, and all the other branches also start
with .pass:[*] or with \A or \G or ^. Such a pattern is automatically anchored if
PCRE2_DOTALL is set for all the .pass:[*] items and PCRE2_MULTILINE is not set for any
...
Dotstar anchor optimization is automatically disabled for .pass:[*] if it is inside an
atomic group or a capture group that is the subject of a backreference, or if
the pattern contains (pass:[*]PRUNE) or (pass:[*]SKIP).
...
that a special item such as (pass:[*]COMMIT) at the start of a pattern is not
considered until after a suitable starting point for the match has been found.
Also, when callouts or (pass:[*]MARK) items are in use, these "start-up" optimizations
...
match", the callouts do occur, and that items such as (pass:[*]COMMIT) and (pass:[*]MARK) are
...
attempt from there. The (pass:[*]COMMIT) item means that the pattern must match the
...
when this fails, (pass:[*]COMMIT) prevents any further matches being tried, so the
...
is long enough. In the process, (pass:[*]MARK:2) is encountered and remembered. When
...
where (pass:[*]MARK:1) is encountered, but there is no "B", so the "last mark seen"
...
PCRE2_ERROR_NULL if either argument is NULL.
...
The value returned by `pcre2_get_offset_limit()` is the largest value a
PCRE2_SIZE variable can hold when there is no offset limit. Passing this value
to `pcre2_set_offset_limit()` removes the offset limit.
...
The _offset_limit_ parameter limits how far an unanchored search can
...
found if the _startoffset_ argument of `pcre2_match()`,
...
The _heap_limit_ parameter specifies, in units of kibibytes (1024 bytes),
...
The _match_limit_ parameter provides a means of preventing PCRE2 from using
...
matching that goes on for a very long time, and so the _match_limit_ value
...
recursive pattern such as /(.(?2pass:[))]pass:[((]?1)|)/, when matched to a very long string
...
int pcre2_config(uint32_t _what_, void pass:[*]_where_);
...
Armv8.1 extension writes "ARM-64 (LSE) 64bit (little endian pass:[+] unaligned)". If
...
If the compile context argument _ccontext_ is NULL, memory for the compiled
...
pass:[NOTE:] When one of the matching functions is called, pointers to the compiled
...
The _options_ argument for `pcre2_compile()` contains various bit
...
If _errorcode_ or _erroroffset_ is NULL, `pcre2_compile()` returns
...
(pass:[*]MARK:NAME) is any sequence of characters that does not include a closing
...
their lower case ASCII equivalents, are case-equivalent with Upass:[+]212A (Kelvin
sign) and Upass:[+]017F (long S) respectively. If you do not want this case
...
succeeds by reaching (pass:[*]ACCEPT), but does not reach the end of the subject, the
...
quantifier and between a quantifier and a following pass:[+] that indicates
...
PCRE2_EXTENDED. These are Upass:[+]0085 (next line), Upass:[+]200E (left-to-right mark),
Upass:[+]200F (right-to-left mark), Upass:[+]2028 (line separator), and Upass:[+]2029 (paragraph
...
though the matched text may continue over the newline. If _startoffset_ is
...
PCRE2_FIRSTLINE if _startoffset_ is greater than 3. See also
...
pattern from enabling this facility by starting the pattern with (pass:[*]UCP). This
...
pattern with (pass:[*]UTF). This option may be useful in applications that process
...
is an optimization that, for example, turns apass:[+]b into apass:[+]pass:[+]b in order to avoid
backtracks into apass:[+] that can never be successful. However, if callouts are in
use, auto-possessification means that some callouts are never taken. You can
...
`pcre2_set_optimize()` with the _directive_ PCRE2_AUTO_POSSESS_OFF rather
...
when .pass:[*] is the first significant item in a top-level branch of a pattern, and
all the other branches also start with .pass:[*] or with \A or \G or ^. The
optimization is automatically disabled for .pass:[*] if it is inside an atomic group
or a capture group that is the subject of a backreference, or if the pattern
contains (pass:[*]PRUNE) or (pass:[*]SKIP). When the optimization is not disabled, such a
pattern is automatically anchored if PCRE2_DOTALL is set for all the .pass:[*] items
...
(If a compile context is available, it is recommended to use
`pcre2_set_optimize()` with the _directive_ PCRE2_DOTSTAR_ANCHOR_OFF
instead.)
...
`pcre2_set_optimize()` with the _directive_ parameter set to
PCRE2_START_OPTIMIZE_OFF.
...
(pass:[*]COMMIT) and (pass:[*]MARK). See the above description of PCRE2_START_OPTIMIZE_OFF
for further details.
...
characters. The ASCII letter S is case-equivalent to Upass:[+]017f (long S) and the
ASCII letter K is case-equivalent to Upass:[+]212a (Kelvin sign). This option disables
...
can be changed within a pattern by the (pass:[*]CASELESS_RESTRICT) or (?r) option
settings.
...
a pattern by the (pass:[*]TURKISH_CASING) start-of-pattern setting. Either the UTF or
...
pass:[256.] By default, higher-valued code points never match escapes such as \w or
\d.
...
[literal,subs=normal]
....
PCRE2_ERROR_NULL           the argument _code_ was NULL
PCRE2_ERROR_BADMAGIC       the "magic number" was not found
PCRE2_ERROR_BADOPTION      the value of _what_ was invalid
...
the compile options as modified by any top-level (pass:[*]XXX) option settings such as
(pass:[*]UTF) at the start of the pattern itself. PCRE2_INFO_EXTRAOPTIONS returns the
extra options that were set in the compile context by calling the
pcre2_set_compile_extra_options() function.

For example, if the pattern /(pass:[*]UTF)abc/ is compiled with the PCRE2_EXTENDED
...
When .pass:[*] is the first significant item, anchoring is possible only when all the
following are true:
...
(pass:[*]LIMIT_DEPTH=nnnn) at the start, the value is returned. The third argument
...
(pass:[*]LIMIT_HEAP=nnnn) at the start, the value is returned. The third argument
should point to a uint32_t integer. If no such value has been set, the call to
`pcre2_pattern_info()` returns the error PCRE2_ERROR_UNSET. Note that this
limit will only be used during matching if it is less than the limit set or
defaulted by the caller of the match function.
...
pass:[0.] The third argument should point to a `uint32_t` variable. (?J) and
...
pattern /^a\dpass:[+]z\dpass:[+]/ the returned value is 1 (with "z" returned from
...
(pass:[*]LIMIT_MATCH=nnnn) at the start, the value is returned. The third argument
should point to a uint32_t integer. If no such value has been set, the call to
`pcre2_pattern_info()` returns the error PCRE2_ERROR_UNSET. Note that this
limit will only be used during matching if it is less than the limit set or
defaulted by the caller of the match function.
...
pass:[(?<=a(?<=ba)c)] returns a maximum lookbehind of 2, but when it is processed, the
...
compiled pattern, which is passed in the _code_ argument. You can call
`pcre2_match()` with the same _code_ argument as many times as you
...
_subject_, a length in _length_, and a starting offset in
_startoffset_. The length and offset are in code units, not characters.
...
UTF processing is enabled. As a special case, if _subject_ is NULL and
_length_ is zero, the subject is assumed to be an empty string. If
_length_ is non-zero, an error occurs if _subject_ is NULL.

If _startoffset_ is greater than the length of the subject,
...
Setting _startoffset_ differs from passing over a shortened string and
...
_startoffset_ set to 4, it finds the second occurrence of "iss" because it
...
the use of .pass:[*] with PCRE2_DOTALL, not by starting the pattern with ^ or \A.

=== Option bits for `pcre2_match()`

The unused bits of the _options_ argument for `pcre2_match()` must be
...
string as a subject, or an invalid value of _startoffset_, is undefined.
...
pattern string with, for example, (pass:[*]CRLF), as described in the
...
expected. For example, if the pattern is .pass:[+]A (and the PCRE2_DOTALL option is
...
of offsets (that is, _ovector[0]_ and _ovector[1]_) are set. They
...
pass:[3.] If there are no captured substrings, the return value from a successful
...
It is possible for capture group number _npass:[+]1_ to match some part of the
subject when group _n_ has not been used at all. For example, if the string
"abc" is matched against the pattern (a|(zpass:[))](bc) the return from the function
...
parentheses, no more than _ovector[0]_ to _ovector[2npass:[+]1]_ are set by
...
(pass:[*]MARK). The same function applies to all the verbs. It returns a pointer to
...
contains (pass:[*]MARK:A)(pass:[*]PRUNE), the name "A" is returned. After a "no match" or a
...
`pcre2_compile()` or by starting the pattern with (pass:[*]NO_START_OPT).
...
different to the value of _ovector[0]_ if the pattern contains the \K
escape sequence. After a partial match, however, this value is always the same
as _ovector[0]_ because \K does not affect the result of a partial match.
...
The value of _startoffset_ was greater than the length of the subject.
...
An unrecognized bit was set in the _options_ argument.
...
_startoffset_ did not point to the beginning of a UTF character or the end
of the subject.
...
Either the _code_, _subject_, or _match_data_ argument was passed
as NULL.
...
The pass:[*]_pstart_offset_ and pass:[*]_poptions_ are set if the function returns 1.
The pass:[*]_pstart_offset_ should be passed to the next match attempt directly,
and the pass:[*]_poptions_ should be passed to the next match attempt by combining
...
(b) the returned pass:[*]_pstart_offset_ is strictly greater than the previous
start_offset; or (c) if the previous match was a successful match of the empty
string then the returned pass:[*]_pstart_offset_ is equal to the previous
ovector[1], and pass:[*]_poptions_ will be set to PCRE2_NOTEMPTY_ATSTART to prevent
...
The address of the memory block is returned via _listptr_, which is also
...
_lengthsptr_. If your strings do not contain binary zeros and you do not
...
capture group number _npass:[+]1_ matches some part of the subject, but group
...
subject string in _outputbuffer_, replacing parts that were matched with
...
permitted, and _match_data_ may be passed as NULL, in which case a match
...
If _match_data_ is not NULL and PCRE2_SUBSTITUTE_MATCHED is not set, the
...
options can be set in the _options_ argument of `pcre2_substitute()`.
One such option is PCRE2_SUBSTITUTE_MATCHED. When this is set, an external
_match_data_ block must be provided, and it must have already been used for
...
option arguments (substitute-specific options can be added to the _options_
argument). If any of these parameters is changed, `pcre2_substitute()`
returns an error. The data in the _match_data_ block (return code, offset
...
The _code_ argument is not used for matching before the first substitution
...
The _outlengthptr_ argument of `pcre2_substitute()` must point to a
...
If the function is not successful, the value set via _outlengthptr_ depends
...
_outlengthptr_ variable, with the result of the function still being
PCRE2_ERROR_NOMEMORY.
...
(pass:[*]MARK) or other control verbs in the pattern. Dollar is the only escape
...
$pass:[+]                  insert the highest-numbered capture group which matched
$pass:[*]MARK or ${pass:[*]MARK}  insert a control verb name
....

Either a group number or a group name can be given for _n_, for example $2
...
"=abc=" and the replacement string "pass:[+]$1$0$1pass:[+]", the result is "=pass:[+]babcbpass:[+]=".

The JavaScript form $<name>, where the angle brackets are part of the syntax,
is also recognized for group names, but not for group numbers or pass:[*]MARK.

$pass:[*]MARK inserts the name from the last encountered backtracking control verb on
the matching path that has a name. (pass:[*]MARK) must always include a name, but the
other verbs need not. For example, in the case of (pass:[*]MARK:A)(pass:[*]PRUNE) the name
inserted is "A", but for (pass:[*]MARK:A)(pass:[*]PRUNE:B) the relevant name is "B". This
...
Iteration is implemented by advancing the _startoffset_ value for each
...
subject string by setting either or both of _startoffset_ and an offset
...
needed is returned via _outlengthptr_. Note that this does not happen by
default.

PCRE2_ERROR_NULL is returned if PCRE2_SUBSTITUTE_MATCHED is set but the
_match_data_ argument is NULL or if the _subject_ or _replacement_
arguments are NULL. For backward compatibility reasons an exception is made for
the _replacement_ argument if the _rlength_ argument is also 0.
...
....
uint32_t    _version_;
uint32_t    _subscount_;
PCRE2_SPTR  _input_;
PCRE2_SPTR  _output_;
PCRE2_SIZE _pass:[*]ovector_;
uint32_t    _oveccount_;
PCRE2_SIZE  _output_offsets[2]_;
....
...
The _subscount_ field is the number of the current match. It is 1 for the
first callout, 2 for the second, and so on. The _input_ and _output_
pointers are copies of the values passed to `pcre2_substitute()`.

The _ovector_ field points to the ovector, which contains the result of the
most recent match. The _oveccount_ field contains the number of pairs that
are set in the ovector, and is always greater than zero.

The _output_offsets_ vector contains the offsets of the replacement in the
...
_callout_data_ when the function was registered. The value returned by the
...
The sixth argument is the _callout_data_ supplied to
...
The unused bits of the _options_ argument for `pcre2_dfa_match()` must
...
_workspace_ and _wscount_ options must reference the same vector as
...
pass:[NOTE:] PCRE2's "auto-possessification" optimization usually applies to character
repeats at the end of a pattern (as well as internally). For example, the
pattern "a\dpass:[+]" is compiled as if it were "a\dpass:[+]pass:[+]". For DFA matching, this
means that only one possible match is found. If you really do want multiple
matches in such cases, either use an ungreedy repeat such as "a\dpass:[+]?" or set
...
itself recursively, using private memory for the ovector and _workspace_.
