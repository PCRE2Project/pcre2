= pcre2serialize(3)
:doctype: manpage
:manname: pcre2serialize
:manpurpose: PCRE2 serialized patterns
:description: Serializing functions for saving precompiled patterns
:manvolnum: 3
:manmanual: PCRE2
:mansource: PCRE2 10.49-DEV
:docdate: 19 January 2024
:revdate: 19 January 2024
:toc: right
...
and type:PCRE2_SIZE type. For example, patterns compiled on a 32-bit system using
...
func:pcre2_serialize_decode() is expected to be trusted data, not data from
...
The function func:pcre2_serialize_encode() creates a serialized byte stream
...
memory management functions. If this argument is const:NULL, [.c]`malloc()` is used
...
const:PCRE2_ERROR_BADDATA::
  the number of patterns is zero or less
...
const:PCRE2_ERROR_NULL::
  The 1st, 3rd, or 4th argument is const:NULL
...
them to a file. It assumes that the variable [.c]`fd` refers to a file that is
...
way by calling func:pcre2_code_free(). When you have finished with the byte
stream, it too must be freed by calling func:pcre2_serialize_free(). If this
function is called with a const:NULL argument, it returns immediately without doing
anything.
...
func:pcre2_serialize_get_number_of_codes() function to find out how many
...
The func:pcre2_serialize_decode() function reads a byte stream and recreates
...
management functions for the decoded patterns. If this argument is const:NULL,
[.c]`malloc()` and [.c]`free()` are used. After deserialization, the byte
...
[.errors]
const:PCRE2_ERROR_BADDATA::
  second argument is zero or less
const:PCRE2_ERROR_BADMAGIC::
  mismatch of id bytes in the data
const:PCRE2_ERROR_BADMODE::
  mismatch of code unit size or PCRE2 version
const:PCRE2_ERROR_BADSERIALIZEDDATA::
  other sanity check failure
const:PCRE2_ERROR_MEMORY::
  memory allocation failed
const:PCRE2_ERROR_NULL::
  first or third argument is const:NULL
...
by calling func:pcre2_code_free(). However, be aware that there is a potential
...
to call func:pcre2_code_free() for these patterns in different threads, you
must arrange your own locking, and ensure that func:pcre2_code_free() cannot
be called by two threads at the same time.

If a pattern was processed by func:pcre2_jit_compile() before being
serialized, the JIT data is discarded and so is no longer available after a
save/restore cycle. You can, however, process a restored pattern with
func:pcre2_jit_compile() if you wish.
