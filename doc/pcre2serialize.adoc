...
memory management functions. If this argument is [.c]`NULL`, [.c]`malloc()` is used
...
const:PCRE2_ERROR_NULL::
  The 1st, 3rd, or 4th argument is [.c]`NULL`
...
them to a file. It assumes that the variable _fd_ refers to a file that is
...
function is called with a [.c]`NULL` argument, it returns immediately without doing
anything.
...
management functions for the decoded patterns. If this argument is [.c]`NULL`,
...
const:PCRE2_ERROR_MEMORY::
  memory allocation failed
const:PCRE2_ERROR_NULL::
  first or third argument is [.c]`NULL`
