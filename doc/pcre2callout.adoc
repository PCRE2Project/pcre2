...
:description: The callout facility
:manvolnum: 3
:manmanual: PCRE2
...
must be one of [.regex]`'`, [.regex]`"`, [.regex]`^`, [.regex]`%`, [.regex]`#`, [.regex]`$`, [.regex]`{` and the ending delimiter is the same as the
start, except for [.regex]`{`, where the ending delimiter is [.regex]`}`. If the ending delimiter
...
This indicates that when matching [.regex]`+[bc]+` fails, there is no backtracking into [.regex]`a{plus}`
(because it is being treated as [.regex]`a{plus}{plus}`) and therefore the callouts that would be
...
This time, when matching [.regex]`+[bc]+` fails, the matcher backtracks into [.regex]`a{plus}` and tries
again, repeatedly, until [.regex]`a{plus}` itself fails.
...
applied to the string [.subject]`aa`, the `pcre2test` output is:
...
Another optimization, described in the section <<other-optimizations>>, means that there is no
...
PCRE2 knows that any matching string must contain the letter [.subject]`d`. If the
subject string is [.subject]`abyz`, the lack of [.subject]`d` means that matching doesn't ever
start, and the callout is never reached. However, with [.subject]`abyd`, though the
...
to a type:pcre2_callout block. The second argument is the [.c]`+void *+` callout data
...
and the arg:callout_flags field for version 2. If you are writing an
...
For a numerical callout, arg:callout_string is const:NULL, and arg:callout_number
...
For callouts with string arguments, arg:callout_number is always zero, and
arg:callout_string points to the string that is contained within the compiled
pattern. Its length is given by arg:callout_string_length. Duplicated ending
...
The arg:callout_string_offset field is the code unit offset to the start of
...
The arg:offset_vector field is a pointer to a vector of capturing offsets
(the "ovector"). You may read the elements in this vector, but you must not
change any of them.

For calls to func:pcre2_match(), the arg:offset_vector field is not (since
...
The arg:capture_last field contains the number of the most recently captured
substring, and the arg:capture_top field contains one more than the number of
the highest numbered captured substring so far. If no substrings have yet been
captured, the value of arg:capture_last is 0 and the value of
arg:capture_top is 1. The values of these fields do not always differ by one;
for example, when the callout in the pattern [.regex]`+((a)(b))(?C2)+` is taken,
arg:capture_last is 1 but arg:capture_top is 4.

The contents of [.c]`+ovector[2]+` to [.c]`+ovector[capture_top*2-1]+` can be inspected in
order to extract substrings that have been matched so far, in the same way as
extracting substrings after a match has completed. The values in [.c]`ovector[0]` and
[.c]`ovector[1]` are always const:PCRE2_UNSET because the match is by definition not
complete. Substrings that have not been captured but whose numbers are less
than arg:capture_top also have both of their ovector slots set to
const:PCRE2_UNSET.

For DFA matching, the arg:offset_vector field points to the ovector that was
...
capturing. The value of arg:capture_top is always 1 and the value of
arg:capture_last is always 0 for DFA matching.

The arg:subject and arg:subject_length fields contain copies of the values
that were passed to the matching function.

The arg:start_match field normally contains the offset within the subject at
...
The arg:current_position field contains the offset within the subject of the
current match pointer.

The arg:pattern_position field contains the offset in the pattern string to
the next item to be matched.

The arg:next_item_length field contains the length of the next item to be
...
callout before an assertion such as [.regex]`(?=ab)` the length is 3. For an alternation
...
The arg:pattern_position and arg:next_item_length fields are intended to
...
In callouts from func:pcre2_match() the arg:mark field contains a pointer to
the zero-terminated name of the most recently passed [.regex]`+(*MARK)+`, [.regex]`+(*PRUNE)+`, or
[.regex]`+(*THEN)+` item in the match, or const:NULL if no such items have been passed. Instances
of [.regex]`+(*PRUNE)+` or [.regex]`+(*THEN)+` without a name do not obliterate a previous [.regex]`+(*MARK)+`. In
callouts from the DFA matching function this field always contains const:NULL.

The arg:callout_flags field is always zero in callouts from
...
The information in the arg:callout_flags field is provided so that
...
cases the arg:callout_flags field is always zero.
...
Negative values should normally be chosen from the set of `PCRE2_ERROR_{empty}__xxx__`
...
PCRE2_SPTR callout_string;        /* Points to callout string or is
                                     NULL */
----
...
Note that the value of arg:pattern_position is unique for each callout.
...
pattern. For example, a pattern such as [.regex]`(a){2}` is compiled as if it were
[.regex]`(a)(a)`. This means that the callout will be enumerated more than once, but
with the same value for arg:pattern_position in each case.
