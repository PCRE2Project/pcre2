...
:doctype: manpage
:manname: pcre2test
:manpurpose: a program for testing Perl-compatible regular expressions
:description: The `pcre2test` command for testing PCRE2
:manvolnum: 1
:manmanual: PCRE2
:mansource: PCRE2 10.49-DEV
:docdate: 22 August 2026
:revdate: 22 August 2026
:toc: right

== Synopsis

[source,shell,role=synopsis]
----
...
are given in generic form, for example, func:pcre2_compile(). The actual
names used in the libraries have a suffix `_8`, `_16`, or `_32`, as appropriate.

== Input encoding

Input to `pcre2test` is processed line by line, either by calling the C
library's [.c]`fgets()` function, or via the `libreadline` or `libedit`
library. In some Windows environments character 26 (char:0x1A) causes an immediate
...
zeros, even though in Unix-like environments, [.c]`fgets()` treats any bytes
...
character values up to char:0x7fffffff. Each character is placed in one 16-bit or
32-bit code unit (in the 16-bit case, values greater than char:0xffff cause an error
to occur).

UTF-8 (in its original definition) is not capable of encoding values greater
than char:0x7fffffff, but such values can be handled by the 32-bit library. When
testing this library in non-UTF mode with `utf8_input` set, if any
character is preceded by the byte char:0xff (which is an invalid byte in UTF-8)
char:0x80000000 is added to the character's value. For subject strings, using an
...
+
--
`linksize` :: Output the configured internal link size (2, 3, or 4).
              The exit code is set to the link size.
`newline`  :: Output the default newline setting:
              CR, LF, CRLF, ANYCRLF, ANY, or NUL.
              The exit code is always 0.
`bsr`      :: Output the default setting for what [.regex]`\R` matches:
...
+
--
`backslash-C`  :: [.regex]`\C` is supported (not locked out)
`ebcdic`       :: compiled for an EBCDIC environment
`ebcdic-io`    :: if PCRE2 is compiled for EBCDIC, whether ``pcre2test``'s input and
                  output is EBCDIC or ASCII
`ebcdic-nl25`  :: if PCRE2 is compiled for EBCDIC, whether NL (= LF) is char:0x25
                  (otherwise it is char:0x15, the default)
...
3:: Unexpected return from test call to func:pcre2_jit_compile()
--
...
With `auto`, the output is coloured if the output is to a terminal. Force or
suppress output of ANSI colour escapes with `always` (or no specifier
provided) and `never` respectively. If no colour option is provided, the
default is `auto`, unless the NO_COLOR environment variable is defined and
non-empty.
...
using the func:pcre2_dfa_match() function instead of the default
func:pcre2_match().
...
`-error __number__[,__number__,\...]`::
Call func:pcre2_get_error_message() for each of the error numbers in the
...
`-malloc`::
Exercise [.c]`malloc()` failures, by first counting the number of calls made to malloc
...
`-S _size_`::
On Unix-like systems, set the size of the run-time stack to _size_
mebibytes (units of 1024*1024 bytes).

`-subject _modifier-list_`::
...
is done, if the input is from a terminal, it is read using the [.c]`readline()`
function. This provides line-editing and history facilities. The output from
the `-help` option states whether or not [.c]`readline()` will be used.
...
Subsequent patterns automatically have the const:PCRE2_NEVER_UTF and const:PCRE2_NEVER_UCP
options set, which locks out the use of the const:PCRE2_UTF and const:PCRE2_UCP options and
the use of ([.regex]`+*UTF+`) and ([.regex]`+*UCP+`) at the start of patterns. This command also forces
an error if a subsequent pattern contains any occurrences of [.regex]`\P`, [.regex]`\p`, or [.regex]`\X`,
which are still supported when const:PCRE2_UTF is not set, but which require Unicode
...
Unicode support is not included in the library. Setting const:PCRE2_NEVER_UTF and
const:PCRE2_NEVER_UCP as a default can also be obtained by the use of `+#pattern+`;
...
described in the section <<saving-restoring-patterns>>
below.
...
in the section <<saving-restoring-patterns>>
below.
...
does not work within [.regex]`+\Q...\E+` literal bracketing because the backslash will
...
Before each subject line is passed to func:pcre2_match(),
func:pcre2_dfa_match(), or func:pcre2_jit_match(), leading and trailing white
...
`\a`              :: alarm (BEL, char:0x07)
`\b`              :: backspace (char:0x08)
`\e`              :: escape (char:0x27)
`\f`              :: form feed (char:0x0c)
`\n`              :: newline (char:0x0a)
`\N{U{plus}__hh\...__}` :: unicode character (any number of hex digits)
`\r`              :: carriage return (char:0x0d)
`\t`              :: tab (char:0x09)
`\v`              :: vertical tab (char:0x0b)
`{backslash}__ddd__` ::
                     octal number (up to 3 octal digits); represent a single
                     code point unless larger than 255 with the 8-bit library
`\o{__dd\...__}`  :: octal number (any number of octal digits) representing a
                     character in UTF mode or a code point
`\x__hh__`        :: hexadecimal byte (up to 2 hex digits)
`\x{__hh\...__}`  :: hexadecimal number (up to 8 hex digits) representing a
                     character in UTF mode or a code point

Invoking `\N{U{plus}__hh\...__}` or `\x{__hh\...__}` doesn't require the use of the `utf`
...
but when using `\N{U{plus}__hh\...__}` with some invalid unicode characters they will
be accepted with a warning instead.

Note that even in UTF-8 mode, `\x__hh__` (and depending of how large, `{backslash}__ddd__`)
...
`\x{__hh\...__}` is interpreted as a UTF-8 character in UTF-8 mode, only generating
more than one byte if the value is greater than 127. To avoid the ambiguity
it is preferred to use `\N{U{plus}__hh__\...}` when describing characters. When testing
the 8-bit library not in UTF-8 mode, `\x{__hh__}` generates one byte for values
that could fit on it, and causes an error for greater values.

When testing the 16-bit library, not in UTF-16 mode, all 4-digit `\x{__hhhh__}`
...
The following modifiers set options for func:pcre2_compile(). Most of them set
bits in the options argument of that function, but those whose names start with
const:PCRE2_EXTRA are additional options that are set in the compile context.
Some of these options have single-letter abbreviations. There is special
handling for [.regex]`+/x+`: if a second x is present, const:PCRE2_EXTENDED is converted into
const:PCRE2_EXTENDED_MORE as in Perl. A third appearance adds const:PCRE2_EXTENDED as well,
though this makes no difference to the way func:pcre2_compile() behaves. See
xref:pcre2api.adoc[`pcre2api`]
for a description of the effects of these options.

[.options]
`allow_empty_class`:: set const:PCRE2_ALLOW_EMPTY_CLASS
`allow_lookaround_bsk`:: set const:PCRE2_EXTRA_ALLOW_LOOKAROUND_BSK
`allow_surrogate_escapes`:: set const:PCRE2_EXTRA_ALLOW_SURROGATE_ESCAPES
`alt_bsux`:: set const:PCRE2_ALT_BSUX
`alt_circumflex`:: set const:PCRE2_ALT_CIRCUMFLEX
`alt_extended_class`:: set const:PCRE2_ALT_EXTENDED_CLASS
`alt_verbnames`:: set const:PCRE2_ALT_VERBNAMES
`anchored`:: set const:PCRE2_ANCHORED
`+/a  ascii_all+`:: set all ASCII options
`ascii_bsd`:: set const:PCRE2_EXTRA_ASCII_BSD
`ascii_bss`:: set const:PCRE2_EXTRA_ASCII_BSS
`ascii_bsw`:: set const:PCRE2_EXTRA_ASCII_BSW
`ascii_digit`:: set const:PCRE2_EXTRA_ASCII_DIGIT
`ascii_posix`:: set const:PCRE2_EXTRA_ASCII_POSIX
`auto_callout`:: set const:PCRE2_AUTO_CALLOUT
`bad_escape_is_literal`:: set const:PCRE2_EXTRA_BAD_ESCAPE_IS_LITERAL
`/i  caseless`:: set const:PCRE2_CASELESS
`+/r  caseless_restrict+`:: set const:PCRE2_EXTRA_CASELESS_RESTRICT
`dollar_endonly`:: set const:PCRE2_DOLLAR_ENDONLY
`/s  dotall`:: set const:PCRE2_DOTALL
`dupnames`:: set const:PCRE2_DUPNAMES
`endanchored`:: set const:PCRE2_ENDANCHORED
`escaped_cr_is_lf`:: set const:PCRE2_EXTRA_ESCAPED_CR_IS_LF
`/x  extended`:: set const:PCRE2_EXTENDED
`+/xx extended_more+`:: set const:PCRE2_EXTENDED_MORE
`extra_alt_bsux`:: set const:PCRE2_EXTRA_ALT_BSUX
`firstline`:: set const:PCRE2_FIRSTLINE
`literal`:: set const:PCRE2_LITERAL
`match_line`:: set const:PCRE2_EXTRA_MATCH_LINE
`match_invalid_utf`:: set const:PCRE2_MATCH_INVALID_UTF
`match_unset_backref`:: set const:PCRE2_MATCH_UNSET_BACKREF
`match_word`:: set const:PCRE2_EXTRA_MATCH_WORD
`/m  multiline`:: set const:PCRE2_MULTILINE
`never_backslash_c`:: set const:PCRE2_NEVER_BACKSLASH_C
`never_callout`:: set const:PCRE2_EXTRA_NEVER_CALLOUT
`never_ucp`:: set const:PCRE2_NEVER_UCP
`never_utf`:: set const:PCRE2_NEVER_UTF
`+/n  no_auto_capture+`:: set const:PCRE2_NO_AUTO_CAPTURE
`no_auto_possess`:: set const:PCRE2_NO_AUTO_POSSESS
`no_bs0`:: set const:PCRE2_EXTRA_NO_BS0
`no_dotstar_anchor`:: set const:PCRE2_NO_DOTSTAR_ANCHOR
`no_start_optimize`:: set const:PCRE2_NO_START_OPTIMIZE
`no_utf_check`:: set const:PCRE2_NO_UTF_CHECK
`python_octal`:: set const:PCRE2_EXTRA_PYTHON_OCTAL
`turkish_casing`:: set const:PCRE2_EXTRA_TURKISH_CASING
`ucp`:: set const:PCRE2_UCP
`ungreedy`:: set const:PCRE2_UNGREEDY
`use_offset_limit`:: set const:PCRE2_USE_OFFSET_LIMIT
`utf`:: set const:PCRE2_UTF

As well as turning on the const:PCRE2_UTF option, the `utf` modifier causes all
non-printing characters in output strings to be printed using the [.regex]`\x{__hh__\...}`
notation. Otherwise, those less than char:0x100 are output in hex without the curly
...
calling func:pcre2_set_optimize() before invoking the regex compiler.
...
`auto_possess_off`::
don't auto-possessify variable quantifiers

`dotstar_anchor`::
anchor patterns starting with [.regex]`+.*+`

`dotstar_anchor_off`::
don't anchor patterns starting with [.regex]`+.*+`
...
`+max_pattern_compiled_length=<n>+`:: set maximum compiled pattern length (bytes)
...
`posix_nosub`:: use the POSIX API with [.c]`REG_NOSUB`
...
The `bsr` modifier specifies what [.regex]`\R` in a pattern should match. If it is
set to "anycrlf", [.regex]`\R` matches CR, LF, or CRLF only. If it is set to "unicode",
[.regex]`\R` matches any Unicode newline sequence. The default can be specified when
...
The `fullbincode` modifier, by contrast, _does_ include length and
offset values. This is used in a few special tests that run only for specific
code unit widths and link sizes, and is also useful for one-off tests.

The `info` modifier requests information about the compiled pattern
(whether it is anchored, has a fixed first character, and so on). The
information is obtained from the func:pcre2_pattern_info() function. Here are
...
used by func:pcre2_match() for handling backtracking. The size depends on the
...
=== Passing a const:NULL context

Normally, `pcre2test` passes a context block to func:pcre2_compile(). If
the `null_context` modifier is set, however, const:NULL is passed. This is for
testing that func:pcre2_compile() behaves correctly in this case (it uses
default values).

=== Passing a const:NULL pattern

The `null_pattern` modifier is for testing the behaviour of
func:pcre2_compile() when the pattern argument is const:NULL. The length value
passed is the default const:PCRE2_ZERO_TERMINATED unless `use_length` is set.
...
`max_varlookbehind` modifier uses the func:pcre2_set_max_varlookbehind()
...
are expanded before the pattern is passed to func:pcre2_compile(). For
...
const:PCRE2_PARTIAL_HARD and const:PCRE2_PARTIAL_SOFT are going to be used, because
...
to func:pcre2_match() with either the const:PCRE2_PARTIAL_SOFT or the
const:PCRE2_PARTIAL_HARD option set. Note that such a call may return a complete
...
"fast path" interface, func:pcre2_jit_match(), which skips some of the sanity
checks that are done by func:pcre2_match(), and of course does not work when
...
The given locale is set, func:pcre2_maketables() is called to build a set of
character tables for the locale, and this is then passed to
func:pcre2_compile() when compiling the regular expression. The same tables
...
type:pcre2_code block; it is just the actual compiled data. If the pattern is
...
length of pattern that func:pcre2_compile() will accept. Breaching the limit
causes a compilation error. The default is the largest number a type:PCRE2_SIZE
...
compilation error. The default is the largest number a type:PCRE2_SIZE variable can
...
`posix_nosub` is used, the POSIX option [.c]`REG_NOSUB` is passed to
[.c]`regcomp()`. The POSIX wrapper supports only the 8-bit library. Note that
...
`caseless`  :: const:REG_ICASE
`multiline` :: const:REG_NEWLINE
`dotall`    :: const:REG_DOTALL
`ungreedy`  :: const:REG_UNGREEDY
`ucp`       :: const:REG_UCP
`utf`       :: const:REG_UTF8

The four options const:REG_DOTALL, const:REG_UNGREEDY, const:REG_UCP, and const:REG_UTF8 are not part of the
POSIX standard.

The `regerror_buffsize` modifier specifies a size for the error buffer that
is passed to [.c]`regerror()` in the event of a compilation error. For example:

....
/abc/posix,regerror_buffsize=20
....

This provides a means of testing the behaviour of [.c]`regerror()` when the
...
The pattern is passed to [.c]`regcomp()` as a zero-terminated string by
default, but if the `use_length` or `hex` modifiers are set, the
[.c]`REG_PEND` extension is used to pass it by length.

=== Testing the stack guard feature

The `stackguard` modifier is used to test the use of
func:pcre2_set_compile_recursion_guard(), a function that is provided to
...
than zero, func:pcre2_set_compile_recursion_guard() is called to set up
callback from func:pcre2_compile() to a local function. The argument it
...
to func:pcre2_compile(). This is used in the PCRE2 tests to check behaviour
with different character tables. The digit specifies the tables as follows:

0:: do not pass any special character tables
1:: the default ASCII tables, as distributed in `pcre2_chartables.c.dist`
...
`substitute_extended`:: use const:PCRE2_SUBSTITUTE_EXTENDED
`substitute_literal`:: use const:PCRE2_SUBSTITUTE_LITERAL
`substitute_matched`:: use const:PCRE2_SUBSTITUTE_MATCHED
`substitute_overflow_length`:: use const:PCRE2_SUBSTITUTE_OVERFLOW_LENGTH
`substitute_replacement_only`:: use const:PCRE2_SUBSTITUTE_REPLACEMENT_ONLY
`+substitute_skip=<n>+`:: skip substitution <n>
`+substitute_stop=<n>+`:: skip substitution <n> and following
`substitute_unknown_unset`:: use const:PCRE2_SUBSTITUTE_UNKNOWN_UNSET
`substitute_unset_empty`:: use const:PCRE2_SUBSTITUTE_UNSET_EMPTY
...
section <<saving-restoring-patterns>>
below.
...
options, which set the equivalent option for the func:pcre2_pattern_convert()
function:

[.options]
`glob`:: const:PCRE2_CONVERT_GLOB
`glob_no_starstar`:: const:PCRE2_CONVERT_GLOB_NO_STARSTAR
`glob_no_wild_separator`:: const:PCRE2_CONVERT_GLOB_NO_WILD_SEPARATOR
`posix_basic`:: const:PCRE2_CONVERT_POSIX_BASIC
`posix_extended`:: const:PCRE2_CONVERT_POSIX_EXTENDED
...
passed to func:pcre2_pattern_convert(). If the conversion is successful, the
result is reflected in the output and then passed to func:pcre2_compile(). The
normal `utf` and `no_utf_check` options, if set, cause the
const:PCRE2_CONVERT_UTF and const:PCRE2_CONVERT_NO_UTF_CHECK options to be passed to
func:pcre2_pattern_convert().
...
The following modifiers set options for func:pcre2_match() or
func:pcre2_dfa_match(). See
xref:pcre2api.adoc[`pcre2api`]
for a description of their effects.

[.options]
`anchored`::
set const:PCRE2_ANCHORED

`copy_matched_subject`::
set const:PCRE2_COPY_MATCHED_SUBJECT

`endanchored`::
set const:PCRE2_ENDANCHORED

`dfa_restart`::
set const:PCRE2_DFA_RESTART

`dfa_shortest`::
set const:PCRE2_DFA_SHORTEST

`disable_recurseloop_check`::
set const:PCRE2_DISABLE_RECURSELOOP_CHECK

`no_jit`::
set const:PCRE2_NO_JIT

`no_utf_check`::
set const:PCRE2_NO_UTF_CHECK

`notbol`::
set const:PCRE2_NOTBOL

`notempty`::
set const:PCRE2_NOTEMPTY

`notempty_atstart`::
set const:PCRE2_NOTEMPTY_ATSTART

`noteol`::
set const:PCRE2_NOTEOL

`partial_hard` (or `ph`)::
set const:PCRE2_PARTIAL_HARD

`partial_soft` (or `ps`)::
set const:PCRE2_PARTIAL_SOFT
...
causing [.c]`REG_NOTBOL`, [.c]`REG_NOTEMPTY`, and [.c]`REG_NOTEOL`, respectively, to be passed to
[.c]`regexec()`. The other modifiers are ignored, with a warning message.
...
This causes the subject string to be passed to [.c]`regexec()` using the
[.c]`REG_STARTEND` option, which uses offsets to specify which part of the string is
searched. If only one number is given, the end offset is passed as the end of
the subject string. For more detail of [.c]`REG_STARTEND`, see the
xref:pcre2posix.adoc[`pcre2posix`]
...
`null_substitute_buffer`:: substitute with a NULL output buffer
`null_substitute_match_data`:: substitute with NULL match data
...
or, for a complete match, a lookahead at the end, or if [.regex]`\K` is used in the
...
time when this occurs is when [.regex]`\K` has been processed as part of the match. In
...
code from func:pcre2_match()). Groups that did not take part in the match
...
`callout_`. Details are given in the section <<callouts>>
below.
Testing callouts from func:pcre2_substitute() is described separately in
...
arg:start_offset argument to func:pcre2_match() or func:pcre2_dfa_match()
...
assertion (including [.regex]`\b` or [.regex]`\B`).

If an empty string is matched, the next match is done with the
const:PCRE2_NOTEMPTY_ATSTART flag set, in order to search for another, non-empty,
match at the same point in the subject. This imitates the way Perl handles such
cases when using the [.regex]`+/g+` modifier or the `split()` function.

=== Testing substring extraction functions

The `copy` and `get` modifiers can be used to test the
func:pcre2_substring_copy_xxx() and func:pcre2_substring_get_xxx() functions.
...
The `getall` modifier tests func:pcre2_substring_list_get(), which
...
If the `replace` modifier is set, the func:pcre2_substitute() function is
called instead of one of the matching functions (or after one call of
func:pcre2_match() in the case of const:PCRE2_SUBSTITUTE_MATCHED). Note that
...
for func:pcre2_substitute():

[.options]
`global`:: const:PCRE2_SUBSTITUTE_GLOBAL
`substitute_extended`:: const:PCRE2_SUBSTITUTE_EXTENDED
`substitute_literal`:: const:PCRE2_SUBSTITUTE_LITERAL
`substitute_matched`:: const:PCRE2_SUBSTITUTE_MATCHED
`substitute_overflow_length`:: const:PCRE2_SUBSTITUTE_OVERFLOW_LENGTH
`substitute_replacement_only`:: const:PCRE2_SUBSTITUTE_REPLACEMENT_ONLY
`substitute_unknown_unset`:: const:PCRE2_SUBSTITUTE_UNKNOWN_UNSET
`substitute_unset_empty`:: const:PCRE2_SUBSTITUTE_UNSET_EMPTY
...
number in square brackets, that number is passed to func:pcre2_substitute() as
...
The default action of func:pcre2_substitute() is to return
const:PCRE2_ERROR_NOMEMORY when the output buffer is too small. However, if the
const:PCRE2_SUBSTITUTE_OVERFLOW_LENGTH option is set (by using the
`substitute_overflow_length` modifier), func:pcre2_substitute() continues
...
API, in which a client calls func:pcre2_match() followed by func:pcre2_substitute()
with const:PCRE2_SUBSTITUTE_MATCHED, but the client performs an unexpected and
...
set to a value greater than zero the callout function returns [.c]`+1` for the match
...
the match context via func:pcre2_set_heap_limit(),
func:pcre2_set_match_limit(), or func:pcre2_set_depth_limit() until it finds
...
such as ([.regex]`+*LIMIT_MATCH=...+`) within it. If such a setting is present and is
lower than the minimum matching value, the minimum value cannot be found
because func:pcre2_set_match_limit() etc. are only able to reduce the value of
...
In the case of DFA matching, `depth_limit` controls the depth of
...
are returned from calls to func:pcre2_match() to be displayed. If a mark is
...
func:pcre2_match() or func:pcre2_dfa_match(). In the latter case, heap memory
...
func:pcre2_match() without JIT. After a match has run (whether successful or
...
several calls to func:pcre2_match() (for example, global matching or for
...
[.c]`regexec()` to be called with a const:NULL capture vector. When not testing the
POSIX API, a value of zero is used to cause
func:pcre2_match_data_create_from_pattern() to be called, in order to create a
...
be passed as const:PCRE2_ZERO_TERMINATED. When matching via the POSIX interface,
this modifier is ignored, with a warning.

When testing func:pcre2_substitute(), this modifier also has the effect of
passing the replacement string as zero-terminated.

=== Passing a const:NULL context, subject, replacement, or substitute buffer

Normally, `pcre2test` passes a context block to func:pcre2_match(),
func:pcre2_dfa_match(), func:pcre2_jit_match() or func:pcre2_substitute().
If the `null_context` modifier is set, however, const:NULL is passed. This is for
...
Similarly, for testing purposes, if the `null_subject`,
or `null_replacement`, or `null_substitute_buffer` modifiers are set, then the
subject, or replacement string, or substitute output buffer pointers are passed
as const:NULL, respectively, to the relevant functions. For `null_substitute_buffer`,
...
func:pcre2_match() to match each subject line. PCRE2 also supports an
alternative matching function, func:pcre2_dfa_match(), which operates in a
...
func:pcre2_match(), is being used.
...
Otherwise, it outputs "No match" when the return is const:PCRE2_ERROR_NOMATCH, or
"Partial match:" followed by the partially matching substring when the
return is const:PCRE2_ERROR_PARTIAL. (Note that this is the
entire substring that was inspected during the partial match; it may include
characters before the actual match start if a lookbehind assertion, [.regex]`\K`, [.regex]`\b`,
or [.regex]`\B` was involved.)
...
are output as `\x{__hh__\...}` escapes. See below for the definition of non-printing
...
When the alternative matching function, func:pcre2_dfa_match(), is used, the
...
const:PCRE2_ERROR_PARTIAL return, the output is "Partial match:", followed by the
partially matching substring. Note that this is the entire substring that was
inspected during the partial match; it may include characters before the actual
match start if a lookbehind assertion, [.regex]`\b`, or [.regex]`\B` was involved. ([.regex]`\K` is not
...
When the alternative matching function has given the const:PCRE2_ERROR_PARTIAL
...
the seventh character, and when the next pattern item was [.regex]`\d`. Just
...
If a pattern contains [.regex]`+(*MARK)+` items, an additional line is output whenever
...
func:pcre2_dfa_match() does not support capturing, so no captures are ever
shown.
...
When using the interpretive matching function func:pcre2_match() without JIT,
...
for any match. If `no_auto_possess` is not used, the [.regex]`a+` item is turned
into [.regex]`a++`, which reduces the number of backtracks.
...
const:PCRE2_ERROR_CALLOUT is returned, causing the entire matching process to be
...
[.c]`isprint()` function is used to distinguish printing and non-printing
characters.

[[saving-restoring-patterns]]
...
width and type:PCRE2_SIZE type. Before compiled patterns can be saved they must be
...
The functions whose names begin with [.c]`pcre2_serialize_` are used
...
xref:pcre2.adoc[`pcre2`], xref:pcre2api.adoc[`pcre2api`], xref:pcre2callout.adoc[`pcre2callout`],
xref:pcre2jit.adoc[`pcre2jit`], xref:pcre2matching.adoc[`pcre2matching`], xref:pcre2partial.adoc[`pcre2partial`],
xref:pcre2pattern.adoc[`pcre2pattern`], xref:pcre2serialize.adoc[`pcre2serialize`].
