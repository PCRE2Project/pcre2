...
\N{Upass:[+]hh..} is synonymous with \x{hh..} but is not supported in environments
...
application can lock out the use of (pass:[*]UTF) and (pass:[*]UCP) by setting the
...
(pass:[*]scan_substring:(grouplist)...), and may identify groups in any of the
following ways:
...
All backtracking control verbs may be in the form (pass:[*]VERB:NAME). For (pass:[*]MARK) the
name is mandatory, for the others it is optional. (pass:[*]SKIP) changes its behaviour
if :NAME is present. The others just set a name for passing back to the caller,
but this is not a name that (pass:[*]SKIP) can see. The following act immediately they
are reached:
...
pass:[1.] Backslash is an escape character, and the forms described in "ESCAPED
...
pass:[2.] The Python form \g<n>, where the angle brackets are part of the syntax and
...
pass:[3.] Capture substitution supports the following additional forms:
