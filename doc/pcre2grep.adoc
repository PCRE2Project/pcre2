...
xref:pcre2syntax.adoc[`pcre2syntax`(3)]
for a quick-reference summary of pattern syntax, or
xref:pcre2pattern.adoc[`pcre2pattern`(3)]
...
Patterns can be no longer than 8KiB or [.c]`BUFSIZ` bytes, whichever is the greater.
...
`-A` _number_, `--after-context=number`::
...
`-B` _number_, `--before-context=number`::
...
`-D` _action_, `--devices=action`::
If an input path is not a regular file or a directory, "action" specifies how
it is to be processed. Valid values are "read" (the default) or "skip"
(silently skip the path).

`-d` _action_, `--directories=action`::
...
`-e` _pattern_, `--regex=pattern`, `--regexp=pattern`::
...
`-onumber`, `--only-matching`=_number_::
...
strings, see the
xref:pcre2unicode.adoc[`pcre2unicode`(3)]
documentation.
...
A callout in a PCRE2 pattern is of the form [.regex]`(?C<arg>)` where the argument is
...
repeats, for example: [.regex]`(a+)*\d` when matched against a line of a's with no final
