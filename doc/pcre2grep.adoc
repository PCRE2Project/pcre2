...
:doctype: manpage
:manname: pcre2grep
:manpurpose: a grep with Perl-compatible regular expressions
:description: The `pcre2grep` command
:manvolnum: 1
:manmanual: PCRE2
:mansource: PCRE2 10.49-DEV
:docdate: 24 January 2025
:revdate: 24 January 2025
:toc: right
...
the beginning and end of a subject string ([.regex]`+^+`, [.regex]`$`, [.regex]`\A`, [.regex]`\Z`, and [.regex]`\z`) match at
...
Patterns can be no longer than 8KiB or const:BUFSIZ bytes, whichever is the greater.
...
matches are never recognized. An example is the pattern [.regex]`+(super)?(man)?+`, in
...
`-A _number_`, `--after-context=__number__`::
Output up to _number_ lines of context after each matching line. Fewer
...
_number_ is expected to be relatively small. When `-c` is used,
`-A` is ignored.
...
PCRE2 now forbids the use of [.regex]`\K` in lookarounds by default, in line with Perl.
This option causes `pcre2grep` to set the const:PCRE2_EXTRA_ALLOW_LOOKAROUND_BSK
option, which enables this somewhat dangerous usage.

`-B _number_`, `--before-context=__number__`::
Output up to _number_ lines of context before each matching line. Fewer
lines are output if the previous match or the start of the file is within
_number_ lines, or if the processing buffer size has been set too small. If
...
input file. The value of _number_ is expected to be relatively small. When
`-c` is used, `-B` is ignored.

`--binary-files=_word_`::
Specify how binary files are to be processed. If the word is `binary` (the
default), pattern matching is performed on binary files, but the only output is
`Binary file _name_ matches` when a match succeeds. If the word is `text`,
...
`-C _number_`, `--context=__number__`::
Output _number_ lines of context both before and after each matching line.
...
`--colour=__value__`, `--color=__value__`::
...
`-D _action_`, `--devices=__action__`::
If an input path is not a regular file or a directory, _action_ specifies how
it is to be processed. Valid values are `read` (the default) or `skip`
(silently skip the path).

`-d _action_`, `--directories=__action__`::
If an input path is a directory, _action_ specifies how it is to be processed.
...
`--depth-limit=__number__`::
See `--match-limit` below.

`-E`, `--case-restrict`::
When case distinctions are being ignored in Unicode mode, two ASCII letters (K
and S) will by default match Unicode characters char:U+212A (Kelvin sign) and char:U+017F
...
`-e _pattern_`, `--regex=__pattern__`, `--regexp=__pattern__`::
...
`-f _filename_`, `--file=__filename__`::
...
reading the file is the operating system's default interpretation of [.c]`\n`. The
...
internal occurrences of [.regex]`+^+` and [.regex]`$` characters, because in multiline mode these can
match at internal newlines. Because `pcre2grep` is scanning multiple lines,
the [.regex]`\Z` and [.regex]`\z` assertions match only at the end of the last line in the file.
The [.regex]`\A` assertion matches at the start of the first line of a match. This can
...
the pattern. For example, to find the phrase [.subject]`regular expression` in a file
where [.subject]`regular` might be at the end of a line and [.subject]`expression` at the start of
...
+
The [.regex]`\s` escape sequence matches any white space character, including newlines,
and is followed by [.regex]`+` so as to match trailing white space on the first line as
...
`-m _number_`, `--max-count=__number__`::
Stop processing after finding _number_ matching lines, or non-matching
...
_number_. This option has no effect if used with `-L`, `-l`, or
`-q`, or when just checking for a match in a binary file.

`--match-limit=__number__`::
...
are the three just mentioned, plus VT (vertical tab, char:U+000B), FF (form feed,
...
+
[.repl]`$__digits__` or [.repl]`${__digits__}` is replaced by the captured substring of the given
decimal number; [.repl]`$&` (or the legacy [.repl]`$0`) substitutes the whole match. If the
number is greater than the number of capturing substrings, or if the capture
is unset, the replacement is empty.

+
[.repl]`$a` is replaced by bell; [.repl]`$b` by backspace; [.repl]`$e` by escape; [.repl]`$f` by form feed; [.repl]`$n` by
newline; [.repl]`$r` by carriage return; [.repl]`$t` by tab; [.repl]`$v` by vertical tab.

+
[.repl]`$o__digits__` or [.repl]`$o{__digits__}` is replaced by the character whose code point is
the given octal number. In the first form, up to three octal digits are processed.
When more digits are needed in Unicode mode to specify a wide character, the
second form must be used.

+
[.repl]`$x__digits__` or [.repl]`$x{__digits__}` is replaced by the character represented by
the given hexadecimal number. In the first form, up to two hexadecimal digits are
processed. When more digits are needed in Unicode mode to specify a wide
character, the second form must be used.

+
Any other character is substituted by itself. In particular, [.repl]`$$` is replaced by
a single dollar.
...
`-o__number__`, `--only-matching=__number__`::
...
or `-U`, the const:PCRE2_UCP option is used by default. This means that the
POSIX classes in patterns match more than just ASCII characters. For example,
[.regex]`+[:digit:]+` matches any Unicode decimal digit. The `--no-ucp` option
suppresses const:PCRE2_UCP, thus restricting the POSIX classes to ASCII characters,
...
when in UCP mode, the sequence [.regex]`+(?aP)+` restricts [.regex]`+[:word:]+` to ASCII letters, while
allowing [.regex]`\w` to match Unicode letters and digits.
...
included as part of it, so if you really need to have pattern ending in [.regex]`\r`,
...
`-v`, `--invert-match`::
Invert the sense of the match, so that lines which do _not_ match any of
...
having [.regex]`+\b(?:+` at the start of each pattern, and [.regex]`+)\b+` at the end. This
option applies only to the patterns that are matched against the contents of
files; it does not apply to patterns specified by any of the `--include` or
`--exclude` options.
...
be more than one line. This is equivalent to having [.regex]`+^(?:+` at the start of each
pattern and [.regex]`)$` at the end. This option applies only to the patterns that are
...
in the GNU `grep` program. Any long option of the form `--__xxx__-regexp`
(GNU terminology) is also available as `--__xxx__-regex` (PCRE2 terminology).
...
A callout in a PCRE2 pattern is of the form [.regex]`(?C__arg__)` where the argument is
...
above). However, [.repl]`$0` or [.repl]`$&` cannot be used to insert a matched substring because
...
pattern with [.regex]`+(*FAIL)+`.

=== Calling external programs or scripts

This facility can be independently disabled when `pcre2grep` is built. It
is supported for Windows, where a call to [.c]`_spawnvp()` is used, for VMS,
where [.c]`+lib$spawn()+` is used, and for any Unix-like environment where
[.c]`fork()` and [.c]`execv()` are available.
...
[literal,subs=normal]
....
_executable_name_|_arg1_|_arg2_|\...
....
...
(`-O`) option documented above, except that [.repl]`$0` or [.repl]`$&` cannot insert the
...
substring, use [.repl]`$$` or [.repl]`$|` respectively. Here is an example:
...
repeats, for example: [.regex]`(a{plus})*\d` when matched against a line of [.subject]``a``s with no final
...
When run under VMS, the return code is placed in the symbol [.c]`PCRE2GREP_RC`
because VMS does not distinguish between [.c]`exit(0)` and [.c]`exit(1)`.

== See also

xref:pcre2pattern.adoc[`pcre2pattern`], xref:pcre2syntax.adoc[`pcre2syntax`], xref:pcre2callout.adoc[`pcre2callout`],
xref:pcre2unicode.adoc[`pcre2unicode`].
