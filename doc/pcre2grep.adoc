...
to signify multiplication by 1024 or 1024pass:[*]1024 respectively.
...
and S) will by default match Unicode characters Upass:[+]212A (Kelvin sign) and Upass:[+]017F
...
and is followed by pass:[+] so as to match trailing white space on the first line as
...
are the three just mentioned, plus VT (vertical tab, Upass:[+]000B), FF (form feed,
...
repeats, for example: (apass:[+])pass:[*]\d when matched against a line of a's with no final
