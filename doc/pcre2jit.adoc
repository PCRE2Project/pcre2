...
pointer to an opaque structure of type type:pcre2_jit_stack, or NULL if there
...
[literal,subs=normal]
....
(1) If _callback_ is NULL and _data_ is NULL, an internal 32KiB block
...
[literal,subs=normal]
....
(2) If _callback_ is NULL and _data_ is not NULL, _data_ must be
...
[literal,subs=normal]
....
(3) If _callback_ is not NULL, it must point to a function that is
    called with _data_ as an argument at the start of matching, in
