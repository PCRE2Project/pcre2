...
:description: Details of size and other limits
:manvolnum: 3
:manmanual: PCRE2
...
the largest number a type:PCRE2_SIZE variable can hold. However, the program that
calls func:pcre2_compile() can specify a smaller limit.

The maximum length (in code units) of a subject string is one less than the
largest number a type:PCRE2_SIZE variable can hold. type:PCRE2_SIZE is an unsigned
integer type, usually defined as type:size_t. Its maximum value (that is
[.c]`+~(PCRE2_SIZE)0+`) is reserved as a special indicator for zero-terminated strings
and unset offsets.
...
change this limit by calling func:pcre2_set_parens_nest_limit() to set the limit in
a compile context.
...
The maximum length of a name in a [.regex]`+(*MARK)+`, [.regex]`+(*PRUNE)+`, [.regex]`+(*SKIP)+`, or [.regex]`+(*THEN)+` verb
