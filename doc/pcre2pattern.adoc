= pcre2pattern(3)
:doctype: manpage
:manname: pcre2pattern
:manpurpose: PCRE2 pattern syntax
:description: Specification of the regular expressions supported by PCRE2
:manvolnum: 3
:manmanual: PCRE2
:mansource: PCRE2 10.49-DEV
:docdate: 25 October 2025
:revdate: 25 October 2025
:toc: right
...
PCRE2 when its main matching function, func:pcre2_match(), is used. PCRE2 also
has an alternative matching function, func:pcre2_dfa_match(), which matches
...
an EBCDIC environment are described in the section <<ebcdic-environments>>
below, which you can ignore unless you really are in an EBCDIC environment.

== Special start-of-pattern items

A number of options that can be passed to func:pcre2_compile() can also be set
...
you must either call the compiling function with one or both of the const:PCRE2_UTF
or const:PCRE2_MATCH_INVALID_UTF options, or the pattern must start with the special
sequence [.regex]`+(*UTF)+`, which is equivalent to setting the relevant const:PCRE2_UTF. How
...
restrict them to non-UTF data for security reasons. If the const:PCRE2_NEVER_UTF
option is passed to func:pcre2_compile(), [.regex]`+(*UTF)+` is not allowed, and its
appearance in a pattern causes an error.

=== Unicode property support

Another special sequence that may appear at the start of a pattern is [.regex]`+(*UCP)+`.
This has the same effect as setting the const:PCRE2_UCP option: it causes sequences
such as [.regex]`+\d+` and [.regex]`+\w+` to use Unicode properties to determine character types,
...
These behaviours can be changed within the pattern; see the section
<<internal-option-setting>>
below.

Some applications that allow their users to supply patterns may wish to
restrict them for security reasons. If the const:PCRE2_NEVER_UCP option is passed to
func:pcre2_compile(), [.regex]`+(*UCP)+` is not allowed, and its appearance in a pattern
causes an error.

=== Locking out empty string matching

Starting a pattern with [.regex]`+(*NOTEMPTY)+` or [.regex]`+(*NOTEMPTY_ATSTART)+` has the same effect
as passing the const:PCRE2_NOTEMPTY or const:PCRE2_NOTEMPTY_ATSTART option to whichever
...
If a pattern starts with [.regex]`+(*NO_AUTO_POSSESS)+`, it has the same effect as setting
the const:PCRE2_NO_AUTO_POSSESS option, or calling func:pcre2_set_optimize() with
a const:PCRE2_AUTO_POSSESS_OFF directive. This stops PCRE2 from making quantifiers
possessive when what follows cannot match the repeated item. For example, by
default [.regex]`a+b` is treated as [.regex]`a++b`. For more details, see the
xref:pcre2api.adoc[`pcre2api`]
documentation.

=== Disabling start-up optimizations

If a pattern starts with [.regex]`+(*NO_START_OPT)+`, it has the same effect as setting the
const:PCRE2_NO_START_OPTIMIZE option, or calling func:pcre2_set_optimize() with
a const:PCRE2_START_OPTIMIZE_OFF directive. This disables several optimizations for
...
If a pattern starts with [.regex]`+(*NO_DOTSTAR_ANCHOR)+`, it has the same effect as
setting the const:PCRE2_NO_DOTSTAR_ANCHOR option, or
calling func:pcre2_set_optimize() with a const:PCRE2_DOTSTAR_ANCHOR_OFF directive.
This disables optimizations that apply to patterns whose top-level branches
all start with [.regex]`+.*+` (match any number of arbitrary characters). For more details,
see the
xref:pcre2api.adoc[`pcre2api`]
documentation.
...
If a pattern that starts with [.regex]`+(*NO_JIT)+` is successfully compiled, an attempt by
the application to apply the JIT optimization by calling
func:pcre2_jit_compile() is ignored.

=== Setting match resource limits

The func:pcre2_match() function contains a counter that is incremented every
time it goes round its main loop. The caller of func:pcre2_match() can set a
...
these limits is reached, func:pcre2_match() gives an error return. The limits
...
where _d_ is any number of decimal digits. However, the value of the setting must
be less than the value set (or defaulted) by the caller of func:pcre2_match()
...
The heap limit applies only when the func:pcre2_match() or
func:pcre2_dfa_match() interpreters are used for matching. It does not apply
to JIT. The match limit is used (but in a different way) when JIT is being
used, or when func:pcre2_dfa_match() is called, to limit computing resource
...
changes the convention to CR. That pattern matches [.subject]`a\nb` because LF is no
longer a newline. If more than one of these settings is present, the last one
is used.

The newline convention affects where the circumflex and dollar assertions are
true. It also affects the interpretation of the dot metacharacter when
const:PCRE2_DOTALL is not set, and the behaviour of [.regex]`\N` when not followed by an
opening brace. However, it does not affect what the [.regex]`\R` escape sequence
matches. By default, this is any Unicode newline sequence, for Perl
compatibility. However, this can be changed; see the section
<<specifying-r-matches>> and the
description of [.regex]`\R` in the section <<newline-sequences>>
below. A change of [.regex]`\R` setting can be combined with a change of newline
convention.

[[specifying-r-matches]]
=== Specifying what [.regex]`\R` matches

It is possible to restrict [.regex]`\R` to match only CR, LF, or CRLF (instead of the
complete set of Unicode line endings) by setting the option const:PCRE2_BSR_ANYCRLF
at compile time. This effect can also be achieved by starting a pattern with
[.regex]`+(*BSR_ANYCRLF)+`. For completeness, [.regex]`+(*BSR_UNICODE)+` is also recognized,
corresponding to const:PCRE2_BSR_UNICODE.
...
caseless matching is specified (the const:PCRE2_CASELESS option or [.regex]`+(?i)+` within the
...
equivalents, are case-equivalent with Unicode char:U+212A (Kelvin sign) and char:U+017F
(long S) respectively when either const:PCRE2_UTF or const:PCRE2_UCP is set, unless the
const:PCRE2_EXTRA_CASELESS_RESTRICT option is in force (either passed to
func:pcre2_compile() or set by [.regex]`+(*CASELESS_RESTRICT)+` or [.regex]`+(?r)+` within the
pattern). If the const:PCRE2_EXTRA_TURKISH_CASING option is in force (either passed
to func:pcre2_compile() or set by [.regex]`+(*TURKISH_CASING)+` within the pattern), then
...
[.regex]`+\+`    :: general escape character with several uses
[.regex]`+^+`    :: assert start of string (or line, in multiline mode)
[.regex]`+$+`    :: assert end of string (or line, in multiline mode)
[.regex]`+.+`    :: match any character except newline (by default)
[.regex]`+[+`    :: start character class definition
[.regex]`{vbar}` :: start of alternative branch
[.regex]`+(+`    :: start group or control verb
[.regex]`+)+`    :: end group or control verb
[.regex]`+*+`    :: 0 or more quantifier
[.regex]`{plus}` :: 1 or more quantifier; also "possessive quantifier"
[.regex]`+?+`    :: 0 or 1 quantifier; also quantifier minimizer
[.regex]`+{+`    :: potential start of min/max quantifier

Brace characters [.regex]`+{+` and [.regex]`+}+` are also used to enclose data for constructions such
as [.regex]`+\g{2}+` or [.regex]`\k{__name__}`. In almost all uses of braces, space and/or horizontal
tab characters that follow [.regex]`+{+` or precede [.regex]`+}+` are allowed and are ignored. In the
case of quantifiers, they may also appear before or after the comma. The
exception to this is [.regex]`+\u{...}+` which is an ECMAScript compatibility feature
that is recognized only when the const:PCRE2_EXTRA_ALT_BSUX option is set. ECMAScript
...
[.regex]`+\+`:: general escape character
[.regex]`+^+`:: negate the class, but only if the first character
[.regex]`+-+`:: indicates character range
[.regex]`+[+`:: POSIX character class (if followed by POSIX syntax)
[.regex]`+]+`:: terminates the character class

If a pattern is compiled with the const:PCRE2_EXTENDED option, most white space in
the pattern, other than in a character class, within a [.regex]`+\Q...\E+` sequence, or
between a [.regex]`#` outside a character class and the next newline, inclusive, is
ignored. An escaping backslash can be used to include a white space or a #
character as part of the pattern. If the const:PCRE2_EXTENDED_MORE option is set, the
...
section <<internal-option-setting>>
below.
...
For example, if you want to match a [.subject]`*` character, you must write [.regex]`+\*+` in the
...
In particular, if you want to match a backslash, you write [.regex]`+\\+`.
...
putting them between [.regex]`+\Q+` and [.regex]`+\E+`. Note that this includes white space even when
the const:PCRE2_EXTENDED option is set so that most other white space is ignored. The
behaviour is different from Perl in that $ and @ are handled as literals in
[.regex]`+\Q...\E+` sequences in PCRE2, whereas in Perl, $ and @ cause variable
interpolation. Also, Perl does "double-quotish backslash interpolation" on any
backslashes between [.regex]`+\Q+` and [.regex]`+\E+` which, its documentation says, "may lead to
confusing results". PCRE2 treats a backslash between [.regex]`+\Q+` and [.regex]`+\E+` just like any
other character. Note the following examples:

[cols="1,1,2",options="header"]
|===
|Pattern                       | PCRE2 matches         | Perl matches
|[.regex]`+\Qabc$xyz\E+`       | [.subject]`abc$xyz`   | [.subject]`abc` followed by the contents of `$xyz`
|[.regex]`+\Qabc\$xyz\E+`      | [.subject]`abc\$xyz`  | [.subject]`abc\$xyz`
|[.regex]`+\Qabc\E\$\Qxyz\E+`  | [.subject]`abc$xyz`   | [.subject]`abc$xyz`
|[.regex]`+\QA\B\E+`           | [.subject]`A\B`       | [.subject]`A\B`
|[.regex]`+\Q\\E+`             | [.subject]`\`         | [.subject]`\\E`
|===

The [.regex]`+\Q...\E+` sequence is recognized both inside and outside character classes.
An isolated [.regex]`+\E+` that is not preceded by [.regex]`+\Q+` is ignored. If [.regex]`+\Q+` is not followed
by [.regex]`+\E+` later in the pattern, the literal interpretation continues to the end of
the pattern (that is, [.regex]`+\E+` is assumed at the end). If the isolated [.regex]`+\Q+` is inside
a character class, this causes an error, because the character class is then
not terminated by a closing square bracket.

Another difference from Perl is that any appearance of [.regex]`+\Q+` or [.regex]`+\E+` inside what
...
is inside [.regex]`+\Q...\E+`, but not if the separating comma is. When not recognized as
a quantifier a sequence such as [.regex]`+{\Q1\E,2}+` is treated as the literal string
[.subject]`{1,2}`.
...
[.regex]`\c__x__`             :: "control-__x__", where __x__ is a non-control ASCII character
[.regex]`\e`                  :: escape (char:0x1B)
[.regex]`\f`                  :: form feed (char:0x0C)
[.regex]`\n`                  :: linefeed (char:0x0A)
[.regex]`\r`                  :: carriage return (char:0x0D) (but see below)
[.regex]`\t`                  :: tab (char:0x09)
[.regex]`\0__dd__`            :: character with octal code 0__dd__
[.regex]`{backslash}__ddd__`  :: character with octal code __ddd__, or back reference
[.regex]`\o{__ddd..__}`       :: character with octal code __ddd..__
[.regex]`\x__hh__`            :: character with hex code __hh__
[.regex]`\x{__hhh..__}`       :: character with hex code __hhh..__
[.regex]`\N{U+__hhh..__}`     :: character with Unicode hex code point __hhh..__
...
By default, after [.regex]`+\x+` that is not followed by {, one or two hexadecimal
digits are read (letters can be in upper or lower case). If the character that
follows [.regex]`+\x+` is neither { nor a hexadecimal digit, an error occurs. This is
...
Any number of hexadecimal digits may appear between [.regex]`+\x{+` and [.regex]`+}+`. If a character
other than a hexadecimal digit appears between [.regex]`+\x{+` and [.regex]`+}+`, or if there is no
terminating [.regex]`}`, an error occurs.

Characters whose code points are less than 256 can be defined by either of the
two syntaxes for [.regex]`\x` or by an octal sequence. There is no difference in the way
they are handled. For example, [.regex]`+\xdc+` is exactly the same as [.regex]`+\x{dc}+` or [.regex]`+\334+`.
However, using the braced versions does make such sequences easier to read.

Support is available for some ECMAScript (JavaScript) escape sequences via
two compile-time options. If const:PCRE2_ALT_BSUX is set, the sequence [.regex]`\x` followed
by [.regex]`{` is not recognized. Only if [.regex]`\x` is followed by two hexadecimal digits is it
recognized as a character escape. Otherwise it is interpreted as a literal [.subject]`x`
character. In this mode, support for code points greater than 256 is provided
by [.regex]`\u`, which must be followed by four hexadecimal digits; otherwise it is
interpreted as a literal [.subject]`u` character.

const:PCRE2_EXTRA_ALT_BSUX has the same effect as const:PCRE2_ALT_BSUX and, in addition,
[.regex]`+\u{hhh..}+` is recognized as the character specified by hexadecimal code point.
...
The [.regex]`\N{U+__hhh\...__}` escape sequence is recognized only when PCRE2 is operating in
UTF mode. Perl also uses [.regex]`\N{__name__}` to specify characters by Unicode name; PCRE2
does not support this. Note that when [.regex]`+\N+` is not followed by an opening brace
(curly bracket) it has an entirely different meaning, matching any character
that is not a newline.

There are some legacy applications where the escape sequence [.regex]`+\r+` is expected to
match a newline. If the const:PCRE2_EXTRA_ESCAPED_CR_IS_LF option is set, [.regex]`+\r+` in a
pattern is converted to [.regex]`+\n+` so that it matches a LF (linefeed) instead of a CR
(carriage return) character.

An error occurs if [.regex]`+\c+` is not followed by a character whose ASCII code point
is in the range 32 to 126. The precise effect of [.regex]`\c__x__` is as follows: if _x_ is a
lower case letter, it is converted to upper case. Then bit 6 of the character
(char:0x40) is inverted. Thus [.regex]`+\cA+` to [.regex]`+\cZ+` become char:0x01 to char:0x1A (A is 41, Z is
5A), but [.regex]`+\c{+` becomes char:0x3B ({ is 7B), and [.regex]`+\c;+` becomes char:0x7B (; is 3B). If
the code unit following [.regex]`+\c+` has a code point less than 32 or greater than 126,
a compile-time error occurs.

For differences in the way some escapes behave in EBCDIC environments,
see section <<ebcdic-environments>>
below.

=== Octal escapes and back references

The escape [.regex]`+\o+` must be followed by a sequence of octal digits, enclosed in
...
If braces are not used, after [.regex]`\0` up to two further octal digits are read.
However, if the const:PCRE2_EXTRA_NO_BS0 option is set, at least one more octal digit
must follow [.regex]`\0` (use [.regex]`\00` to generate a NUL character). Make sure you supply
...
stand for themselves. The sequences [.regex]`\8` and [.regex]`\9` are treated as the literal
characters [.subject]`8` and [.subject]`9`.
...
called const:PCRE2_EXTRA_PYTHON_OCTAL that causes PCRE2 to use Python's unambiguous
rules. The next two subsections describe the two sets of rules.

For greater clarity and unambiguity, it is best to avoid following backslash by a
digit greater than zero. Instead, use [.regex]`+\o{...}+` or [.regex]`+\x{...}+`
to specify numerical
character code points, and [.regex]`+\g{...}+` to specify backreferences.
...
[.regex]`\040`  ::  is another way of writing an ASCII space
[.regex]`\40`   ::  is the same, provided there are fewer than 40 previous capture groups
[.regex]`\7`    ::  is always a backreference
[.regex]`\11`   ::  might be a backreference, or another way of writing a tab
[.regex]`\011`  ::  is always a tab
[.regex]`\0113` ::  is a tab followed by the character "3"
[.regex]`\113`  ::  might be a backreference, otherwise the character with octal code 113
[.regex]`\377`  ::  might be a backreference, otherwise the value 255 (decimal)
[.regex]`\81`   ::  is always a backreference
...
[.regex]`\12` is always a back reference, independent of how many captures there are in
...
8-bit non-UTF mode::
    No greater than char:0xff
16-bit non-UTF mode::
    No greater than char:0xffff
32-bit non-UTF mode::
    No greater than char:0xffffffff
All UTF modes::
    No greater than char:0x10ffff and a valid code point

Invalid Unicode code points are all those in the range char:0xd800 to char:0xdfff (the
so-called "surrogate" code points). The check for these can be disabled by the
caller of func:pcre2_compile() by setting the option
const:PCRE2_EXTRA_ALLOW_SURROGATE_ESCAPES. However, this is possible only in UTF-8
and UTF-32 modes, because these values are not representable in UTF-16.

=== Escape sequences in character classes

All the sequences that define a single character value can be used both inside
and outside character classes. In addition, inside a character class, [.regex]`+\b+` is
interpreted as the backspace character (char:0x08).

When not followed by an opening brace, [.regex]`+\N+` is not allowed in a character class.
[.regex]`+\B+`, [.regex]`+\R+`, and [.regex]`+\X+` are not special inside a character class. Like other
unrecognized alphabetic escape sequences, they cause an error. Outside a
character class, these sequences have different meanings.

=== Unsupported escape sequences

In Perl, the sequences [.regex]`\F`, [.regex]`\l`, [.regex]`\L`, [.regex]`\u`, and [.regex]`\U` are recognized by its string
handler and used to modify the case of following characters. By default, PCRE2
does not support these escape sequences in patterns. However, if either of the
const:PCRE2_ALT_BSUX or const:PCRE2_EXTRA_ALT_BSUX options is set, [.regex]`\U` matches a [.subject]`U`
character, and [.regex]`\u` can be used to define a character by code point, as
described above.

=== Absolute and relative backreferences

The sequence [.regex]`+\g+` followed by a signed or unsigned number, optionally enclosed
in braces, is an absolute or relative backreference. A named backreference
can be coded as [.regex]`\g{__name__}`. Backreferences are discussed
later,
following the discussion of
parenthesized groups.
...
For compatibility with Oniguruma, the non-Perl syntax [.regex]`+\g+` followed by a name or
a number enclosed either in angle brackets or single quotes, is an alternative
syntax for referencing a capture group as a subroutine. Details are discussed
later.
Note that [.regex]`+\g{...}+` (Perl syntax) and [.regex]`+\g<...>+` (Oniguruma syntax) are _not_
...
[.regex]`\d`:: Any decimal digit.
[.regex]`\D`:: Any character that is not a decimal digit.
[.regex]`\h`:: Any horizontal white space character.
[.regex]`\H`:: Any character that is not a horizontal white space character.
[.regex]`\N`:: Any character that is not a newline.
[.regex]`\s`:: Any white space character.
[.regex]`\S`:: Any character that is not a white space character.
[.regex]`\v`:: Any vertical white space character.
[.regex]`\V`:: Any character that is not a vertical white space character.
[.regex]`\w`:: Any "word" character.
[.regex]`\W`:: Any "non-word" character.

The [.regex]`+\N+` escape sequence has the same meaning as
the [.regex]`.` metacharacter
when const:PCRE2_DOTALL is not set, but setting const:PCRE2_DOTALL does not change the
meaning of [.regex]`+\N+`. Note that when [.regex]`+\N+` is followed by an opening brace it has a
different meaning. See the section <<non-printing-characters>>
above for details. Perl also uses [.regex]`\N{__name__}` to specify characters by Unicode
...
The default [.regex]`+\s+` characters are HT (9), LF (10), VT (11), FF (12), CR (13), and
...
the "non-breaking space" character (char:0xA0) is recognized as white space, and in
...
accented letters, and these are then matched by [.regex]`+\w+`. The use of locales with
Unicode is discouraged.

By default, characters whose code points are greater than 127 never match [.regex]`+\d+`,
[.regex]`+\s+`, or [.regex]`+\w+`, and always match [.regex]`+\D+`, [.regex]`+\S+`, and [.regex]`+\W+`, although this may be different
for characters in the range 128--255 when locale-specific matching is happening.
These escape sequences retain their original meanings from before Unicode
support was available, mainly for efficiency reasons. If the const:PCRE2_UCP option
is set, the behaviour is changed so that Unicode properties are used to
determine character types, as follows:

[.regex]`\d`:: Any character that matches [.regex]`\p{Nd}` (decimal digit).
[.regex]`\s`:: Any character that matches [.regex]`\p{Z}`, [.regex]`\h`, or [.regex]`\v`.
[.regex]`\w`:: Any character that matches [.regex]`\p{L}`, [.regex]`\p{N}`, [.regex]`\p{Mn}`, or [.regex]`\p{Pc}`.

The addition of [.regex]`+\p{Mn}+` (non-spacing mark) and the replacement of an explicit
test for underscore with a test for [.regex]`+\p{Pc}+` (connector punctuation) happened in
PCRE2 release 10.43. This brings PCRE2 into line with Perl.

The upper case escapes match the inverse sets of characters. Note that [.regex]`\d`
matches only decimal digits, whereas [.regex]`\w` matches any Unicode digit, as well as
other character categories. Note also that const:PCRE2_UCP affects [.regex]`\b`, and
[.regex]`\B` because they are defined in terms of [.regex]`\w` and [.regex]`\W`. Matching these sequences
is noticeably slower when const:PCRE2_UCP is set.

The effect of const:PCRE2_UCP on any one of these escape sequences can be negated by
the options const:PCRE2_EXTRA_ASCII_BSD, const:PCRE2_EXTRA_ASCII_BSS, and
const:PCRE2_EXTRA_ASCII_BSW, respectively. These options can be set and reset within
a pattern by means of an internal option setting
(see below).

The sequences [.regex]`+\h+`, [.regex]`+\H+`, [.regex]`+\v+`, and [.regex]`+\V+`, in contrast to the other sequences, which
match only ASCII characters by default, always match a specific list of code
points, whether or not const:PCRE2_UCP is set. The horizontal space characters are:

char:U+0009:: Horizontal tab (HT)
char:U+0020:: Space
char:U+00A0:: Non-break space
char:U+1680:: Ogham space mark
char:U+180E:: Mongolian vowel separator
char:U+2000:: En quad
char:U+2001:: Em quad
char:U+2002:: En space
char:U+2003:: Em space
char:U+2004:: Three-per-em space
char:U+2005:: Four-per-em space
char:U+2006:: Six-per-em space
char:U+2007:: Figure space
char:U+2008:: Punctuation space
char:U+2009:: Thin space
char:U+200A:: Hair space
char:U+202F:: Narrow no-break space
char:U+205F:: Medium mathematical space
char:U+3000:: Ideographic space

The vertical space characters are:

char:U+000A:: Linefeed (LF)
char:U+000B:: Vertical tab (VT)
char:U+000C:: Form feed (FF)
char:U+000D:: Carriage return (CR)
char:U+0085:: Next line (NEL)
char:U+2028:: Line separator
char:U+2029:: Paragraph separator
...
Outside a character class, by default, the escape sequence [.regex]`+\R+` matches any
Unicode newline sequence. In 8-bit non-UTF-8 mode [.regex]`+\R+` is equivalent to the
following:
...
LF, or one of the single characters LF (linefeed, char:U+000A), VT (vertical tab,
char:U+000B), FF (form feed, char:U+000C), CR (carriage return, char:U+000D), or NEL (next
line, char:U+0085). Because this is an atomic group, the two-character sequence is
treated as a single unit that cannot be split.

In other modes, two additional characters whose code points are greater than 255
are added: LS (line separator, char:U+2028) and PS (paragraph separator, char:U+2029).
...
be requested via the const:PCRE2_BSR_UNICODE option. It is also possible to specify
...
They can also be combined with the [.regex]`+(*UTF)+` or [.regex]`+(*UCP)+` special sequences. Inside a
character class, [.regex]`+\R+` is treated as an unrecognized escape sequence, and causes
an error.
...
less than char:U+0100 or char:U+10000, respectively. In 32-bit non-UTF mode, code points
greater than char:0x10ffff (the Unicode limit) may be encountered. These are all
...
the traditional escape sequences such as [.regex]`+\d+` and [.regex]`+\w+` do not use Unicode
properties in PCRE2 by default, though you can make them do so by setting the
const:PCRE2_UCP option or by starting the pattern with [.regex]`+(*UCP)+`.

The extra escape sequences that provide property support are:

[.regex]`\p{__xx__}`  :: a character with the _xx_ property
[.regex]`\P{__xx__}`  :: a character without the _xx_ property
[.regex]`\X`          :: a Unicode extended grapheme cluster

For compatibility with Perl, negation can be specified by including a
circumflex between the opening brace and the property. For example, [.regex]`+\p{^Lu}+` is
the same as [.regex]`+\P{Lu}+`.
...
case sensitive. Note, however, that the escapes themselves, [.regex]`+\p+` and [.regex]`+\P+`,
...
PCRE2. Note that [.regex]`+\P{Any}+` does not match any characters, so always causes a
match failure.

=== Script properties for [.regex]`+\p+` and [.regex]`+\P+`
...
example, [.regex]`+\p{sc:Adlam}+` matches characters whose basic script is Adlam, whereas
[.regex]`+\p{scx:Adlam}+` matches, in addition, characters that have Adlam in their
...
property type, for example, [.regex]`+\p{Adlam}+`, it is treated as [.regex]`+\p{scx:Adlam}+`. Perl
...
greater than char:0x10FFFF) are assigned the "Unknown" script. Others that are not
...
=== The general category property for [.regex]`+\p+` and [.regex]`+\P+`

Each character has exactly one Unicode general category property, specified by
a two-letter abbreviation. If only one letter is specified with [.regex]`\p` or [.regex]`\P`, it
...
Perl originally used the name [.regex]`L&` for the [.regex]`Lc` property. This is still supported
by Perl, but discouraged. PCRE2 also still supports it. This property matches
any character that has the [.regex]`Lu`, [.regex]`Ll`, or [.regex]`Lt` property, in other words, any letter
that is not classified as a modifier or "other". From release 10.45 of PCRE2
the properties [.regex]`Lu`, [.regex]`Ll`, and [.regex]`Lt` are all treated as [.regex]`Lc` when case-independent
matching is set by the const:PCRE2_CASELESS option or [.regex]`(?i)` within the pattern. The
other properties are not affected by caseless matching.

The [.regex]`Cs` (Surrogate) property applies only to characters whose code points are in
the range char:U+D800 to char:U+DFFF. These characters are no different to any other
...
discussion of const:PCRE2_NO_UTF_CHECK in the
xref:pcre2api.adoc[`pcre2api`]
page).

The long synonyms for property names that Perl supports (such as [.regex]`+\p{Letter}+`)
...
=== Binary (yes/no) properties for [.regex]`\p` and [.regex]`\P`
...
The [.regex]`\X` escape matches any number of Unicode characters that form an "extended
...
[.regex]`\X` always matches at least one character. Then it decides whether to add
...
. Do not break Hangul (a Korean script) syllable sequences. Hangul characters
are of five types: L, V, T, LV, and LVT. An L character may be followed by an
L, V, LV, or LVT character; an LV or V character may be followed by a V or T
character; an LVT or T character may be followed only by a T character.

. Do not end before extending characters or spacing marks or the zero-width
...
. Do not end within emoji modifier sequences or emoji ZWJ (zero-width
...
more that make it possible to convert traditional escape sequences such as [.regex]`\w`
and [.regex]`\s` to use Unicode properties. PCRE2 uses these non-standard, non-Perl
properties internally when const:PCRE2_UCP is set. However, they may also be used
explicitly. These properties are:

[.regex]`Xan`:: Any alphanumeric character
[.regex]`Xps`:: Any POSIX space character
[.regex]`Xsp`:: Any Perl space character
[.regex]`Xwd`:: Any Perl "word" character

[.regex]`Xan` matches characters that have either the [.regex]`L` (letter) or the [.regex]`N` (number)
property. [.regex]`Xps` matches the characters tab, linefeed, vertical tab, form feed, or
carriage return, and any other character that has the [.regex]`Z` (separator) property
(this includes the space character). [.regex]`Xsp` is the same as [.regex]`Xps`; in PCRE1 it used
to exclude vertical tab, for Perl compatibility, but Perl changed. [.regex]`Xwd` matches
the same characters as [.regex]`Xan`, plus those that match [.regex]`Mn` (non-spacing mark) or [.regex]`Pc`
(connector punctuation, which includes underscore).

There is another non-standard property, [.regex]`Xuc`, which matches any character that
can be represented by a Universal Character Name in C{plus}{plus} and other programming
languages. These are the characters $, @, ` (grave accent), and all characters
with Unicode code points greater than or equal to char:U+00A0, except for the
surrogates char:U+D800 to char:U+DFFF. Note that most base (ASCII) characters are
excluded. (Universal Character Names are of the form [.regex]`\u__HHHH__` or [.regex]`\U__HHHHHHHH__`
where H is a hexadecimal digit. Note that the [.regex]`Xuc` property does not match these
sequences but the characters that they represent.)

=== Resetting the match start

In normal use, the escape sequence [.regex]`\K` causes any previously matched characters
...
matches [.subject]`foobar`, but reports that it has matched [.subject]`bar`. [.regex]`\K` does not interact
...
matches only when the subject begins with [.subject]`foobar` (in single line mode),
though it again reports the matched string as [.subject]`bar`. This feature is similar to
a lookbehind assertion
(described below),
but the part of the pattern that precedes [.regex]`+\K+` is not constrained to match a
limited number of characters, as is required for a lookbehind assertion. The
use of [.regex]`+\K+` does not interfere with the setting of
captured substrings.
...
matches [.subject]`foobar`, the first substring is still set to [.subject]`foo`.

From version 5.32.0 Perl forbids the use of [.regex]`+\K+` in lookaround assertions. From
release 10.38 PCRE2 also forbids this by default. However, the
const:PCRE2_EXTRA_ALLOW_LOOKAROUND_BSK option can be used when calling
func:pcre2_compile() to re-enable the previous behaviour. When this option is
set, [.regex]`+\K+` is acted upon when it occurs inside positive assertions, but is
ignored in negative assertions. Note that when a pattern such as [.regex]`+(?=ab\K)+`
matches, the reported start of the match can be greater than the end of the
match. Using [.regex]`+\K+` in a lookbehind assertion at the start of a pattern can also
...
If the subject is [.subject]`foobar`, a call to func:pcre2_match() with a starting
offset of 3 succeeds and reports the matching string as [.subject]`foobar`, that is, the
...
[.regex]`\b`:: Matches at a word boundary.
[.regex]`\B`:: Matches when not at a word boundary.
[.regex]`\A`:: Matches at the start of the subject.
[.regex]`\Z`:: Matches at the end of the subject; also matches before a newline at the end of the subject.
[.regex]`\z`:: Matches only at the end of the subject.
[.regex]`\G`:: Matches at the first matching position in the subject.

Inside a character class, [.regex]`+\b+` has a different meaning; it matches the backspace
...
and the previous character do not both match [.regex]`\w` or [.regex]`\W` (i.e. one matches
[.regex]`\w` and the other matches [.regex]`\W`), or the start or end of the string if the
first or last character matches [.regex]`\w`, respectively. When PCRE2 is built with
Unicode support, the meanings of [.regex]`\w` and [.regex]`\W` can be changed by setting the
const:PCRE2_UCP option. When this is done, it also affects [.regex]`\b` and [.regex]`\B`. Neither PCRE2
nor Perl has a separate "start of word" or "end of word" metasequence. However,
whatever follows [.regex]`\b` normally determines which it is. For example, the fragment
[.regex]`+\ba+` matches [.subject]`a` at the start of a word.

The [.regex]`\A`, [.regex]`\Z`, and [.regex]`\z` assertions differ from the traditional circumflex and
dollar (described in the section <<circumflex-and-dollar>>) in that they only ever match at the very
start and end of the subject string, whatever options are set. Thus, they are
independent of multiline mode. These three assertions are not affected by the
const:PCRE2_NOTBOL or const:PCRE2_NOTEOL options, which affect only the behaviour of the
circumflex and dollar metacharacters. However, if the arg:start_offset
argument of func:pcre2_match() is non-zero, indicating that matching is to
start at a point other than the beginning of the subject, [.regex]`\A` can never match.
The difference between [.regex]`\Z` and [.regex]`\z` is that [.regex]`\Z` matches before a newline at the
end of the string as well as at the very end, whereas [.regex]`\z` matches only at the
end.

The [.regex]`\G` assertion is true only when the current matching position is at the
start point of the matching process, as specified by the arg:start_offset
argument of func:pcre2_match(). It differs from [.regex]`\A` when the value of
arg:start_offset is non-zero. By calling func:pcre2_match() multiple times
with appropriate arguments, you can mimic Perl's /g option, and it is in this
kind of implementation where [.regex]`\G` can be useful.

Note, however, that PCRE2's implementation of [.regex]`\G`, being true at the starting
...
If all the alternatives of a pattern begin with [.regex]`\G`, the expression is anchored
...
the start of the subject string. If the arg:start_offset argument of
func:pcre2_match() is non-zero, or if const:PCRE2_NOTBOL is set, circumflex can
never match if the const:PCRE2_MULTILINE option is unset. Inside a character class,
...
the end of the string (by default), unless const:PCRE2_NOTEOL is set. Note, however,
...
the string, by setting the const:PCRE2_DOLLAR_ENDONLY option at compile time. This
does not affect the [.regex]`\Z` assertion.

The meanings of the circumflex and dollar metacharacters are changed if the
const:PCRE2_MULTILINE option is set. When this is the case, a dollar character
...
const:PCRE2_ALT_CIRCUMFLEX option.

For example, the pattern [.regex]`^abc$` matches the subject string [.subject]`def\nabc` (where
[.regex]`\n` represents a newline) in multiline mode, but not otherwise. Consequently,
patterns that are anchored in single line mode because all branches start with
^ are not anchored in multiline mode, and a match for circumflex is possible
when the arg:start_offset argument of func:pcre2_match() is non-zero. The
const:PCRE2_DOLLAR_ENDONLY option is ignored if const:PCRE2_MULTILINE is set.
...
circumflex matches before [.subject]`xyz` in the string [.subject]`abc\r\nxyz` rather than after
CR, even though CR on its own is a valid newline. (It also matches at the very
start of the string, of course.)

Note that the sequences [.regex]`\A`, [.regex]`\Z`, and [.regex]`\z` can be used to match the start and
end of the subject in both modes, and if all branches of a pattern start with
[.regex]`\A` it is always anchored, whether or not const:PCRE2_MULTILINE is set.

== Full stop (period, dot) and [.regex]`+\n+`
...
const:PCRE2_DOTALL option is set, a dot matches any one character, without exception.
...
The escape sequence [.regex]`+\N+` when not followed by an opening brace behaves like a
dot, except that it is not affected by the const:PCRE2_DOTALL option. In other words,
it matches any character except one that signifies the end of a line.

When [.regex]`+\N+` is followed by an opening brace it has a different meaning. See the
...
Outside a character class, the escape sequence [.regex]`\C` matches any one code unit,
...
32-bit unit. Unlike a dot, [.regex]`\C` always matches line-ending characters. The
feature is provided in Perl in order to match individual bytes in UTF-8 mode,
but it is unclear how it can usefully be used.

Because [.regex]`+\C+` breaks up characters into individual code units, matching one unit
with [.regex]`+\C+` in UTF-8 or UTF-16 mode means that the rest of the string may start
with a malformed UTF character. This has undefined results, because PCRE2
assumes that it is matching character by character in a valid UTF string (by
default it checks the subject string's validity at the start of processing
unless the const:PCRE2_NO_UTF_CHECK or const:PCRE2_MATCH_INVALID_UTF option is used).

An application can lock out the use of [.regex]`+\C+` by setting the
const:PCRE2_NEVER_BACKSLASH_C option when compiling a pattern. It is also possible to
build PCRE2 with the use of [.regex]`+\C+` permanently disabled.

PCRE2 does not allow [.regex]`+\C+` to appear in lookbehind assertions
(described below)
...
func:pcre2_dfa_match() nor the JIT optimizer support [.regex]`\C` in these UTF modes.
The former gives a match-time error; the latter fails to optimize and so the
match is always run using the interpreter.

In the 32-bit library, however, [.regex]`+\C+` is always supported (when not explicitly
locked out) because it always matches a single code unit, whether or not UTF-32
is specified.

In general, the [.regex]`+\C+` escape sequence is best avoided. However, one way of using
...
In this example, a group that starts with [.regex]`+(?|+` resets the capturing parentheses
...
be defined. However, if the const:PCRE2_ALLOW_EMPTY_CLASS option is set, a closing
...
For example, the character class [.regex]`[aeiou]` matches any lower case English vowel,
whereas [.regex]`[^aeiou]` matches all other characters. Note that a circumflex is just a
...
Characters in a class may be specified by their code points using [.regex]`+\o+`, [.regex]`+\x+`, or
[.regex]`\N{U+__hh\...__}` in the usual way. When caseless matching is set, any letters in a
class represent both their upper case and lower case versions, so for example,
a caseless [.regex]`[aeiou]` matches [.subject]`A` as well as [.subject]`a`, and a caseless [.regex]`[^aeiou]` does not
match [.subject]`A`, whereas a caseful version would. Note that there are two ASCII
characters, K and S, that, in addition to their lower case ASCII equivalents,
are case-equivalent with Unicode char:U+212A (Kelvin sign) and char:U+017F (long S)
respectively when either const:PCRE2_UTF or const:PCRE2_UCP is set. If you do not want
these ASCII/non-ASCII case equivalences, you can suppress them by setting
const:PCRE2_EXTRA_CASELESS_RESTRICT, either as an option in a compile context, or by
including [.regex]`+(*CASELESS_RESTRICT)+` or [.regex]`+(?r)+` within a pattern.

Characters that might indicate line breaks are never treated in any special way
when matching character classes, whatever line-ending sequence is in use, and
whatever setting of the const:PCRE2_DOTALL and const:PCRE2_MULTILINE options is used. A
class such as [.regex]`[^a]` always matches one of these characters.

The generic character type escape sequences [.regex]`+\d+`, [.regex]`+\D+`, [.regex]`+\h+`, [.regex]`+\H+`, [.regex]`+\p+`, [.regex]`+\P+`, [.regex]`+\s+`,
[.regex]`+\S+`, [.regex]`+\v+`, [.regex]`+\V+`, [.regex]`+\w+`, and [.regex]`+\W+` may appear in a character class, and add the
characters that they match to the class. For example, [.regex]`[\dABCDEF]` matches any
hexadecimal digit. In UTF modes, the const:PCRE2_UCP option affects the meanings of
[.regex]`+\d+`, [.regex]`+\s+`, [.regex]`+\w+` and their upper case partners, just as it does when they appear
outside a character class, as described in the section
<<generic-character-types>>
above. The escape sequence [.regex]`+\b+` has a different meaning inside a character
class; it matches the backspace character. The sequences [.regex]`+\B+`, [.regex]`+\R+`, and [.regex]`+\X+` are
not special inside a character class. Like any other unrecognized escape
sequences, they cause an error. The same is true for [.regex]`+\N+` when not followed by
an opening brace.

The minus (hyphen) character can be used to specify a range of characters in a
character class. For example, [.regex]`[d-m]` matches any letter between d and m,
inclusive. If a minus character is required in a class, it must be escaped with
a backslash or appear in a position where it cannot be interpreted as
indicating a range, typically as the first or last character in the class,
or immediately after a range. For example, [.regex]`[b-d-z]` matches letters in the range
...
(see below) or before or after a character type escape such as [.regex]`+\d+` or [.regex]`+\H+`.
...
It is not possible to have the literal character [.regex]`+]+` as the end character of a
range. A pattern such as [.regex]`[W-]46]` is interpreted as a class of two characters
(`W` and `-`) followed by a literal string [.regex]`46]`, so it would match [.subject]`W46]` or
[.subject]`-46]`. However, if the [.regex]`+]+` is escaped with a backslash it is interpreted as
the end of a range, so [.regex]`[W-\]46]` is interpreted as a class containing a range
and two other characters. The octal or hexadecimal representation of [.regex]`+]+` can
also be used to end a range.

Ranges normally include all code points between the start and end characters,
inclusive. They can also be used for code points specified numerically, for
example [.regex]`[\000-\037]`. Ranges can include any characters that are valid for the
current mode. In any UTF mode, the so-called "surrogate" characters (those
whose code points lie between char:0xd800 and char:0xdfff inclusive) may not be specified
explicitly by default (the const:PCRE2_EXTRA_ALLOW_SURROGATE_ESCAPES option disables
this check). However, ranges such as [.regex]`[\x{d7ff}-\x{e000}]`, which include the
surrogates, are always permitted.

If a range that includes letters is used when caseless matching is set, it
matches the letters in either case. For example, [.regex]`[W-c]` is equivalent to
[.regex]``[][\\^_`wxyzabc]``, matched caselessly, and in a non-UTF mode, if character
tables for a French locale are in use, [.regex]`[\xc8-\xcb]` matches accented E
characters in both cases.

A circumflex can conveniently be used with the upper case character types to
specify a more restricted set of characters than the matching lower case type.
For example, the class [.regex]`[^\W_]` matches any letter or digit, but not underscore,
whereas [.regex]`[\w]` includes underscore. A positive character class should be read as
...
From release 10.45 PCRE2 supports Perl's [.regex]`+(?[...])+` extended character class
syntax. This can be used to perform set operations such as intersection on
character classes.

The syntax permitted within [.regex]`+(?[...])+` is quite different to ordinary character
classes. Inside the extended class, there is an expression syntax consisting of
"atoms", operators, and ordinary parentheses [.regex]`()` used for grouping. Such
classes always have the Perl [.regex]`/xx` modifier (PCRE2 option const:PCRE2_EXTENDED_MORE)
...
as [.regex]`+\n+` or [.regex]`+\x{123}+`, character types such as [.regex]`+\d+`, POSIX classes such as
[.regex]`[:alpha:]`, and nested ordinary (non-extended) character classes. For example,
in [.regex]`+(?[\d & [...]])+` the nested class [.regex]`+[...]+` follows the usual rules for ordinary
...
syntax does not introduce any additional escape sequences, so [.regex]`(?[\y])` is an
unknown escape, as it would be in [.regex]`[\y]`.

In the extended syntax, [.regex]`^` does not negate a class (except within an
ordinary class nested inside an extended class); it is instead a binary
operator.

The binary operators are [.regex]`&` (intersection), [.regex]`|` or [.regex]`{plus}` (union), [.regex]`+-+`
(subtraction) and [.regex]`+^+` (symmetric difference). These are left-associative and
[.regex]`&` has higher (tighter) precedence, while the others have equal lower precedence.
The one prefix unary operator is [.regex]`!` (complement), with highest
precedence.

== UTS#18 extended character classes

The const:PCRE2_ALT_EXTENDED_CLASS option enables an alternative to Perl's [.regex]`+(?[...])+`
syntax, allowing instead extended class behaviour inside ordinary [.regex]`+[...]+`
character classes. This altered syntax for [.regex]`+[...]+` classes is loosely described
by the Unicode standard UTS#18. The const:PCRE2_ALT_EXTENDED_CLASS option does not
prevent use of [.regex]`+(?[...])+` classes; it just changes the meaning of all
[.regex]`+[...]+` classes that are not nested inside a Perl [.regex]`+(?[...])+` class.

Firstly, in ordinary Perl [.regex]`+[...]+` syntax, an expression such as [.regex]`[a[]` is a
character class with two literal characters [.regex]`a` and [.regex]`[`, but in UTS#18 extended
classes the "[" character becomes an additional metacharacter within classes,
denoting the start of a nested class, so a literal [.regex]`[` must be escaped as [.regex]`\[`.

Secondly, within the UTS#18 extended syntax, there are operators [.regex]`||`, [.regex]`&&`,
[.regex]`--` and [.regex]`~~` which denote character class union, intersection, subtraction,
and symmetric difference respectively. In standard Perl syntax, these would
simply be needlessly-repeated literals (except for [.regex]`--` which could be the
start or end of a range). In UTS#18 extended classes these operators can be used
in constructs such as [.regex]`[\p{L}--[QW]]` for "Unicode letters, other than Q and W".
A literal [.regex]`-` at the start or end of a range must be escaped, so while
[.regex]`[--1]` in Perl syntax is the range from hyphen to 1, it must be
escaped as [.regex]`[\--1]`
in UTS#18 extended classes.

Unlike Perl's [.regex]`+(?[...])+` extended classes, the const:PCRE2_EXTENDED_MORE option to
...
For example, [.regex]`[\p{L}&&[\p{Thai}||\p{Greek}]]` matches any letter that is in
the Thai or Greek scripts. Note that this means that no special grouping
characters (such as the parentheses used in Perl's [.regex]`+(?[...])+` class syntax) are
needed.

Individual class items (literal characters, literal ranges, properties such as
[.regex]`+\d+` or [.regex]`+\p{...}+`, and nested classes) can be combined by juxtaposition or by an
...
as if it is enclosed in square brackets. For example, [.regex]`[A-Z0-9&&[^E8]]` is the
same as [.regex]`[[A-Z0-9]&&[^E8]]`, which matches any upper case alphanumeric character
except "E" or "8".

Precedence between the explicit operators is not defined, so mixing operators
is a syntax error. For example, [.regex]`+[A&&B--C]+` is an error, but
[.regex]`+[A&&[B--C]]+` is valid.

This is an emerging syntax which is being adopted gradually across the regex
ecosystem: for example JavaScript adopted the [.regex]`/v` flag in ECMAScript 2024;
Python's `re` module reserves the syntax for future use with a FutureWarning
for unescaped use of [.regex]`[` as a literal within character classes. Due to UTS#18
...
extended classes, but with slight incompatibilities ([.regex]`[A||B&&C]` is parsed as
[.regex]`[A||[B&&C]]` in Python's `regex` but as [.regex]`[[A||B]&&C]` in Rust's `regex`).

PCRE2's syntax adds syntax restrictions similar to ECMAScript's [.regex]`/v` flag, so
...
enclosed by [.regex]`[:` and [.regex]`:]` within the enclosing square brackets. PCRE2 also supports
...
matches [.subject]`0`, [.subject]`1`, any alphabetic character, or [.subject]`%`. The supported class names
are:

[.regex]`[:alnum:]`:: Letters and digits.
[.regex]`[:alpha:]`:: Letters.
[.regex]`[:ascii:]`:: Character codes 0--127.
[.regex]`[:blank:]`:: Space or tab only.
[.regex]`[:cntrl:]`:: Control characters.
[.regex]`[:digit:]`:: Decimal digits (same as [.regex]`\d`).
[.regex]`[:graph:]`:: Printing characters, excluding space.
[.regex]`[:lower:]`:: Lower case letters.
[.regex]`[:print:]`:: Printing characters, including space.
[.regex]`[:punct:]`:: Printing characters, excluding letters, digits, and space.
[.regex]`[:space:]`:: White space (the same as [.regex]`\s` from PCRE2 8.34).
[.regex]`[:upper:]`:: Upper case letters.
[.regex]`[:word:]`:: "Word" characters (same as [.regex]`\w`).
[.regex]`[:xdigit:]`:: Hexadecimal digits.

The default "space" characters are HT (9), LF (10), VT (11), FF (12), CR (13),
and space (32). If locale-specific matching is taking place, the list of space
characters may be different; there may be fewer or more of them. "Space" and
[.regex]`+\s+` match the same set of characters, as do "word" and [.regex]`+\w+`.
...
matches [.subject]`1`, [.subject]`2`, or any non-digit. PCRE2 (and Perl) also recognize the POSIX
syntax [.regex]`[.ch.]` and [.regex]`[=ch=]` where [.subject]`ch` is a "collating element", but these are not
...
[.regex]`[:alnum:]`:: Becomes [.regex]`\p{Xan}`
[.regex]`[:alpha:]`:: Becomes [.regex]`\p{L}`
[.regex]`[:blank:]`:: Becomes [.regex]`\h`
[.regex]`[:cntrl:]`:: Becomes [.regex]`\p{Cc}`
[.regex]`[:digit:]`:: Becomes [.regex]`\p{Nd}`
[.regex]`[:lower:]`:: Becomes [.regex]`\p{Ll}`
[.regex]`[:space:]`:: Becomes [.regex]`\p{Xps}`
[.regex]`[:upper:]`:: Becomes [.regex]`\p{Lu}`
[.regex]`[:word:]`:: Becomes [.regex]`\p{Xwd}`

Negated versions, such as [.regex]`[:^alpha:]` use [.regex]`+\P+` instead of [.regex]`+\p+`.
...
+
char:U+061C::: Arabic Letter Mark
char:U+180E::: Mongolian Vowel Separator
char:U+2066 -- char:U+2069::: Various "isolates"

[.regex]`[:print:]`::
This matches the same characters as [.regex]`[:graph:]` plus space characters that are
...
versions of those characters, whose Unicode code points start at char:U+FF10. This
is a change that was made in PCRE2 release 10.43 for Perl compatibility.

The other POSIX classes are unchanged by const:PCRE2_UCP, and match only characters
with code points less than 256.

There are two options that can be used to restrict the POSIX classes to ASCII
characters when const:PCRE2_UCP is set. The option const:PCRE2_EXTRA_ASCII_DIGIT affects
just [.regex]`[:digit:]` and [.regex]`[:xdigit:]`. Within a pattern, this can be set and unset by
[.regex]`+(?aT)+` and [.regex]`+(?-aT)+`. The const:PCRE2_EXTRA_ASCII_POSIX option disables UCP processing
for all POSIX classes, including [.regex]`[:digit:]` and [.regex]`[:xdigit:]`. Within a pattern,
[.regex]`+(?aP)+` and [.regex]`+(?-aP)+` set and unset both these options for consistency.
...
[.regex]`[[:<:]]`:: Is converted to [.regex]`\b(?=\w)`.
[.regex]`[[:>:]]`:: Is converted to [.regex]`+\b(?<=\w)+`.

Only these exact character sequences are recognized. A sequence such as
[a[:<:]b] provokes error for an unrecognized POSIX class name. This support is
not compatible with Perl. It is provided to help migrations from other
environments, and is best not used in any new patterns. Note that [.regex]`+\b+` matches
at the start and the end of a word (see
"Simple assertions"
above), and in a Perl-style pattern the preceding or following character
normally shows which is wanted, without the need for the assertions that are
used above in order to give exactly the POSIX behaviour. Note also that the
const:PCRE2_UCP option changes the meaning of [.regex]`\w` (and therefore [.regex]`\b`) by default, so
...
matches either [.subject]`gilbert` or [.subject]`sullivan`. Any number of alternatives may appear,
...
of letters enclosed between [.regex]`(?` and [.regex]`)`. The following are Perl-compatible,
and are described in detail in the
xref:pcre2api.adoc[`pcre2api`]
documentation. The option letters are:

[.regex]`i`:: For const:PCRE2_CASELESS
[.regex]`m`:: For const:PCRE2_MULTILINE
[.regex]`n`:: For const:PCRE2_NO_AUTO_CAPTURE
[.regex]`s`:: For const:PCRE2_DOTALL
[.regex]`x`:: For const:PCRE2_EXTENDED
[.regex]`xx`:: For const:PCRE2_EXTENDED_MORE

For example, [.regex]`+(?im)+` sets caseless, multiline matching. It is also possible to
unset these options by preceding the relevant letters with a hyphen, for
example [.regex]`+(?-im)+`. The two "extended" options are not independent; unsetting
either one cancels the effects of both of them.

A combined setting and unsetting such as [.regex]`+(?im-sx)+`, which sets const:PCRE2_CASELESS
and const:PCRE2_MULTILINE while unsetting const:PCRE2_DOTALL and const:PCRE2_EXTENDED, is also
...
setting [.regex]`(?)` is allowed. Needless to say, it has no effect.

If the first character following [.regex]`(?` is a circumflex, it causes all of the above
...
[.regex]`aD`:: For const:PCRE2_EXTRA_ASCII_BSD
[.regex]`aS`:: For const:PCRE2_EXTRA_ASCII_BSS
[.regex]`aW`:: For const:PCRE2_EXTRA_ASCII_BSW
[.regex]`aP`:: For const:PCRE2_EXTRA_ASCII_POSIX and const:PCRE2_EXTRA_ASCII_DIGIT
[.regex]`aT`:: For const:PCRE2_EXTRA_ASCII_DIGIT
[.regex]`r`:: For const:PCRE2_EXTRA_CASELESS_RESTRICT
[.regex]`J`:: For const:PCRE2_DUPNAMES
[.regex]`U`:: For const:PCRE2_UNGREEDY

However, except for [.regex]`r`, these are not unset by [.regex]`(?^)`, which is equivalent to
[.regex]`+(?-imnrsx)+`. If [.regex]`a` is not followed by any of the upper case letters shown
above, it sets (or unsets) all the ASCII options.

const:PCRE2_EXTRA_ASCII_DIGIT has no additional effect when const:PCRE2_EXTRA_ASCII_POSIX
is set, but including it in [.regex]`+(?aP)+` means that [.regex]`+(?-aP)+` suppresses all ASCII
...
matches [.subject]`abc` and [.subject]`aBc` and no other strings (assuming const:PCRE2_CASELESS is not set
...
matches [.subject]`ab`, [.subject]`aB`, [.subject]`c`, and [.subject]`C`, even though when matching [.subject]`C` the first
branch is abandoned before the option setting. This is because the effects of
option settings happen at compile time. There would be some very weird
behaviour otherwise.

As a convenient shorthand, if any option settings are required at the start of
a non-capturing group (see the section <<groups>>), the option letters may
appear between the [.regex]`?` and the [.regex]`:`. Thus the two patterns
...
the above patterns match [.subject]`SUNDAY` as well as [.subject]`Saturday`.
...
[.regex]`+(*CRLF)+` to override what the application has set or what has been defaulted.
Details are given in the section <<newline-sequences>>
above. There are also the [.regex]`+(*UTF)+` and [.regex]`+(*UCP)+` leading sequences that can be used
to set UTF and Unicode property modes; they are equivalent to setting the
const:PCRE2_UTF and const:PCRE2_UCP options, respectively. However, the application can set
the const:PCRE2_NEVER_UTF or const:PCRE2_NEVER_UCP options, which lock out the use of the
[.regex]`+(*UTF)+` and [.regex]`+(*UCP)+` sequences.
====
...
+
matches [.subject]`cataract`, [.subject]`caterpillar`, or [.subject]`cat`. Without the parentheses, it would
match [.subject]`cataract`, [.subject]`erpillar` or an empty string.

. It creates a "capture group". This means that, when the whole pattern
...
numbers for capture groups. For example, if the string [.subject]`the red king` is
...
+
the captured substrings are [.subject]`red king`, [.subject]`red`, and [.subject]`king`, and are numbered 1,
...
subsequent capture groups. For example, if the string [.subject]`the white queen`
...
the captured substrings are [.subject]`white queen` and [.subject]`queen`, and are numbered 1 and 2. The maximum number of capture groups is 65535.

As a convenient shorthand, if any option settings are required at the start of
a non-capturing group, the option letters may appear between the [.regex]`?` and the
[.regex]`:` (see the section <<groups>>).

[[duplicate-group-numbers]]
== Duplicate group numbers

Perl 5.10 introduced a feature whereby each alternative in a group uses the
same numbers for its capturing parentheses. Such a group starts with [.regex]`+(?|+` and is
...
Because the two alternatives are inside a [.regex]`+(?|+` group, both sets of capturing
parentheses are numbered one. Thus, when the pattern matches, you can look
at captured substring number one, whichever alternative matched. This construct
is useful when you want to capture part, but not all, of one of a number of
alternatives. Inside a [.regex]`+(?|+` group, parentheses are numbered as usual, but the
...
the group. The following pattern matches [.subject]`abcabc` or [.subject]`defdef`:
...
[.subject]`abcabc` or [.subject]`defabc`:
...
A relative reference such as [.regex]`+(?-1)+` is no different: it is just a convenient way
...
duplicate named groups, as described in the section <<named-capture-groups>>.
...
In PCRE2, a capture group can be named in one of three ways: [.regex]`(?<__name__>...)` or
[.regex]`(?'__name__'...)` as in Perl, or [.regex]`(?P<__name__>...)` as in Python. Names may be up to
128 code units long. When const:PCRE2_UTF is not set, they may contain only ASCII
alphanumeric characters and underscores, but must start with a non-digit. When
const:PCRE2_UTF is set, the syntax of group names is extended to allow any Unicode
letter or Unicode decimal digit. In other words, group names must match one of
these patterns:

[.regex]`^[_A-Za-z][_A-Za-z0-9]*\z`  :: when const:PCRE2_UTF is not set
[.regex]`^[_\p{L}][_\p{L}\p{Nd}]*\z` :: when const:PCRE2_UTF is set
...
described in the section <<duplicate-group-numbers>>, a name given to one of them applies to all
...
a successful match, both names yield the same value (either [.subject]`aa` or [.subject]`bb`).
...
still an alias for any group 1. Whether the pattern matches [.subject]`aa` or [.subject]`bb`, a
...
The duplicate name constraint can be disabled by setting the const:PCRE2_DUPNAMES
option at compile time, or by the use of [.regex]`+(?J)+` within the pattern, as described
in the section <<internal-option-setting>>
above.
...
section <<duplicate-group-numbers>>.)
...
reference. For example, this pattern matches both [.subject]`foofoo` and [.subject]`barbar` but not
[.subject]`foobar` or [.subject]`barfoo`:
...
* the [.regex]`\C` escape sequence
* the [.regex]`\R` escape sequence
* the [.regex]`\X` escape sequence
...
matches [.subject]`zz`, [.subject]`zzz`, or [.subject]`zzzz`. A closing brace on its own is not a special
...
[.regex]`X{,4}` is interpreted as [.regex]`X{0,4}`.
...
means that [.regex]`{,}` is a literal string of three characters.

Note that not every opening brace is potentially the start of a quantifier
because braces are used in other items such as [.regex]`\N{U+345}` or [.regex]`\k{_name_}`.

In UTF modes, quantifiers apply to characters rather than to individual code
units. Thus, for example, [.regex]`+\x{100}{2}+` matches two characters, each of
which is represented by a two-byte sequence in a UTF-8 string. Similarly,
[.regex]`+\X{3}+` matches three Unicode extended grapheme clusters, each of which may be
several code units long (and they may be of different lengths).

The quantifier [.regex]`{0}` is permitted, causing the expression to behave as if the
...
below). Except for parenthesized groups, items that have a [.regex]`{0}` quantifier are
omitted from the compiled pattern.

For convenience, the three most common quantifiers have single-character
abbreviations:

[.regex]`+*+`:: Is equivalent to [.regex]`{0,}`.
[.regex]`+`:: Is equivalent to [.regex]`{1,}`.
[.regex]`+?+`:: Is equivalent to [.regex]`{0,1}`.
...
trying to match comments in C programs. These appear between [.c]`+/*+` and [.c]`+*/+` and
...
fails, because it matches the entire string owing to the greediness of the [.regex]`+.*+`
...
If the const:PCRE2_UNGREEDY option is set (an option that is not available in Perl),
...
If a pattern starts with [.regex]`+.*+` or [.regex]`+.{0,}+` and the const:PCRE2_DOTALL option (equivalent
to Perl's [.regex]`/s`) is set, thus allowing the dot to match newlines, the pattern is
implicitly anchored, because whatever follows will be tried against every
character position in the subject string, so there is no point in retrying the
overall match at any position after the first. PCRE2 normally treats such a
pattern as though it were preceded by [.regex]`+\A+`.

In cases where it is known that the subject string contains no newlines, it is
worth setting const:PCRE2_DOTALL in order to obtain this optimization, or
alternatively, using [.regex]`^` to indicate anchoring explicitly.

However, there are some cases where the optimization cannot be used. When [.regex]`+.*+`
...
If the subject is [.subject]`xyz123abc123` the match point is the fourth character. For
this reason, such a pattern is not implicitly anchored.

Another case where implicit anchoring is not applied is when the leading [.regex]`+.*+` is
...
It matches [.subject]`ab` in the subject [.subject]`aab`. The use of the backtracking control verbs
[.regex]`+(*PRUNE)+` and [.regex]`+(*SKIP)+` also disable this optimization. To do so explicitly,
either pass the compile option const:PCRE2_NO_DOTSTAR_ANCHOR, or call
func:pcre2_set_optimize() with a const:PCRE2_DOTSTAR_ANCHOR_OFF directive.
...
has matched [.subject]`tweedledum tweedledee` the value of the captured substring is
[.subject]`tweedledee`. However, if there are nested capture groups, the corresponding
...
matches [.subject]`aba` the value of the second captured substring is [.subject]`b`.
...
Consider, for example, the pattern [.regex]`\d+foo` when applied to the subject line
[.subject]`123456bar`.

After matching all 6 digits and then failing to match [.subject]`foo`, the normal
action of the matcher is to try again with only 5 digits matching the [.regex]`\d{plus}`
item, and then with 4, and so on, before ultimately failing. "Atomic grouping"
(a term taken from Jeffrey Friedl's book) provides the means for specifying
that once a group has matched, it is not to be re-evaluated in this way.

If we use atomic grouping for the previous example, the matcher gives up
immediately on failing to match [.subject]`foo` the first time. The notation is a kind of
special parenthesis, starting with [.regex]`(?>` as in this example:
...
Perl 5.28 introduced an experimental alphabetic form starting with [.regex]`+(*+` which may
...
So, while both [.regex]`\d{plus}` and [.regex]`\d+?` are prepared to adjust the number of digits they
match in order to make the rest of the pattern match, [.regex]`(?>\d+)` can only match
...
additional [.regex]`+` character following a quantifier. Using this notation, the
...
Possessive quantifiers are always greedy; the setting of the const:PCRE2_UNGREEDY
...
pattern constructs. For example, the sequence [.regex]`A{plus}B` is treated as [.regex]`A{plus}{plus}B` because
there is no point in backtracking into a sequence of A's when B must follow.
This feature can be disabled by the const:PCRE2_NO_AUTO_POSSESS option, by calling
func:pcre2_set_optimize() with a const:PCRE2_AUTO_POSSESS_OFF directive, or by
starting the pattern with [.regex]`+(*NO_AUTO_POSSESS)+`.
...
be divided between the internal [.regex]`\D{plus}` repeat and the external [.regex]`+*+` repeat in a
large number of ways, and all have to be tried. (The example uses [.regex]`[!?]` rather
...
number is 8 or more using this syntax because a sequence such as [.regex]`\50` is
interpreted as a character defined in octal. See the subsection
<<non-printing-characters>>
above
...
backslash is to use the [.regex]`+\g+` escape sequence. This escape must be followed by a
signed or unsigned number, optionally enclosed in braces. These examples are
all identical ways of matching [.subject]`ring, ring`:
...
The sequence [.regex]`+\g{-1}+` is a reference to the capture group whose number is one
less than the number of the next group to be started, so in this example (where
the next group would be numbered 3) is it equivalent to [.regex]`\2`, and [.regex]`+\g{-2}+` would
be equivalent to [.regex]`\1`. Note that if this construct is inside a capture group,
that group is included in the count, so in this example [.regex]`+\g{-2}+` also refers to
group 1:
...
The sequence [.regex]`\g{+1}` is a reference to the next capture group that is started
after this item, and [.regex]`\g{+2}` refers to the one after that, and so on. This kind
of forward reference can be useful in patterns that repeat. Perl does not
support the use of [.regex]`{plus}` in this way.
...
matches [.subject]`sense and sensibility` and [.subject]`response and responsibility`, but not
...
matches [.subject]`rah rah` and [.subject]`RAH RAH`, but not [.subject]`RAH rah`, even though the original
capture group is matched caselessly.

There are several different ways of writing backreferences to named capture
groups. The .NET syntax is [.regex]`\k{_name_}`, the Python syntax is [.regex]`(?=__name__)`, and the
original Perl syntax is [.regex]`\k<__name__>` or [.regex]`\k'__name__'`. All of these are now supported
by both Perl and PCRE2. Perl 5.10's unified backreference syntax, in which [.regex]`+\g+`
...
always fails if it starts to match [.subject]`a` rather than [.subject]`bc`. However, if the
const:PCRE2_MATCH_UNSET_BACKREF option is set at compile time, a backreference to an
...
backreference. If the const:PCRE2_EXTENDED or const:PCRE2_EXTENDED_MORE option is set, this
can be white space. Otherwise, the [.regex]`+\g{}+` syntax or an empty comment (see
"Comments"
below) can be used.
...
matches any number of [.subject]`a` characters and also [.subject]`aba`, [.subject]`ababbaa`, etc. At each iteration of
...
succeed for the match to continue. The simple assertions coded as [.regex]`+\b+`, [.regex]`+\B+`,
[.regex]`+\A+`, [.regex]`+\G+`, [.regex]`+\Z+`, [.regex]`+\z+`, [.regex]`^` and [.regex]`$` are described
above.
...
section <<scan-substring-assertions>>
...
useful. PCRE2 has some support for these, described in the section
...
such as [.regex]`+(.)\g{-1}+` can be used to check that two adjacent characters are the
same.
...
example, [.regex]`{3,}` is treated as [.regex]`{3,4}`.

=== Alphabetic assertion names

Traditionally, symbolic sequences such as [.regex]`+(?=+` and [.regex]`+(?<=+` have been used to
...
[.regex]`+(*+` instead of [.regex]`+(?+` and must be written using lower case letters. PCRE2 supports
the following synonyms:

[.regex]`+(*positive_lookahead:+`::
[.regex]`+(*pla:+`::
    Is the same as [.regex]`+(?=+`.
...
For example, [.regex]`(*pla:__foo__)` is the same assertion as [.regex]`(?=__foo__)`. In the following
sections, the various assertions are described using the original symbolic
forms.

=== Lookahead assertions

Lookahead assertions start with [.regex]`+(?=+` for positive assertions and [.regex]`+(?!+` for
...
matches any occurrence of [.subject]`foo` that is not followed by [.subject]`bar`. Note that the
...
does not find an occurrence of [.subject]`bar` that is preceded by something other than
[.subject]`foo`; it finds any occurrence of [.subject]`bar` whatsoever, because the assertion
[.regex]`+(?!foo)+` is always true when the next three characters are [.subject]`bar`. A
lookbehind assertion is needed to achieve the other effect.

If you want to force a matching failure at some point in a pattern, the most
convenient way to do it is with [.regex]`+(?!)+` because an empty string always matches, so
an assertion that requires there not to be an empty string must always fail.
The backtracking control verb [.regex]`+(*FAIL)+` or [.regex]`+(*F)+` is a synonym for [.regex]`+(?!)+`.

=== Lookbehind assertions

Lookbehind assertions start with [.regex]`+(?<=+` for positive assertions and [.regex]`+(?<!+` for
negative assertions. For example,
...
does find an occurrence of [.subject]`bar` that is not preceded by [.subject]`foo`. The contents of
...
function func:pcre2_dfa_match().

In PCRE2 10.43 and later, func:pcre2_match() supports lookbehind assertions in
...
(for example [.regex]`+\d*+`) is not supported. In some cases, the escape sequence [.regex]`+\K+`
(see above)
can be used instead of a lookbehind assertion at the start of a pattern to get
round the length limit restriction.

In UTF-8 and UTF-16 modes, PCRE2 does not allow the [.regex]`+\C+` escape (which matches a
...
[.regex]`+\X+` and [.regex]`+\R+` escapes, which can match different numbers of code units, are never
permitted in lookbehinds.

"Subroutine"
calls (see below) such as [.regex]`(?2)` or [.regex]`(?&X)` are permitted in lookbehinds, as long
...
are met. The const:PCRE2_MATCH_UNSET_BACKREF option must not be set, there must be no
use of [.regex]`+(?|+` in the pattern (it creates duplicate group numbers), and if the
...
from left to right, PCRE2 will look for each [.subject]`a` in the subject and then see if
...
the initial [.regex]`+.*+` matches the entire string at first, but when this fails (because
there is no following [.subject]`a`), it backtracks to match all but the last character,
then all but the last two characters, and so on. Once again the search for [.subject]`a`
...
there can be no backtracking for the [.regex]`.*{plus}` item because of the possessive
...
matches [.subject]`foo` preceded by three digits that are not [.subject]`999`. Notice that each of
the assertions is applied independently at the same point in the subject
string. First there is a check that the previous three characters are all
digits, and then there is a check that the same three characters are not [.subject]`999`.
This pattern does _not_ match [.subject]`foo` preceded by six characters, the first
of which are digits and the last three of which are not [.subject]`999`. For example, it
doesn't match [.subject]`123abcfoo`. A pattern to do that is
...
preceding three characters are not [.subject]`999`.
...
matches an occurrence of [.subject]`baz` that is preceded by [.subject]`bar` which in turn is not
preceded by [.subject]`foo`, while
...
is another pattern that matches [.subject]`foo` preceded by three digits and any three
characters that are not [.subject]`999`.
...
[.regex]`+(*non_atomic_positive_lookahead:+`::
[.regex]`+(*napla:+`::
[.regex]`+(?*+`::
    Non-atomic positive lookahead.
[.regex]`+(*non_atomic_positive_lookbehind:+`::
[.regex]`+(*naplb:+`::
[.regex]`+(?<*+`::
...
For a subject such as [.subject]`word1 word2 word3 word2 word3 word4` the result is
[.subject]`word3`. How does it work? At the start, [.regex]`^(?x)` anchors the pattern and sets the
[.regex]`x` option, which causes white space (introduced for readability) to be
ignored. Inside the assertion, the greedy [.regex]`+.*+` at first consumes the entire
...
using an ungreedy [.regex]`+.*?+` to scan from the left. If this succeeds, we are done, but
if the last word in the string does not occur twice, this part of the pattern
fails. If a traditional atomic lookahead [.regex]`+(?=+` or [.regex]`+(*pla:+` had been used, the
...
[.regex]`+(*ACCEPT)+` control verb is triggered, the assertion succeeds atomically. That
...
func:pcre2_dfa_match(). They are supported by JIT, but only if they do not
contain any control verbs such as [.regex]`+(*ACCEPT)+`. (This may change in future). Note
...
A scan substring assertion starts with the sequence [.regex]`+(*scan_substring:+` or
[.regex]`+(*scs:+` which is followed by a list of substring numbers (absolute or relative)
...
const:PCRE2_DUPNAMES is set and there are ambiguous group names, all groups with the
...
substring, so [.regex]`+\Z+`, [.regex]`+\z+`, and [.regex]`+$+` will match there. However, the start of the
subject is _not_ reset. This means that [.regex]`+^+` matches only if the substring is
...
sequence of letters [.subject]`rh` not at the start:
...
the rules that PCRE2 uses in the section
xref:pcre2unicode.adoc#script-runs[Script runs]
in the
xref:pcre2unicode.adoc[`pcre2unicode`]
documentation.

If part of a pattern is enclosed between [.regex]`+(*script_run:+` or [.regex]`+(*sr:+` and a closing
...
from different scripts. The string [.subject]`paypal.com` is an infamous example, where
...
[.regex]`+(*atomic_script_run:+` or [.regex]`+(*asr:+`

[.regex]`+(*asr:...)+`::
    Is the same as [.regex]`+(*sr:(?>...))+`.
...
func:pcre2_dfa_match() because they use the same mechanism as capturing
parentheses.

Warning: The [.regex]`+(*ACCEPT)+` control verb
(see below)
...
[source,regex,subs=normal]
----
(?(__condition__)__yes-pattern__)
(?(__condition__)__yes-pattern__|__no-pattern__)
----
...
this condition) can be referenced by [.regex]`(?(-1)`, the next most recent by [.regex]`(?(-2)`,
and so on. Inside loops it can also make sense to refer to subsequent groups.
The next capture group to be opened can be referenced as [.regex]`(?(+1)`, and so on. The
value zero in any of these forms is not used; it provokes a compile-time error.

Consider the following pattern, which contains non-significant white space to
make it more readable (assume the const:PCRE2_EXTENDED option) and to divide it into
...
Perl uses the syntax [.regex]`(?(<__name__>)\...)` or [.regex]`(?('__name__')\...)` to test for a used
capture group by name. For compatibility with earlier versions of PCRE1, which
had this facility before Perl, the syntax [.regex]`(?(__name__)\...)` is also recognized.
Note, however, that undelimited names consisting of the letter R followed by
digits are ambiguous (see the section <<checking-pattern-recursion>>). Rewriting the above example
...
sections <<recursive-patterns>>
and
<<groups-as-subroutines>>
below for details of recursion and subroutine calls.

If a condition is the string [.regex]`+(R)+`, and there is no capture group with the name
...
overall pattern. This is a contrived example that is equivalent to [.regex]`a+b`:
...
adding [.regex]`+(?<R1>)+` to the above pattern completely changes its meaning.
...
If the condition is the string [.regex]`(DEFINE)`, the condition is always false, even if
...
[.subject]`192.168.23.245` could be written like this (ignore white space and line
breaks):
...
func:pcre2_config() with appropriate arguments. Users of applications that do
...
[.subject]`yesno`. VERSION must be followed either by [.regex]`=` or [.regex]`>=` and a version number.
For example:
...
This pattern matches [.subject]`yes` if the PCRE2 version is greater or equal to 10.4, or
[.subject]`no` otherwise. The fractional part of the version number could be ommited.
...
[.subject]`_dd_-_aaa_-_dd_` or [.subject]`_dd_-_dd_-_dd_`, where _aaa_ are letters and _dd_ are digits.
...
[.regex]`(?:` or a group name or number or a Unicode property name. The characters that
make up a comment play no part in the pattern matching.

The sequence [.regex]`(?#` marks the start of a comment that continues up to the next
closing parenthesis. Nested parentheses are not permitted. If the
const:PCRE2_EXTENDED or const:PCRE2_EXTENDED_MORE option is set, an unescaped # character
...
described in the section <<newline-conventions>>
above. Note that the end of this type of comment is a literal newline sequence
in the pattern; escape sequences that happen to represent a newline do not
count. For example, consider this pattern when const:PCRE2_EXTENDED is set, and the
...
On encountering the # character, func:pcre2_compile() skips along, looking for
a newline in the pattern. The sequence [.regex]`+\n+` is still literal at this stage, so
it does not terminate the comment. Only an actual character with the code value
char:0x0a (the default newline) does so.
...
The [.regex]`+(?p{...})+` item interpolates Perl code at run time, and in this case refers
...
A special item that consists of [.regex]`(?` followed by a number greater than zero and a
...
call, which is described in the section <<groups-as-subroutines>>.) The special item [.regex]`+(?R)+` or [.regex]`+(?0)+` is
a recursive call of the entire regular expression.

This PCRE2 pattern solves the nested parentheses problem (assume the
const:PCRE2_EXTENDED option is set so that white space is ignored):
...
is made easier by the use of relative references. Instead of [.regex]`(?1)` in the
pattern above you can write [.regex]`+(?-2)+` to refer to the second most recently opened
...
The first two capture groups [.regex]`(a)` and [.regex]`(b)` are both numbered 1, and group [.regex]`(c)`
is number 2. When the reference [.regex]`+(?-2)+` is encountered, the second most recently
opened parentheses has the number 1, but it is the first such group (the [.regex]`(a)`
group) to which the recursion refers. This would be the same if an absolute
reference [.regex]`(?1)` was used. In other words, relative references are just a
shorthand for computing a group number.

It is also possible to refer to subsequent capture groups, by writing
references such as [.regex]`(?+2)`. However, these cannot be recursive because the
reference is not inside the parentheses that are referenced. They are always
non-recursive subroutine
calls, as described in the section <<groups-as-subroutines>>.

An alternative approach is to use named parentheses. The Perl syntax for this
is [.regex]`(?&__name__)`; PCRE1's earlier syntax [.regex]`(?P>__name__)` is also supported. We could
...
ways the [.regex]`{plus}` and [.regex]`+*+` repeats can carve up the subject, and all have to be tested
...
the value for the inner capturing parentheses (numbered 2) is [.subject]`ef`, which is
...
Do not confuse the [.regex]`+(?R)+` item with the condition [.regex]`+(R)+`, which tests for recursion.
...
In this pattern, [.regex]`+(?(R)+` is the start of a conditional group, with two different
alternatives for the recursive and non-recursive cases. The [.regex]`+(?R)+` item is the
...
If run with the const:PCRE2_CASELESS option, this pattern matches phrases such as [.subject]`A
man, a plan, a canal: Panama!`. Note the use of the possessive quantifier [.regex]`*{plus}` to
...
group was called recursively or as a subroutine (see the section
<<groups-as-subroutines>>), it
...
This pattern matches [.subject]`bab`. The first capturing parentheses match [.subject]`b`, then in
the second group, when the backreference [.regex]`\1` fails to match [.subject]`b`, the second
alternative matches [.subject]`a` and then recurses. In the recursion, [.regex]`\1` does now match
[.subject]`b` and so the whole match succeeds. This match used to fail in Perl, but in
...
matches [.subject]`sense and sensibility` and [.subject]`response and responsibility`, but not
[.subject]`sense and responsibility`. If instead the pattern
...
is used, it does match [.subject]`sense and responsibility` as well as the other two
...
It matches [.subject]`abcabc`. It does not match [.subject]`abcABC` because the change of
processing option does not affect the called group.

The behaviour of
backtracking control verbs
in groups when called as subroutines is described in the section
...
[.regex]`(?+_n_(_grouplist_))`::
    Recurse the whole pattern, returning capture groups.
[.regex]`(?-_n_(_grouplist_))`::
[.regex]`(?&__name__(_grouplist_))`::
[.regex]`(?P>__name__(_grouplist_))`::
    Call a subroutine, returning capture groups.

The list of capture groups _grouplist_ is a comma-separated list of (absolute
...
This feature is not available using the Oniguruma syntax [.regex]`+\g<...>+` or [.regex]`+\g'...'+`
below.
...
Perl has a feature whereby using the sequence [.regex]`+(?{...})+` causes arbitrary Perl
...
func:pcre2_set_callout(), and then passing that context to func:pcre2_match()
or func:pcre2_dfa_match(). If no match context is passed, or if the callout
...
const:PCRE2_EXTRA_NEVER_CALLOUT option.

Within a regular expression, [.regex]`(?C__arg__)` indicates a point at which the external
function is to be called. There are two kinds of callout: those with a
numerical argument and those with a string argument. [.regex]`+(?C)+` on its own with no
argument is treated as [.regex]`+(?C0)+`. A numerical argument allows the application to
...
If the const:PCRE2_AUTO_CALLOUT flag is passed to func:pcre2_compile(), numerical
...
starting delimiter must be one of [.regex]`{backtick}`, [.regex]`'`, [.regex]`"`, [.regex]`^`, [.regex]`%`, [.regex]`#`, [.regex]`$` or [.regex]`{` and the ending delimiter is
the same as the start, except for [.regex]`{`, where the ending delimiter is [.regex]`}`. If the
...
are generally of the form [.regex]`+(*VERB)+` or [.regex]`(*VERB:__NAME__)`. Some verbs take either form,
...
This can be changed by setting the const:PCRE2_ALT_VERBNAMES option, but the result
is no longer Perl-compatible.

When const:PCRE2_ALT_VERBNAMES is set, backslash processing is applied to verb names
and only an unescaped closing parenthesis terminates the name. However, the
only backslash items that are permitted are [.regex]`+\Q+`, [.regex]`+\E+`, and sequences such as
[.regex]`+\x{100}+` that define character code points. Character type escapes such as [.regex]`\d`
are faulted.

A closing parenthesis can be included in a name either as [.regex]`+\)+` or between [.regex]`+\Q+`
and [.regex]`+\E+`. In addition to backslash processing, if the const:PCRE2_EXTENDED or
const:PCRE2_EXTENDED_MORE option is also set, unescaped white space in verb names is
skipped, and #-comments are recognized, exactly as in the rest of the pattern.
const:PCRE2_EXTENDED and const:PCRE2_EXTENDED_MORE do not affect verb names unless
const:PCRE2_ALT_VERBNAMES is also set.

The maximum length of a name is 255 in the 8-bit library and 65535 in the
16-bit and 32-bit libraries. If the name is empty, that is, if the closing
parenthesis immediately follows the colon, the effect is as if the colon were
not there. Any number of these verbs may occur in a pattern. Except for
[.regex]`+(*ACCEPT)+`, they may not be quantified.
...
of [.regex]`+(*FAIL)+`, which behaves like a failing negative assertion, the backtracking
...
the start-of-match optimizations by setting the const:PCRE2_NO_START_OPTIMIZE option
when calling func:pcre2_compile(), by calling func:pcre2_set_optimize() with a
const:PCRE2_START_OPTIMIZE_OFF directive, or by starting the pattern with
[.regex]`+(*NO_START_OPT)+`. There is more discussion of this option in the section
entitled
"Compiling a pattern"
in the
xref:pcre2api.adoc[`pcre2api`]
...
[.regex]`(*ACCEPT)` or [.regex]`(*ACCEPT:__NAME__)`::
...
at the outer level. If [.regex]`+(*ACCEPT)+` in triggered in a positive assertion, the
assertion succeeds; in a negative assertion, the assertion fails.
+
If [.regex]`+(*ACCEPT)+` is inside capturing parentheses, the data so far is captured. For
example:
+
[source,regex]
----
...
----
+
This matches [.subject]`AB`, [.subject]`AAD`, or [.subject]`ACD`; when it matches [.subject]`AB`, [.subject]`B` is captured by
the outer parentheses.
+
[.regex]`+(*ACCEPT)+` is the only backtracking verb that is allowed to be quantified
...
----
+
where A, B, and C may be complex expressions. After matching [.subject]`A`, the matcher
processes [.subject]`BC`; if that fails, causing a backtrack, [.regex]`+(*ACCEPT)+` is triggered and
the match succeeds. In both cases, all but C is captured. Whereas [.regex]`+(*COMMIT)+`
(see below) means "fail on backtrack", a repeated [.regex]`+(*ACCEPT)+` of this type means
"succeed on backtrack".
+
Warning: [.regex]`+(*ACCEPT)+` should not be used within a script run group, because
it causes an immediate exit from the group, bypassing the script run checking.

[.regex]`(*FAIL)` or [.regex]`(*FAIL:__NAME__)`::

This verb causes a matching failure, forcing backtracking to occur. It may be
abbreviated to [.regex]`+(*F)+`. It is equivalent to [.regex]`+(?!)+` but easier to read. The Perl
documentation notes that it is probably useful only when combined with [.regex]`(?{})` or
[.regex]`(??{})`. Those are, of course, Perl features that are not present in PCRE2. The
...
----
+
A match with the string [.subject]`aaaa` always fails, but the callout is taken before
each backtrack happens (in this example, 10 times).

[.regex]`(*ACCEPT:__NAME__)` and [.regex]`(*FAIL:__NAME__)` behave the same as [.regex]`(*MARK:__NAME__)(*ACCEPT)` and
[.regex]`(*MARK:__NAME__)(*FAIL)`, respectively, that is, a [.regex]`+(*MARK)+` is recorded just before
the verb acts.
...
starting point (see [.regex]`+(*SKIP)+` below).

[.regex]`(*MARK:__NAME__)` or [.regex]`({asterisk}:__NAME__)`::

A name is always required with this verb. For all the other backtracking
control verbs, a _NAME_ argument is optional.
+
...
xref:pcre2api.adoc#other-match-information[Other information about a match]
in
xref:pcre2api.adoc[`pcre2api`]
documentation. This applies to all instances of [.regex]`+(*MARK)+` and other verbs,
including those inside assertions and atomic groups. However, there are
differences in those cases when [.regex]`+(*MARK)+` is used in conjunction with [.regex]`+(*SKIP)+` as
described below.
+
The mark name that was last encountered on the matching path is passed back. A
verb without a _NAME_ argument is ignored for this purpose. Here is an example of
`pcre2test` output, where the "mark" modifier requests the retrieval and
outputting of [.regex]`+(*MARK)+` data:
+
[source,text]
----
...
----
+
The [.regex]`+(*MARK)+` name is tagged with "MK:" in this output, and in this example it
...
[.regex]`+(*MARK)+` item, but nevertheless do not reset it.
+
If you are interested in [.regex]`+(*MARK)+` values after failed matches, you should
probably either set the const:PCRE2_NO_START_OPTIMIZE option or call
func:pcre2_set_optimize() with a const:PCRE2_START_OPTIMIZE_OFF directive
(see above)
...
[.regex]`(*COMMIT)` or [.regex]`(*COMMIT:__NAME__)`::
...
take place. If [.regex]`+(*COMMIT)+` is the only backtracking verb that is encountered,
once it has been passed func:pcre2_match() is committed to finding a match at
...
----
+
This matches [.subject]`xxaab` but not [.subject]`aacaab`. It can be thought of as a kind of
dynamic anchor, or "I've started, so I must finish."
+
The behaviour of [.regex]`(*COMMIT:__NAME__)` is not the same as [.regex]`(*MARK:__NAME__)(*COMMIT)`. It is
like [.regex]`(*MARK:__NAME__)` in that the name is remembered for passing back to the
caller. However, [.regex]`(*SKIP:__NAME__)` searches only for names that are set with
[.regex]`(*MARK)`, ignoring those set by any of the other backtracking verbs.
+
If there is more than one backtracking verb in a pattern, a different one that
follows [.regex]`+(*COMMIT)+` may be triggered first, so merely passing [.regex]`+(*COMMIT)+` during a
match does not always guarantee that a match must be at this starting point.
+
Note that [.regex]`+(*COMMIT)+` at the start of a pattern is not the same as an anchor,
...
----
+
For the first pattern, PCRE2 knows that any match must start with [.subject]`a`, so the
optimization skips along the subject to [.subject]`a` before applying the pattern to the
...
applied starting at [.subject]`x`, and so the [.regex]`+(*COMMIT)+` causes the match to fail without
trying any other starting points.

[.regex]`+(*PRUNE)+` or [.regex]`(*PRUNE:__NAME__)`::
...
[.regex]`+(*PRUNE)+`, before it is reached, or when matching to the right of [.regex]`+(*PRUNE)+`, but
if there is no match to the right, backtracking cannot cross [.regex]`+(*PRUNE)+`. In
simple cases, the use of [.regex]`+(*PRUNE)+` is just an alternative to an atomic group or
possessive quantifier, but there are some uses of [.regex]`+(*PRUNE)+` that cannot be
expressed in any other way. In an anchored pattern [.regex]`+(*PRUNE)+` has the same effect
as [.regex]`+(*COMMIT)+`.
+
The behaviour of [.regex]`(*PRUNE:__NAME__)` is not the same as [.regex]`(*MARK:__NAME__)(*PRUNE)`. It is
like [.regex]`(*MARK:__NAME__)` in that the name is remembered for passing back to the
caller. However, [.regex]`(*SKIP:__NAME__)` searches only for names set with [.regex]`(*MARK)`,
ignoring those set by other backtracking verbs.

[.regex]`(*SKIP)`::

This verb, when given without a name, is like [.regex]`+(*PRUNE)+`, except that if the
pattern is unanchored, the "bumpalong" advance is not to the next character,
but to the position in the subject where [.regex]`+(*SKIP)+` was encountered. [.regex]`+(*SKIP)+`
...
----
+
If the subject is [.subject]`aaaac...`, after the first match attempt fails (starting at
the first character in the string), the starting point skips on to start the
next attempt at [.subject]`c`. Note that a possessive quantifier does not have the same
effect as this example; although it would suppress backtracking during the
first match attempt, the second attempt would start at the second character
instead of skipping on to "c".
+
If [.regex]`+(*SKIP)+` is used to specify a new starting position that is the same as the
starting position of the current match, or (by being inside a lookbehind)
earlier, the position specified by [.regex]`+(*SKIP)+` is ignored, and instead the normal
"bumpalong" occurs.

[.regex]`(*SKIP:__NAME__)`::

When [.regex]`+(*SKIP)+` has an associated name, its behaviour is modified. When such a
[.regex]`+(*SKIP)+` is triggered, the previous path through the pattern is searched for the
most recent [.regex]`+(*MARK)+` that has the same name. If one is found, the "bumpalong"
advance is to the subject position that corresponds to that [.regex]`+(*MARK)+` instead of
to where [.regex]`+(*SKIP)+` was encountered. If no [.regex]`+(*MARK)+` with a matching name is found,
the [.regex]`+(*SKIP)+` is ignored.
+
The search for a [.regex]`+(*MARK)+` name uses the normal backtracking mechanism, which
means that it does not see [.regex]`+(*MARK)+` settings that are inside atomic groups or
...
----
+
In the first example, the [.regex]`+(*MARK)+` setting is in an atomic group, so it is not
seen when [.regex]`+(*SKIP:X)+` triggers, causing the [.regex]`+(*SKIP)+` to be ignored. This allows
the second branch of the pattern to be tried at the first character position.
In the second example, the [.regex]`+(*MARK)+` setting is not in an atomic group. This
allows [.regex]`+(*SKIP:X)+` to find the [.regex]`+(*MARK)+` when it backtracks, and this causes a new
matching attempt to start at the second character. This time, the [.regex]`+(*MARK)+` is
never seen because [.subject]`a` does not match [.subject]`b`, so the matcher immediately jumps to
the second branch of the pattern.
+
Note that [.regex]`(*SKIP:__NAME__)` searches only for names set by [.regex]`(*MARK:__NAME__)`. It ignores
names that are set by other backtracking verbs.

[.regex]`(*THEN)` or [.regex]`(*THEN:__NAME__)`::
...
group. If [.regex]`+(*THEN)+` is not inside an alternation, it acts like [.regex]`+(*PRUNE)+`.
+
The behaviour of [.regex]`(*THEN:__NAME__)` is not the same as [.regex]`(*MARK:__NAME__)(*THEN)`. It is
like [.regex]`(*MARK:__NAME__)` in that the name is remembered for passing back to the
caller. However, [.regex]`(*SKIP:__NAME__)` searches only for names set with [.regex]`+(*MARK)+`,
ignoring those set by other backtracking verbs.
+
...
effect of [.regex]`+(*THEN)+` extends beyond such a group to the enclosing alternative.
...
However, if the group containing [.regex]`+(*THEN)+` is given an alternative, it
behaves differently:
+
[source,regex]
----
...
----
+
The effect of [.regex]`+(*THEN)+` is now confined to the inner group. After a failure in C,
matching moves to [.regex]`+(*FAIL)+`, which causes the whole group to fail because there
...
----
+
If the subject is [.subject]`ba`, this pattern does not match. Because [.regex]`+.*?+` is ungreedy,
it initially matches zero characters. The condition [.regex]`+(?=a)+` then fails, the
character [.subject]`b` is matched, but [.subject]`c` is not. At this point, matching does not
backtrack to [.regex]`+.*?+` as might perhaps be expected from the presence of the |
character. The conditional group is part of the single alternative that
comprises the whole pattern, and so the match fails. (If there was a backtrack
into [.regex]`+.*?+`, allowing it to match [.subject]`b`, the match would succeed.)

The verbs just described provide four different "strengths" of control when
subsequent matching fails. [.regex]`+(*THEN)+` is the weakest, carrying on the match at the
next alternative. [.regex]`+(*PRUNE)+` comes next, failing the match at the current
starting position, but allowing an advance to the next character (for an
unanchored pattern). [.regex]`+(*SKIP)+` is similar, except that the advance may be more
than one character. [.regex]`+(*COMMIT)+` is the strongest, causing the entire match to
fail.
...
If A matches but B fails, the backtrack to [.regex]`+(*COMMIT)+` causes the entire match to
fail. However, if A and B match, but C fails, the backtrack to [.regex]`+(*THEN)+` causes
...
If there is a matching failure to the right, backtracking onto [.regex]`+(*PRUNE)+` causes
...
If the subject is [.subject]`abac`, Perl matches unless its optimizations are disabled,
but PCRE2 always fails because the [.regex]`+(*COMMIT)+` in the second repeat of the group
acts.

=== Backtracking verbs in assertions

[.regex]`+(*FAIL)+` in any assertion has its normal effect: it forces an immediate
...
[.regex]`+(*ACCEPT)+` in a standalone positive assertion causes the assertion to succeed
without any further processing; captured strings and a mark name (if set) are
retained. In a standalone negative assertion, [.regex]`+(*ACCEPT)+` causes the assertion to
fail without any further processing; captured substrings and any mark name are
discarded.

If the assertion is a condition, [.regex]`+(*ACCEPT)+` causes the condition to be true for
...
into the assertion. Note in particular that a [.regex]`+(*MARK)+` name that is set in an
assertion is not "seen" by an instance of [.regex]`(*SKIP:__NAME__)` later in the pattern.

PCRE2 now supports non-atomic positive assertions and also "scan substring"
assertions, as described in the sections <<non-atomic-assertions>>
and
<<scan-substring-assertions>>
...
into the assertion, and therefore verbs such as [.regex]`+(*COMMIT)+` can be triggered by
backtracks from later in the pattern.

The effect of [.regex]`+(*THEN)+` is not allowed to escape beyond an assertion. If there
are no more branches to try, [.regex]`+(*THEN)+` causes a positive assertion to be false,
...
backtracking (from within the assertion) into [.regex]`+(*COMMIT)+`, [.regex]`+(*SKIP)+`, or [.regex]`+(*PRUNE)+`
causes the condition to be false. However, for both standalone and conditional
negative assertions, backtracking into [.regex]`+(*COMMIT)+`, [.regex]`+(*SKIP)+`, or [.regex]`+(*PRUNE)+` causes
...
[.regex]`+(*ACCEPT)+` in a group called as a subroutine causes the subroutine match to
...
[.regex]`+(*FAIL)+` in a group called as a subroutine has its normal effect: it forces
an immediate backtrack.

[.regex]`+(*COMMIT)+`, [.regex]`+(*SKIP)+`, and [.regex]`+(*PRUNE)+` cause the subroutine match to fail when
triggered by being backtracked to in a group called as a subroutine. There is
then a backtrack at the outer level.

[.regex]`+(*THEN)+`, when triggered, skips to the next alternative in the innermost
...
When PCRE2 is compiled in EBCDIC mode, [.regex]`\N{U+__hhh\...__}` is not supported. [.regex]`+\a+`, [.regex]`+\e+`,
[.regex]`+\f+`, [.regex]`+\n+`, [.regex]`+\r+`, and [.regex]`+\t+` generate the appropriate EBCDIC code values. The [.regex]`+\c+`
escape is processed as specified for Perl in the `perlebcdic` document. The
only characters that are allowed after [.regex]`+\c+` are A--Z, a--z, or one of @, [, \, ],
^, _, or ?. Any other character provokes a compile-time error. The sequence
[.regex]`+\c@+` encodes character code 0; after [.regex]`+\c+` the letters (in either case) encode
characters 1--26 (char:0x01 to char:0x1A); [, \, ], ^, and _ encode characters 27--31
(char:0x1B to char:0x1F), and [.regex]`+\c?+` becomes either 255 (char:0xFF) or 95 (char:0x5F).

Thus, apart from [.regex]`+\c?+`, these escapes generate the same character code values as
they do in an ASCII or Unicode environment, though the meanings of the values
mostly differ. For example, [.regex]`+\cG+` always generates code value 7, which is BEL in
ASCII but DEL in EBCDIC.

The sequence [.regex]`+\c?+` generates DEL (127, char:0x7F) in an ASCII environment, but
because 127 is not a control character in EBCDIC, Perl makes it generate the
APC character. Unfortunately, there are several variants of EBCDIC. In most of
them the APC character has the value 255 (char:0xFF), but in the one Perl calls
POSIX-BC its value is 95 (char:0x5F). If certain other characters have POSIX-BC
values, PCRE2 makes [.regex]`+\c?+` generate 95; otherwise it generates 255.
...
letters are omitted. For example, [.regex]`[h-k]` matches only four characters, even
though the EBCDIC codes for h and k are char:0x88 and char:0x92, a range of 11 code
points. However, if the range is specified numerically, for example,
[.regex]`[\x88-\x92]` or [.regex]`[h-\x92]`, all code points are included.

== See also

xref:pcre2api.adoc[`pcre2api`], xref:pcre2callout.adoc[`pcre2callout`], xref:pcre2matching.adoc[`pcre2matching`],
xref:pcre2syntax.adoc[`pcre2syntax`], xref:pcre2.adoc[`pcre2`].
