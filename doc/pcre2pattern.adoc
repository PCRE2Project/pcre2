...
description of [.regex]`\R` in the section entitled
"Newline sequences"
...
caseless matching is specified (the const:PCRE2_CASELESS option or (?i) within the
...
func:pcre2_compile() or set by [.regex]`(*CASELESS_RESTRICT)` or (?r) within the
...
as [.regex]`\g{2}` or [.regex]`\k{name}`. In almost all uses of braces, space and/or horizontal
...
For example, if you want to match a [.regex]`*` character, you must write [.regex]`\*` in the
...
by { is not recognized. Only if [.regex]`\x` is followed by two hexadecimal digits is it
...
The [.regex]`\N{U+hhh..}` escape sequence is recognized only when PCRE2 is operating in
...
match a newline. If the const:PCRE2_EXTRA_ESCAPED_CR_IS_LF option is set, \r in a
...
const:PCRE2_ALT_BSUX or const:PCRE2_EXTRA_ALT_BSUX options is set, [.regex]`\U` matches a "U"
...
matching is set by the const:PCRE2_CASELESS option or (?i) within the pattern. The
...
If the subject is "foobar", a call to func:pcre2_match() with a starting
...
[.regex]`\ba` matches "a" at the start of a word.
...
_startoffset_ is non-zero. By calling func:pcre2_match() multiple times
...
For example, the pattern [.regex]`/^abc$/` matches the subject string "def\nabc" (where
\n represents a newline) in multiline mode, but not otherwise. Consequently,
patterns that are anchored in single line mode because all branches start with
^ are not anchored in multiline mode, and a match for circumflex is possible
when the _startoffset_ argument of func:pcre2_match() is non-zero. The
...
[.regex]`\N{U+hh..}` in the usual way. When caseless matching is set, any letters in a
...
including [.regex]`(*CASELESS_RESTRICT)` or (?r) within a pattern.
...
classes always have the Perl /xx modifier (PCRE2 option const:PCRE2_EXTENDED_MORE)
...
The binary operators are "&" (intersection), "|" or [.regex]`+` (union), "-"
...
The const:PCRE2_ALT_EXTENDED_CLASS option enables an alternative to Perl's (?[...])
...
Unlike Perl's (?[...]) extended classes, the const:PCRE2_EXTENDED_MORE option to
...
(?aT) and (?-aT). The const:PCRE2_EXTRA_ASCII_POSIX option disables UCP processing
...
A combined setting and unsetting such as (?im-sx), which sets const:PCRE2_CASELESS
...
matches abc and aBc and no other strings (assuming const:PCRE2_CASELESS is not set
...
because braces are used in other items such as [.regex]`\N{U+345}` or \k{name}.
...
action of the matcher is to try again with only 5 digits matching the [.regex]`\d+`
...
So, while both [.regex]`\d+` and [.regex]`\d+?` are prepared to adjust the number of digits they
...
pattern constructs. For example, the sequence [.regex]`A+B` is treated as [.regex]`A++B` because
...
be divided between the internal [.regex]`\D+` repeat and the external [.regex]`*` repeat in a
...
support the use of [.regex]`&#43;` in this way.
...
[.regex]`(*` instead of (? and must be written using lower case letters. PCRE2 supports
...
For example, [.regex]`(*pla:foo)` is the same assertion as (?=foo). In the following
...
The backtracking control verb [.regex]`(*FAIL)` or [.regex]`(*F)` is a synonym for (?!).
...
(for example [.regex]`\d*`) is not supported. In some cases, the escape sequence \K
(see above)
...
there can be no backtracking for the [.regex]`.*+` item because of the possessive
...
fails. If a traditional atomic lookahead (?= or [.regex]`(*pla:` had been used, the
...
ways the [.regex]`+` and [.regex]`*` repeats can carve up the subject, and all have to be tested
...
If run with the const:PCRE2_CASELESS option, this pattern matches phrases such as "A
man, a plan, a canal: Panama!". Note the use of the possessive quantifier [.regex]`*+` to
...
are generally of the form [.regex]`(*VERB)` or [.regex]`(*VERB:NAME)`. Some verbs take either form,
...
processes "BC"; if that fails, causing a backtrack, [.regex]`(*ACCEPT)` is triggered and
...
abbreviated to [.regex]`(*F)`. It is equivalent to (?!) but easier to read. The Perl
...
[.regex]`(*ACCEPT:NAME)` and [.regex]`(*FAIL:NAME)` behave the same as [.regex]`(*MARK:NAME)(*ACCEPT)` and
[.regex]`(*MARK:NAME)(*FAIL)`, respectively, that is, a [.regex]`(*MARK)` is recorded just before
the verb acts.
...
The behaviour of [.regex]`(*COMMIT:NAME)` is not the same as [.regex]`(*MARK:NAME)(*COMMIT)`. It is
like [.regex]`(*MARK:NAME)` in that the name is remembered for passing back to the
caller. However, [.regex]`(*SKIP:NAME)` searches only for names that are set with
...
applied starting at "x", and so the [.regex]`(*COMMIT)` causes the match to fail without
...
The behaviour of [.regex]`(*PRUNE:NAME)` is not the same as [.regex]`(*MARK:NAME)(*PRUNE)`. It is
like [.regex]`(*MARK:NAME)` in that the name is remembered for passing back to the
caller. However, [.regex]`(*SKIP:NAME)` searches only for names set with [.regex]`(*MARK)`,
...
Note that [.regex]`(*SKIP:NAME)` searches only for names set by [.regex]`(*MARK:NAME)`. It ignores
...
The behaviour of [.regex]`(*THEN:NAME)` is not the same as [.regex]`(*MARK:NAME)(*THEN)`. It is
like [.regex]`(*MARK:NAME)` in that the name is remembered for passing back to the
caller. However, [.regex]`(*SKIP:NAME)` searches only for names set with [.regex]`(*MARK)`,
ignoring those set by other backtracking verbs.
...
If the subject is "ba", this pattern does not match. Because [.regex]`.*?` is ungreedy,
...
into [.regex]`.*?`, allowing it to match "b", the match would succeed.)
...
assertion is not "seen" by an instance of [.regex]`(*SKIP:NAME)` later in the pattern.
...
When PCRE2 is compiled in EBCDIC mode, [.regex]`\N{U+hhh..}` is not supported. \a, \e,
