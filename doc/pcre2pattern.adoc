...
sequence (pass:[*]UTF), which is equivalent to setting the relevant PCRE2_UTF. How
...
option is passed to `pcre2_compile()`, (pass:[*]UTF) is not allowed, and its
appearance in a pattern causes an error.

=== Unicode property support

Another special sequence that may appear at the start of a pattern is (pass:[*]UCP).
...
`pcre2_compile()`, (pass:[*]UCP) is not allowed, and its appearance in a pattern
causes an error.

=== Locking out empty string matching

Starting a pattern with (pass:[*]NOTEMPTY) or (pass:[*]NOTEMPTY_ATSTART) has the same effect
...
If a pattern starts with (pass:[*]NO_AUTO_POSSESS), it has the same effect as setting
...
default apass:[+]b is treated as apass:[+]pass:[+]b. For more details, see the
xref:pcre2api.adoc[`pcre2api`]
documentation.

=== Disabling start-up optimizations

If a pattern starts with (pass:[*]NO_START_OPT), it has the same effect as setting the
...
If a pattern starts with (pass:[*]NO_DOTSTAR_ANCHOR), it has the same effect as
...
all start with .pass:[*] (match any number of arbitrary characters). For more details,
see the
xref:pcre2api.adoc[`pcre2api`]
documentation.
...
If a pattern that starts with (pass:[*]NO_JIT) is successfully compiled, an attempt by
...
(pass:[*]BSR_ANYCRLF). For completeness, (pass:[*]BSR_UNICODE) is also recognized,
...
equivalents, are case-equivalent with Unicode Upass:[+]212A (Kelvin sign) and Upass:[+]017F
...
`pcre2_compile()` or set by (pass:[*]CASELESS_RESTRICT) or (?r) within the
pattern). If the PCRE2_EXTRA_TURKISH_CASING option is in force (either passed
to `pcre2_compile()` or set by (pass:[*]TURKISH_CASING) within the pattern), then
...
For example, if you want to match a pass:[*] character, you must write pass:[\*] in the
...
The \N{Upass:[+]hhh..} escape sequence is recognized only when PCRE2 is operating in
...
LF, or one of the single characters LF (linefeed, Upass:[+]000A), VT (vertical tab,
Upass:[+]000B), FF (form feed, Upass:[+]000C), CR (carriage return, Upass:[+]000D), or NEL (next
line, Upass:[+]0085). Because this is an atomic group, the two-character sequence is
treated as a single unit that cannot be split.

In other modes, two additional characters whose code points are greater than 255
are added: LS (line separator, Upass:[+]2028) and PS (paragraph separator, Upass:[+]2029).
...
They can also be combined with the (pass:[*]UTF) or (pass:[*]UCP) special sequences. Inside a
...
less than Upass:[+]0100 or Upass:[+]10000, respectively. In 32-bit non-UTF mode, code points
...
PCRE2_UCP option or by starting the pattern with (pass:[*]UCP).
...
the range Upass:[+]D800 to Upass:[+]DFFF. These characters are no different to any other
...
pass:[1.] End at the end of the subject string.

pass:[2.] Do not end between CR and LF; otherwise end after any control character.

pass:[3.] Do not break Hangul (a Korean script) syllable sequences. Hangul characters
are of five types: L, V, T, LV, and LVT. An L character may be followed by an
L, V, LV, or LVT character; an LV or V character may be followed by a V or T
character; an LVT or T character may be followed only by a T character.

pass:[4.] Do not end before extending characters or spacing marks or the zero-width
joiner (ZWJ) character. Characters with the "mark" property always have the
"extend" grapheme breaking property.

pass:[5.] Do not end after prepend characters.

pass:[6.] Do not end within emoji modifier sequences or emoji ZWJ (zero-width
...
pass:[7.] Do not break within emoji flag sequences. That is, do not break between
regional indicator (RI) characters if there are an odd number of RI characters
before the break point.

pass:[8.] Otherwise, end the cluster.
...
can be represented by a Universal Character Name in Cpass:[+]pass:[+] and other programming
languages. These are the characters $, @, ` (grave accent), and all characters
with Unicode code points greater than or equal to Upass:[+]00A0, except for the
surrogates Upass:[+]D800 to Upass:[+]DFFF. Note that most base (ASCII) characters are
...
_startoffset_ is non-zero. By calling `pcre2_match()` multiple times
...
when the _startoffset_ argument of `pcre2_match()` is non-zero. The
...
\N{Upass:[+]hh..} in the usual way. When caseless matching is set, any letters in a
...
are case-equivalent with Unicode Upass:[+]212A (Kelvin sign) and Upass:[+]017F (long S)
...
including (pass:[*]CASELESS_RESTRICT) or (?r) within a pattern.
...
The binary operators are "&" (intersection), "|" or "pass:[+]" (union), "-"
...
is a syntax error. For example, [A&&Bpass:[--]C] is an error, but
[A&&[Bpass:[--]C]] is valid.
...
versions of those characters, whose Unicode code points start at Upass:[+]FF10. This
...
(pass:[*]CRLF) to override what the application has set or what has been defaulted.
Details are given in the section entitled
"Newline sequences"
above. There are also the (pass:[*]UTF) and (pass:[*]UCP) leading sequences that can be used
...
(pass:[*]UTF) and (pass:[*]UCP) sequences.
...
pass:[1.] It localizes a set of alternatives. For example, the pattern
...
pass:[2.] It creates a "capture group". This means that, when the whole pattern
...
pass:[2.] The maximum number of capture groups is 65535.
...
because braces are used in other items such as \N{Upass:[+]345} or \k{name}.
...
trying to match comments in C programs. These appear between /pass:[*] and pass:[*]/ and
within the comment, individual pass:[*] and / characters may appear. An attempt to
...
fails, because it matches the entire string owing to the greediness of the .pass:[*]
...
If a pattern starts with .pass:[*] or .{0,} and the PCRE2_DOTALL option (equivalent
...
However, there are some cases where the optimization cannot be used. When .pass:[*]
...
Another case where implicit anchoring is not applied is when the leading .pass:[*] is
...
(pass:[*]PRUNE) and (pass:[*]SKIP) also disable this optimization. To do so explicitly,
...
Consider, for example, the pattern \dpass:[+]foo when applied to the subject line
...
action of the matcher is to try again with only 5 digits matching the \dpass:[+]
...
Perl 5.28 introduced an experimental alphabetic form starting with (pass:[*] which may
...
So, while both \dpass:[+] and \dpass:[+]? are prepared to adjust the number of digits they
match in order to make the rest of the pattern match, (?>\dpass:[+]) can only match
...
additional pass:[+] character following a quantifier. Using this notation, the
...
pattern constructs. For example, the sequence Apass:[+]B is treated as Apass:[+]pass:[+]B because
...
starting the pattern with (pass:[*]NO_AUTO_POSSESS).
...
be divided between the internal \Dpass:[+] repeat and the external pass:[*] repeat in a
...
The sequence \g{pass:[+]1} is a reference to the next capture group that is started
after this item, and \g{pass:[+]2} refers to the one after that, and so on. This kind
of forward reference can be useful in patterns that repeat. Perl does not
support the use of pass:[+] in this way.
...
(pass:[*] instead of (? and must be written using lower case letters. PCRE2 supports
...
For example, (pass:[*]pla:foo) is the same assertion as (?=foo). In the following
...
The backtracking control verb (pass:[*]FAIL) or (pass:[*]F) is a synonym for (?!).
...
(for example \dpass:[*]) is not supported. In some cases, the escape sequence \K
(see above)
...
the initial .pass:[*] matches the entire string at first, but when this fails (because
...
there can be no backtracking for the .pass:[*]pass:[+] item because of the possessive
...
ignored. Inside the assertion, the greedy .pass:[*] at first consumes the entire
...
using an ungreedy .pass:[*]? to scan from the left. If this succeeds, we are done, but
if the last word in the string does not occur twice, this part of the pattern
fails. If a traditional atomic lookahead (?= or (pass:[*]pla: had been used, the
...
(pass:[*]ACCEPT) control verb is triggered, the assertion succeeds atomically. That
...
contain any control verbs such as (pass:[*]ACCEPT). (This may change in future). Note
...
A scan substring assertion starts with the sequence (pass:[*]scan_substring: or
(pass:[*]scs: which is followed by a list of substring numbers (absolute or relative)
...
If part of a pattern is enclosed between (pass:[*]script_run: or (pass:[*]sr: and a closing
...
(pass:[*]atomic_script_run: or (pass:[*]asr:
...
`Warning:` The (pass:[*]ACCEPT) control verb
(see below)
...
The next capture group to be opened can be referenced as (?(pass:[+]1), and so on. The
...
overall pattern. This is a contrived example that is equivalent to apass:[+]b:
...
references such as (?pass:[+]2). However, these cannot be recursive because the
...
ways the pass:[+] and pass:[*] repeats can carve up the subject, and all have to be tested
...
man, a plan, a canal: Panama!". Note the use of the possessive quantifier pass:[*]pass:[+] to
...
are generally of the form (pass:[*]VERB) or (pass:[*]VERB:NAME). Some verbs take either form,
...
(pass:[*]ACCEPT), they may not be quantified.
...
of (pass:[*]FAIL), which behaves like a failing negative assertion, the backtracking
...
(pass:[*]NO_START_OPT). There is more discussion of this option in the section
entitled
"Compiling a pattern"
in the
xref:pcre2api.adoc[`pcre2api`]
...
at the outer level. If (pass:[*]ACCEPT) in triggered in a positive assertion, the
assertion succeeds; in a negative assertion, the assertion fails.

If (pass:[*]ACCEPT) is inside capturing parentheses, the data so far is captured. For
example:
...
(pass:[*]ACCEPT) is the only backtracking verb that is allowed to be quantified
...
processes "BC"; if that fails, causing a backtrack, (pass:[*]ACCEPT) is triggered and
the match succeeds. In both cases, all but C is captured. Whereas (pass:[*]COMMIT)
(see below) means "fail on backtrack", a repeated (pass:[*]ACCEPT) of this type means
"succeed on backtrack".

`Warning:` (pass:[*]ACCEPT) should not be used within a script run group, because
...
abbreviated to (pass:[*]F). It is equivalent to (?!) but easier to read. The Perl
...
(pass:[*]ACCEPT:NAME) and (pass:[*]FAIL:NAME) behave the same as (pass:[*]MARK:NAME)(pass:[*]ACCEPT) and
(pass:[*]MARK:NAME)(pass:[*]FAIL), respectively, that is, a (pass:[*]MARK) is recorded just before
the verb acts.
...
starting point (see (pass:[*]SKIP) below).
...
in the
xref:pcre2api.adoc[`pcre2api`]
documentation. This applies to all instances of (pass:[*]MARK) and other verbs,
including those inside assertions and atomic groups. However, there are
differences in those cases when (pass:[*]MARK) is used in conjunction with (pass:[*]SKIP) as
described below.
...
The (pass:[*]MARK) name is tagged with "MK:" in this output, and in this example it
...
(pass:[*]MARK) item, but nevertheless do not reset it.

If you are interested in (pass:[*]MARK) values after failed matches, you should
...
take place. If (pass:[*]COMMIT) is the only backtracking verb that is encountered,
...
The behaviour of (pass:[*]COMMIT:NAME) is not the same as (pass:[*]MARK:NAME)(pass:[*]COMMIT). It is
like (pass:[*]MARK:NAME) in that the name is remembered for passing back to the
caller. However, (pass:[*]SKIP:NAME) searches only for names that are set with
(pass:[*]MARK), ignoring those set by any of the other backtracking verbs.

If there is more than one backtracking verb in a pattern, a different one that
follows (pass:[*]COMMIT) may be triggered first, so merely passing (pass:[*]COMMIT) during a
match does not always guarantee that a match must be at this starting point.

Note that (pass:[*]COMMIT) at the start of a pattern is not the same as an anchor,
...
applied starting at "x", and so the (pass:[*]COMMIT) causes the match to fail without
...
(pass:[*]PRUNE), before it is reached, or when matching to the right of (pass:[*]PRUNE), but
if there is no match to the right, backtracking cannot cross (pass:[*]PRUNE). In
simple cases, the use of (pass:[*]PRUNE) is just an alternative to an atomic group or
possessive quantifier, but there are some uses of (pass:[*]PRUNE) that cannot be
expressed in any other way. In an anchored pattern (pass:[*]PRUNE) has the same effect
as (pass:[*]COMMIT).

The behaviour of (pass:[*]PRUNE:NAME) is not the same as (pass:[*]MARK:NAME)(pass:[*]PRUNE). It is
like (pass:[*]MARK:NAME) in that the name is remembered for passing back to the
caller. However, (pass:[*]SKIP:NAME) searches only for names set with (pass:[*]MARK),
...
This verb, when given without a name, is like (pass:[*]PRUNE), except that if the
pattern is unanchored, the "bumpalong" advance is not to the next character,
but to the position in the subject where (pass:[*]SKIP) was encountered. (pass:[*]SKIP)
...
If (pass:[*]SKIP) is used to specify a new starting position that is the same as the
starting position of the current match, or (by being inside a lookbehind)
earlier, the position specified by (pass:[*]SKIP) is ignored, and instead the normal
"bumpalong" occurs.
...
When (pass:[*]SKIP) has an associated name, its behaviour is modified. When such a
(pass:[*]SKIP) is triggered, the previous path through the pattern is searched for the
most recent (pass:[*]MARK) that has the same name. If one is found, the "bumpalong"
advance is to the subject position that corresponds to that (pass:[*]MARK) instead of
to where (pass:[*]SKIP) was encountered. If no (pass:[*]MARK) with a matching name is found,
the (pass:[*]SKIP) is ignored.

The search for a (pass:[*]MARK) name uses the normal backtracking mechanism, which
means that it does not see (pass:[*]MARK) settings that are inside atomic groups or
...
In the first example, the (pass:[*]MARK) setting is in an atomic group, so it is not
seen when (pass:[*]SKIP:X) triggers, causing the (pass:[*]SKIP) to be ignored. This allows
the second branch of the pattern to be tried at the first character position.
In the second example, the (pass:[*]MARK) setting is not in an atomic group. This
allows (pass:[*]SKIP:X) to find the (pass:[*]MARK) when it backtracks, and this causes a new
matching attempt to start at the second character. This time, the (pass:[*]MARK) is
never seen because "a" does not match "b", so the matcher immediately jumps to
the second branch of the pattern.

Note that (pass:[*]SKIP:NAME) searches only for names set by (pass:[*]MARK:NAME). It ignores
...
group. If (pass:[*]THEN) is not inside an alternation, it acts like (pass:[*]PRUNE).

The behaviour of (pass:[*]THEN:NAME) is not the same as (pass:[*]MARK:NAME)(pass:[*]THEN). It is
like (pass:[*]MARK:NAME) in that the name is remembered for passing back to the
caller. However, (pass:[*]SKIP:NAME) searches only for names set with (pass:[*]MARK),
ignoring those set by other backtracking verbs.
...
effect of (pass:[*]THEN) extends beyond such a group to the enclosing alternative.
...
However, if the group containing (pass:[*]THEN) is given an alternative, it
behaves differently:
...
The effect of (pass:[*]THEN) is now confined to the inner group. After a failure in C,
matching moves to (pass:[*]FAIL), which causes the whole group to fail because there
...
If the subject is "ba", this pattern does not match. Because .pass:[*]? is ungreedy,
it initially matches zero characters. The condition (?=a) then fails, the
character "b" is matched, but "c" is not. At this point, matching does not
backtrack to .pass:[*]? as might perhaps be expected from the presence of the |
character. The conditional group is part of the single alternative that
comprises the whole pattern, and so the match fails. (If there was a backtrack
into .pass:[*]?, allowing it to match "b", the match would succeed.)

The verbs just described provide four different "strengths" of control when
subsequent matching fails. (pass:[*]THEN) is the weakest, carrying on the match at the
next alternative. (pass:[*]PRUNE) comes next, failing the match at the current
starting position, but allowing an advance to the next character (for an
unanchored pattern). (pass:[*]SKIP) is similar, except that the advance may be more
than one character. (pass:[*]COMMIT) is the strongest, causing the entire match to
fail.
...
If A matches but B fails, the backtrack to (pass:[*]COMMIT) causes the entire match to
fail. However, if A and B match, but C fails, the backtrack to (pass:[*]THEN) causes
...
If there is a matching failure to the right, backtracking onto (pass:[*]PRUNE) causes
...
but PCRE2 always fails because the (pass:[*]COMMIT) in the second repeat of the group
acts.

=== Backtracking verbs in assertions

(pass:[*]FAIL) in any assertion has its normal effect: it forces an immediate
...
(pass:[*]ACCEPT) in a standalone positive assertion causes the assertion to succeed
without any further processing; captured strings and a mark name (if set) are
retained. In a standalone negative assertion, (pass:[*]ACCEPT) causes the assertion to
fail without any further processing; captured substrings and any mark name are
discarded.

If the assertion is a condition, (pass:[*]ACCEPT) causes the condition to be true for
...
into the assertion. Note in particular that a (pass:[*]MARK) name that is set in an
assertion is not "seen" by an instance of (pass:[*]SKIP:NAME) later in the pattern.
...
into the assertion, and therefore verbs such as (pass:[*]COMMIT) can be triggered by
backtracks from later in the pattern.

The effect of (pass:[*]THEN) is not allowed to escape beyond an assertion. If there
are no more branches to try, (pass:[*]THEN) causes a positive assertion to be false,
...
backtracking (from within the assertion) into (pass:[*]COMMIT), (pass:[*]SKIP), or (pass:[*]PRUNE)
causes the condition to be false. However, for both standalone and conditional
negative assertions, backtracking into (pass:[*]COMMIT), (pass:[*]SKIP), or (pass:[*]PRUNE) causes
...
(pass:[*]ACCEPT) in a group called as a subroutine causes the subroutine match to
...
(pass:[*]FAIL) in a group called as a subroutine has its normal effect: it forces
an immediate backtrack.

(pass:[*]COMMIT), (pass:[*]SKIP), and (pass:[*]PRUNE) cause the subroutine match to fail when
triggered by being backtracked to in a group called as a subroutine. There is
then a backtrack at the outer level.

(pass:[*]THEN), when triggered, skips to the next alternative in the innermost
...
When PCRE2 is compiled in EBCDIC mode, \N{Upass:[+]hhh..} is not supported. \a, \e,
