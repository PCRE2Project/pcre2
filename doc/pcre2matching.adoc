...
pattern "a\dpass:[+]" is compiled as if it were "a\dpass:[+]pass:[+]" because there is no point
even considering the possibility of backtracking into the repeated digits. For
DFA matching, this means that only one possible match is found. If you really
do want multiple matches in such cases, either use an ungreedy repeat
("a\dpass:[+]?") or set the PCRE2_NO_AUTO_POSSESS option when compiling.
...
pass:[1.] Because the algorithm finds all possible matches, the greedy or ungreedy
...
pass:[2.] When dealing with multiple paths through the tree simultaneously, it is not
straightforward to keep track of captured substrings for the different matching
possibilities, and PCRE2's implementation of this algorithm does not attempt to
do this. This means that no captured substrings are available.

pass:[3.] Because no substrings are captured, a number of related features are not
available:
...
pass:[4.] Because many paths through the tree may be active, the \K escape sequence,
which resets the start of the match when encountered (but may be on some paths
and not on others), is not supported.

pass:[5.] Callouts are supported, but the value of the _capture_top_ field is
always 1, and the value of the _capture_last_ field is always 0.

pass:[6.] The \C escape sequence, which (in the standard algorithm) always matches a
...
pass:[7.] Except for (pass:[*]FAIL), the backtracking control verbs such as (pass:[*]PRUNE) are not
supported. (pass:[*]FAIL) is supported, and behaves like a failing negative assertion.

pass:[8.] The PCRE2_MATCH_INVALID_UTF option for `pcre2_compile()` is not
...
pass:[1.] It is substantially slower than the standard algorithm. This is partly
because it has to search for all possible matches, but is also because it is
less susceptible to optimization.

pass:[2.] Capturing parentheses and other features such as backreferences that rely on
them are not supported.

pass:[3.] Matching within invalid UTF strings is not supported.

pass:[4.] Although atomic groups are supported, their use does not provide the
performance advantage that it does for the standard algorithm.

pass:[5.] JIT optimization is not supported.
