...
:description: Discussion of Unicode and UTF-8/UTF-16/UTF-32 support
:manvolnum: 3
:manmanual: PCRE2
...
[.regex]`Lu` for an upper case letter or [.regex]`Nd` for a decimal number, the derived properties
[.regex]`Any` and [.regex]`Lc` (synonym [.regex]`L&`), the Unicode script names such as [.regex]`Arabic` or [.regex]`Han`,
[.regex]`Bidi_Class`, [.regex]`Bidi_Control`, and a few binary properties.
...
[.regex]`Is`, for compatibility with Perl 5.6. PCRE2 does not support this.
...
The escape sequence [.regex]`\N{U{plus}__hex digits__}` is recognized as another way of
...
Without const:PCRE2_EXTRA_CASELESS_RESTRICT::
  'k' = 'K' = char:U+212A (Kelvin sign) +
  's' = 'S' = char:U+017F (long S)
With const:PCRE2_EXTRA_CASELESS_RESTRICT::
  'k' = 'K' +
  char:U+212A (Kelvin sign) -- only case-equivalent to itself +
  's' = 'S' +
  char:U+017F (long S) -- only case-equivalent to itself
...
Without const:PCRE2_EXTRA_TURKISH_CASING::
  'i' = 'I' +
  char:U+0130 (capital I with dot above)  -- only case-equivalent to itself +
  char:U+0131 (small dotless i) -- only case-equivalent to itself
With const:PCRE2_EXTRA_TURKISH_CASING::
  'i' = char:U+0130 (capital I with dot above) +
  char:U+0131 (small dotless i) = 'I'

It is not allowed to specify both const:PCRE2_EXTRA_CASELESS_RESTRICT and
const:PCRE2_EXTRA_TURKISH_CASING together.

From release 10.45 the Unicode letter properties [.regex]`Lu` (upper case), [.regex]`Ll` (lower
case), and [.regex]`Lt` (title case) are all treated as [.regex]`Lc` (cased letter) when caseless
...
points are greater than the Unicode maximum (char:U+10FFFF), which are accessible
...
normally used with a small number of scripts. For example, char:U+102E0 (Coptic
...
property. However, for characters such as char:U+102E0 more than one Script is
listed. There are also some Common characters that have a single, non-Common
script in their Script Extension list.

The section <<basic-script-run-rules>> describes the basic rules for deciding whether a given string
...
A simple example is an Internet name such as [.subject]`google.com`. The letters are all
in the Latin script, and the dot is Common, so this string is a script run.
However, the Cyrillic letter [.subject]`o` looks exactly the same as the Latin [.subject]`o`; a
string that looks the same, but with Cyrillic [.subject]``o``s is not a script run.
...
* char:U+060C, Arabic comma
* char:U+06D4, Arabic full stop
...
func:pcre2_compile() and is discussed below in the section
<<utf8-string-errors>>. The rest of
...
that all code points lie in the range char:U+0 to char:U+10FFFF, excluding the surrogate
...
char:0xFFFF. The code points that are encoded by UTF-16 pairs are available
...
const:PCRE2_ERROR_UTF8_ERR13::
  A 4-byte character has a value greater than char:0x10ffff; these code points are
  excluded by RFC 3629.

const:PCRE2_ERROR_UTF8_ERR14::
  A 3-byte character has a value in the range char:0xd800 to char:0xdfff; this range of
...
const:PCRE2_ERROR_UTF8_ERR19::
  ...
  the two bytes char:0xc0, char:0xae give the value char:0x2e, whose correct coding uses just
  one byte.
...
const:PCRE2_ERROR_UTF8_ERR21::
  The first byte of a character has the value char:0xfe or char:0xff. These values can
...
const:PCRE2_ERROR_UTF32_ERR1::
  Surrogate character (char:0xd800 to char:0xdfff)
const:PCRE2_ERROR_UTF32_ERR2::
  Code point is greater than char:0x10ffff
...
If you do not set const:PCRE2_MATCH_INVALID_UTF when calling func:pcre2_compile(), and
...
would match an instance of [.subject]`WORD` that is surrounded by invalid UTF code units.
