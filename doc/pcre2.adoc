...
:doctype: manpage
:manname: pcre2
:manpurpose: Perl-compatible regular expressions (PCRE2 API)
...
Zoltán Herczeg and Christian Persch, respectively. In all three cases, strings
...
`_8`, `_16`, or `_32`, respectively (for example, func:pcre2_compile_8()). However,
by defining const:PCRE2_CODE_UNIT_WIDTH to be 8, 16, or 32, a program that uses just
one code unit width can be written using generic names such as
func:pcre2_compile(), and the documentation is written assuming that this is
the case.
...
is built. The func:pcre2_config()
...
[.c]``_pcre2``, which hopefully will not provoke any name clashes. In some
...
8-bit pattern that begins with [.regex]`+(*UTF)+` turns on UTF-8 mode, which interprets
...
func:pcre2_pattern_info() function to check the compiled pattern's options for
const:PCRE2_UTF. Alternatively, you can set the const:PCRE2_NEVER_UTF option when calling
func:pcre2_compile(). This causes a compile time error if the pattern contains
a UTF-setting sequence.

The use of Unicode properties for character types such as [.regex]`\d` can also be
enabled from within the pattern, by specifying [.regex]`+(*UCP)+`. This feature can be
disallowed by setting the const:PCRE2_NEVER_UCP option.

If your application is one that supports UTF, be aware that validity checking
can take time. If the same data string is to be matched many times, you can use
the const:PCRE2_NO_UTF_CHECK option for the second and subsequent matches to avoid
running redundant checks.

The use of the [.regex]`\C` escape sequence in a UTF-8 or UTF-16 pattern can lead to
problems, because it may leave the current matching point in the middle of a
multi-code-unit character. The const:PCRE2_NEVER_BACKSLASH_C option can be used by an
application to lock out the use of [.regex]`\C`, causing a compile-time error if it is
encountered. It is also possible to build PCRE2 with the use of [.regex]`\C` permanently
disabled.

Another way that performance can be hit is by running a pattern that has a very
large search tree against a string that will never match. Nested unlimited
repeats in a pattern are a common example. PCRE2 provides some protection
against this: see the func:pcre2_set_match_limit() function in the
xref:pcre2api.adoc[`pcre2api`]
page. There is a similar function called func:pcre2_set_depth_limit() that can
...
pcre2pattern::
    syntax and semantics of supported regular
    expression patterns
pcre2perform::
...
The current maintainers of PCRE2 are Nicholas Wilson and Zoltán Herczeg.
