...
set the PCRE2_UCP option or start the pattern with (pass:[*]UCP) if you want Unicode
...
When a pattern begins with .pass:[*] not in atomic parentheses, nor in parentheses
...
contains (pass:[*]PRUNE) or (pass:[*]SKIP).
...
the pattern with ^.pass:[*] or ^.pass:[*]? to indicate explicit anchoring. That saves PCRE2
...
rapidly as the string gets longer. (The pass:[*] repeat can match 0, 1, 2, 3, or 4
times, and for each of those cases other than 0 or 4, the pass:[+] repeats can match
