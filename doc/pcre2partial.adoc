...
:description: Using PCRE2 for partial matching
:manvolnum: 3
:manmanual: PCRE2
...
Example 1: if the pattern is [.regex]`abc` and the subject is [.subject]`ab`, more characters are
definitely needed to complete a match. In this case both hard and soft matching
options yield a partial match.

Example 2: if the pattern is [.regex]`ab+` and the subject is [.subject]`ab`, a complete match
...
. At least one character has already been inspected. An inspected character
  need not form part of the final matched string; lookbehind assertions and the
  [.regex]`\K` escape sequence provide ways of inspecting characters before the start of a
  matched string.

. The pattern contains one or more lookbehind assertions. This condition
  exists in case there is a lookbehind that inspects characters before the start
  of the match.

. There is a special case when the whole pattern can match an empty string.
...
If it is matched against [.subject]`456abc123xyz` the result is a complete match, and the
ovector defines the matched string as [.subject]`123`, because [.regex]`\K` resets the "start of
match" point. However, if a partial match is requested and the subject string
is [.subject]`456abc12`, a partial match is found for the string [.subject]`abc12`, because all
...
If this is matched against the subject string [.subject]`abc123dog`, both alternatives
...
[.subject]`123dog` as the first partial match. (In this example, there are two partial
matches, because [.subject]`dog` on its own partially matches the second alternative.)
...
This matches either [.subject]`dog` or [.subject]`dogsbody`, greedily (that is, it prefers the
longer string if possible). If it is matched against the string [.subject]`dog` with
const:PCRE2_PARTIAL_SOFT, it yields a complete match for [.subject]`dog`. However, if
...
* [.regex]`dog(sbody)?`    is the same as  [.regex]`dogsbody|dog`
* [.regex]`dog(sbody)??`   is the same as  [.regex]`dog|dogsbody`

The second pattern will never match [.subject]`dogsbody`, because it will always find the
shorter match first.
...
different from func:pcre2_match(). Consider the string [.subject]`dog` matched
...
[.subject]`dog`, the DFA function also finds the partial match for [.subject]`dogsbody`, and so
...
The first call has [.subject]`23ja` as the subject, and requests partial matching; the
second call has [.subject]`n05` as the subject for the continued (restarted) match.
...
If the first part of the subject is [.subject]`ABC123`, a partial match of the first
alternative is found at offset 3. There is no partial match for the second
alternative, because such a match does not start at the same point in the
subject string. Attempting to continue with the string [.subject]`7890` does not yield a
...
can then try a new match starting at offset _n_+1 in the first buffer.
