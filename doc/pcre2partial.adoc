...
Example 2: if the pattern is /abpass:[+]/ and the subject is "ab", a complete match
...
subject and the match re-run, using the _startoffset_ argument of
...
pattern such as "pass:[(?<=(?<!b)a)]" has a maximum lookbehind value of one, but
...
can then try a new match starting at offset _npass:[+]1_ in the first buffer.
