...
If arg:buffer is NULL, the function just returns the length of the converted
...
If arg:buffer points to a NULL pointer, an output buffer is obtained using
...
func:pcre2_converted_pattern_free(). If this function is called with a NULL
argument, it returns immediately without doing anything.

If arg:buffer points to a non-NULL pointer, arg:blength must be set to the
...
In basic patterns, [.regex]`? + | {} ()` must be escaped to be recognized
