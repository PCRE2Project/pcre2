...
3. Via Meson (Linux)
    ...
    See [the Meson build documentation](./meson/README.md) for options and
    integration details.
...
PCRE2 releases support CMake for building, Meson on Linux, and for UNIX platforms include a `./configure` script built by Autoconf. Build files for the Bazel build system and `zig build` are also included. Integrating PCRE2 with other systems can be done by including the `.c` files in an existing project.
